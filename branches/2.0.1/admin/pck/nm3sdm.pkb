...
--   PVCS Identifiers :-
--
--       sccsid           : $Header:   //vm_latest/archives/nm3/admin/pck/nm3sdm.pkb-arc   2.0.1.0   Sep 25 2007 11:55:40   malexander  $
--       Module Name      : $Workfile:   nm3sdm.pkb  $
--       Date into SCCS   : $Date:   Sep 25 2007 11:55:40  $
--       Date fetched Out : $Modtime:   Sep 25 2007 11:03:18  $
...
--
   g_body_sccsid     CONSTANT VARCHAR2 (2000) := '@(#)nm3sdm.pkb	2.0.1.0 25/09/07';
...
---------------------------------------------------------------------------------------------------
--
   PROCEDURE process_inv_sdo_data
   IS
-- This process is designed to handle all asset changes after a trigger has stored
-- the affected data.
-- Note that this cannot be triggered to handle changes to assets when a network
-- operation takes place because the new element shape will not be created at a point
-- when the member records are triggered.
      CURSOR c1
      IS
         SELECT DISTINCT nstm_ne_id_in, nstm_obj_type, nstm_operation,
                         nm_start_date, nm_end_date
                    FROM NM_SDO_TRG_MEMBERS, NM_MEMBERS_ALL nm
                   WHERE nm.ROWID = nstm_rowid
                     AND nstm_operation IN
                            ('D', 'E')
                                -- eam change - only end-date where necessary.
                ORDER BY DECODE (nstm_operation, 'D', 10, 'E', 20, 'I', 30);
      ...
      CURSOR c3
      IS                  --cursor to retrieve all affected continuous objects
         SELECT   nstm_ne_id_in, nstm_obj_type, MAX (nm_start_date)
                                                                   start_date,
                  nstm_ne_id_of, nm_begin_mp, nm_end_mp
             FROM NM_SDO_TRG_MEMBERS, NM_MEMBERS_ALL nm, nm_inv_types
            WHERE nstm_operation = 'I'
              AND nm_ne_id_in = nstm_ne_id_in
              AND nit_inv_type = nstm_obj_type
              AND nit_pnt_or_cont = 'C'
         GROUP BY nstm_ne_id_in, nstm_obj_type;

      CURSOR c4
      IS                       --cursor to retrieve all affected point objects
         SELECT   nstm_ne_id_in, nstm_ne_id_of, nm_begin_mp, nstm_obj_type,
                  MAX (nm_start_date) start_date
             FROM NM_SDO_TRG_MEMBERS, NM_MEMBERS_ALL nm, nm_inv_types
            WHERE nstm_operation = 'I'
              AND nm_ne_id_in = nstm_ne_id_in
              AND nit_inv_type = nstm_obj_type
              AND nit_pnt_or_cont = 'P'
         GROUP BY nstm_ne_id_in, nstm_ne_id_of, nm_begin_mp, nstm_obj_type;

      CURSOR c5
      IS
     --cursor to retrieve all affected distinct assets for single shape assets
         SELECT DISTINCT nstm_ne_id_in, nstm_obj_type,
...
--   nm_debug.debug('End-date the previous shape');
         set_obj_shape_end_date (irec.nstm_obj_type,
                                 irec.nstm_ne_id_in,
                                 irec.nm_start_date
                                );
      END LOOP;

-- If the object location has changed then an insert will follow.- careful with
-- continuous data - it could have many affected members - only one shape

      --   nm_debug.debug('----- Continuous data--------');
      IF Nm3sdo.single_shape_inv = 'N'
      THEN
         FOR irec IN c3
         LOOP
--        nm_debug.debug('Affected object '||irec.nstm_obj_type||' Id - '||to_char(irec.nstm_ne_id_in) );
            FOR jrec IN c2 (irec.nstm_obj_type)
            LOOP
--          nm_debug.debug('get new object shape'||irec.nstm_obj_type||' Id - '||to_char(irec.nstm_ne_id_in) );
               l_geom :=
                  Nm3sdo.get_pt_shape_from_ne (jrec.nbth_base_theme,
                                               irec.nstm_ne_id_of,
                                               irec.nm_begin_mp
                                              );
               ...
               IF l_geom IS NOT NULL
               THEN
--            nm_debug.debug('Insert new object shape'||irec.nstm_obj_type||' Id - '||to_char(irec.nstm_ne_id_in) );
                  insert_obj_shape (irec.nstm_obj_type,
                                    irec.nstm_ne_id_in,
                                    irec.start_date,
                                    NULL,
                                    l_geom
                                   );
--will not work on multi-shape inv - needs the member details per shape
               END IF;
            END LOOP;
         END LOOP;

         -- Now deal with the point data
            --   nm_debug.debug('----- Point data--------');
         FOR irec IN c4
         LOOP
            --   nm_debug.debug('Affected object '||irec.nstm_obj_type||' Id - '||to_char(irec.nstm_ne_id_in)||
            --  'on base element '||to_char( irec.nstm_ne_id_of )||' measure - '||to_char( irec.nm_begin_mp ) );
            FOR jrec IN c2 (irec.nstm_obj_type)
            LOOP
               --   nm_debug.debug('get new object shape'||irec.nstm_obj_type||' Id - '||to_char(irec.nstm_ne_id_in) );
               l_geom :=
...
/* Procedure to deal with creating subordinate user metadata triggered on
   nm_theme_roles data */
   IS
      CURSOR c1 (c_role IN VARCHAR2, c_theme IN NUMBER)
      IS
         SELECT grantee, nthr_theme_id
           FROM dba_role_privs, HIG_USERS, NM_THEME_ROLES a
          WHERE granted_role = c_role
            AND hus_username = grantee
            AND hus_username != Hig.get_application_owner
            AND a.nthr_role = granted_role
            AND a.nthr_theme_id = c_theme;
--         AND NOT EXISTS (
--                SELECT 1
--                  FROM NM_THEME_ROLES b
--                 WHERE b.nthr_role != A.nthr_role
--                   AND b.nthr_theme_id = A.nthr_theme_id);
   BEGIN
--   Nm_Debug.debug_on;
      Nm_Debug.DEBUG (   'in process subuser nthr - index = '
                      || TO_CHAR (Nm3sdm.g_role_idx)
                     );
      ...
      LOOP
         ...
         THEN
            FOR irec IN c1 (Nm3sdm.g_role_array (i), Nm3sdm.g_theme_role (i))
            LOOP
               Nm_Debug.DEBUG (   'Creating SDO layer for '
                               || irec.nthr_theme_id
                               || '-'
                               || irec.grantee
                              );
               Nm3sdo.create_sub_sdo_layer (p_theme_id      => irec.nthr_theme_id,
                                            p_username      => irec.grantee
                                           );
            END LOOP;
         ...
         THEN
            FOR irec IN c1 (Nm3sdm.g_role_array (i), Nm3sdm.g_theme_role (i))
            LOOP
               Nm3sdo.remove_sub_sdo_layer (p_theme_id      => irec.nthr_theme_id,
                                            p_username      => irec.grantee
                                           );
            END LOOP;
         END IF;
...
/* Procedure to deal with creating subordinate user metadata triggered on
   nm_theme_roles data */
   IS
      CURSOR c1 (c_role IN VARCHAR2, c_username IN VARCHAR2)
      IS
         SELECT grantee, nthr_theme_id
           FROM dba_role_privs, HIG_USERS, NM_THEME_ROLES a
          WHERE granted_role = c_role
            AND hus_username = grantee
            AND hus_username = c_username
            AND hus_username != Hig.get_application_owner
            AND a.nthr_role = granted_role;
--         AND NOT EXISTS (
--                SELECT 1
...
--                   AND b.nthr_theme_id = A.nthr_theme_id);
   BEGIN
   nm_debug.debug_on;
      Nm_Debug.DEBUG (   'in process subuser hurr - index = '||TO_CHAR (Nm3sdm.g_role_idx));
      ...
      LOOP
         ...
         THEN
         --
            FOR irec IN c1 (Nm3sdm.g_role_array (i),Nm3sdm.g_username_array (i))
            LOOP
               Nm_Debug.DEBUG ('Creating SDO data for '||irec.nthr_theme_id||' - '||irec.grantee);
               Nm3sdo.create_sub_sdo_layer (p_theme_id      => irec.nthr_theme_id,
                                            p_username      => irec.grantee
                                           );
            END LOOP;
