...
--
FUNCTION join_gdo_array( p_session_id in number, p_ntl IN nm_theme_list  ) RETURN nm_theme_list  IS
curstr VARCHAR2(2000);
retval nm_theme_list := Nm3array.init_nm_theme_list;
BEGIN

curstr := 'select /*+cardinality( a '||to_char( p_ntl.ntl_theme_list.last)||') */ '||
          ' nm_theme_detail( a.ntd_theme_id, a.ntd_pk_id, a.ntd_fk_id,  a.ntd_name, a.ntd_distance, a.ntd_measure, a.ntd_descr )   '||
                  ' FROM TABLE ( :p_ntl.ntl_theme_list ) a, gis_data_objects '||
                 ' where a.ntd_pk_id  = gdo_pk_id and gdo_session_id = :session_id order by  a.ntd_distance';
