...
--   PVCS Identifiers :-
--
--       sccsid           : $Header:   //vm_latest/archives/nm3/admin/pck/nm3rsc.pkb-arc   2.4.1.6   Apr 17 2013 15:31:10   Rob.Coupe  $
--       Module Name      : $Workfile:   nm3rsc.pkb  $
--       Date into PVCS   : $Date:   Apr 17 2013 15:31:10  $
--       Date fetched Out : $Modtime:   Apr 17 2013 15:28:32  $
--       PVCS Version     : $Revision:   2.4.1.6  $
--
--   Author : R.A. Coupe
--
...
--
   g_body_sccsid     CONSTANT  varchar2(30) :='"$Revision:   2.4.1.6  $"';
...
--CURSOR c2 IS
--select nvl(p_ne_id, -1) from ( 
--select lag( q_ne_id, 1) over (order by rn ) p_ne_id, q2.* 
--from (
--select rownum rn, q1.* from (
--select s_ne_id, ne_id q_ne_id, nm_cardinality, 
--   case nm_cardinality 
...
--     else ne_no_start
--   end end_node, 
--nm_begin_mp, nm_end_mp, ne_length 
--from nm_rescale_write ) q1
--connect by nocycle prior end_node = start_node
--start with q_ne_id = -1 
--) q2 )
--where q_ne_id = pi_ne_id and rownum = 1;
/*
...
--      raise_application_error( -20001, 'No starting element found for '||to_char(pi_ne_id));
  ...
  ELSE
    ...
    where not exists ( select 1 from v_nm_ordered_members b
                       where b.end_node = decode( a.nm_cardinality, 1,a. ne_no_start, a.ne_no_end ));
--    
    OPEN c2;
...
BEGIN
  ...
  --
    IF l_shape_option != 'N'
    THEN
      nm3sdm.reshape_route( pi_ne_id, nvl( l_max_date, nm3user.get_effective_date), CASE WHEN l_shape_option = 'H' THEN 'Y' ELSE 'N' END );
    END IF;
  --
  END IF;
