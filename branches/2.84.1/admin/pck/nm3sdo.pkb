...
---   PVCS Identifiers :-
--
--       sccsid           : $Header:   //new_vm_latest/archives/nm3/admin/pck/nm3sdo.pkb-arc   2.84.1.0   Jan 27 2016 18:38:20   Rob.Coupe  $
--       Module Name      : $Workfile:   nm3sdo.pkb  $
--       Date into PVCS   : $Date:   Jan 27 2016 18:38:20  $
--       Date fetched Out : $Modtime:   Jan 27 2016 18:38:12  $
--       PVCS Version     : $Revision:   2.84.1.0  $
--       Based on
------------------------------------------------------------------
...
-----------------------------------------------------------------------------
   ...
   g_body_sccsid     CONSTANT VARCHAR2(2000) := '"$Revision:   2.84.1.0  $"';
   g_package_name    CONSTANT VARCHAR2 (30)  := 'NM3SDO';
   g_batch_size      INTEGER                 := NVL( TO_NUMBER(Hig.get_sysopt('SDOBATSIZE')), 10);
   g_clip_type       VARCHAR2(30)            := NVL(Hig.get_sysopt('SDOCLIPTYP'),'SDO');
   g_only_live       VARCHAR2(1)             := NVL(hig.get_sysopt('SDOONLYLIV'), 'N');
--
...
CURSOR c_themes ( c_layer IN NUMBER ) IS
   ...
   UNION
   SELECT nth.nth_theme_id, nth.nth_feature_table, nlt_gty_type objtype, 3 dim_setting, 'G' g_or_i, NULL nth_xsp_column, nth_sequence_name
   ...
   UNION
   SELECT nth.nth_theme_id, nth.nth_feature_table, nith_nit_id objtype,
          DECODE (nit_pnt_or_cont, 'P', 2, 'C', 3) dim_setting, 'F' g_or_i, NULL nth_xsp_column, nth_sequence_name
     FROM NM_THEMES_ALL nth, NM_INV_THEMES, nm_inv_types, NM_BASE_THEMES
    WHERE nbth_base_theme = c_layer
   AND nbth_theme_id = nth_theme_id
      AND nth_update_on_edit = 'I'
...
BEGIN
  ...
  EXCEPTION
    when ex_object_exists_ddl then
      NULL;
  end;
  RETURN l_seq_name;
END;
...
--      it should return the distance (measure) along the object.
  ...
  IF l_nth.nth_feature_table = l_nth.nth_table_name THEN
     ...
     cur_string := 'select distinct t.'||l_nth.nth_pk_column||',SUBSTR(t.'||SUBSTR(l_nth.nth_label_column,1,100)||',1,100), null'||', t.'||l_nth.nth_pk_column;
...
-- AE 4100
  ...
  ELSIF p_nth.nth_feature_fk_column IS NOT NULL THEN
     ...
     cur_string := 'select distinct t.'||l_nth.nth_pk_column||',SUBSTR(t.'||SUBSTR(l_nth.nth_label_column,1,100)||',1,100), null'||', f.'||l_nth.nth_feature_pk_column;
...
sdo_lb, sdo_ub, sdo_tolerance, srid 
   ...
   AND   nnth_nlt_id = nlt_id
   AND   nlt_g_i_d = 'D' and nth_base_table_theme is NULL)
   order by dim_row   
   )
...
FUNCTION IS_FT_TYPE(p_obj_type in VARCHAR2 ) return Boolean is
retval Boolean := FALSE;
l_dummy integer := 0;
begin
  begin
    select 1 into l_dummy
    from dual where exists ( select 1 from nm_inv_types where nit_inv_type = p_obj_type and nit_table_name is not null );
--
    retval := TRUE;
  Exception
    when no_data_found 
...
BEGIN
   ...
   for irec in c_base_themes(p_type, p_obj_type) loop
      ...
      if l_gty_partial = 'N' then
   
        curstr_whole := replace(curstr_whole, '<nth_feature_table>', irec.nth_feature_table );
        curstr_whole := replace(curstr_whole, '<nth_feature_shape_column>', irec.nth_feature_shape_column );
        curstr_whole := replace(curstr_whole, '<nth_feature_pk_column>', irec.nth_feature_pk_column );
        curstr_whole := replace(curstr_whole, '<nm_obj_type>', p_obj_type );
        
        if p_only_live = 'Y' then
          curstr_whole := curstr_whole||lf||' and nm_end_date is null';
        end if;
        ...
        if p_type = 'I' then
          ...
          if is_ft_type (p_obj_type) then
          
             declare
               l_nit nm_inv_types%rowtype := nm3get.get_nit(p_obj_type);
             begin
          
                curstr_inv_ft  := replace(curstr_inv_ft, '<nth_feature_table>', irec.nth_feature_table );
                curstr_inv_ft := replace(curstr_inv_ft, '<nth_feature_shape_column>', irec.nth_feature_shape_column );
                curstr_inv_ft := replace(curstr_inv_ft, '<nth_feature_pk_column>', irec.nth_feature_pk_column );
                curstr_inv_ft := replace(curstr_inv_ft, '<nit_foreign_pk_column>', l_nit.nit_foreign_pk_column );
                curstr_inv_ft := replace(curstr_inv_ft, '<nit_lr_ne_column_name>', l_nit.nit_lr_ne_column_name );
          ...
          curstr_partial_gty  := replace(curstr_partial_gty, '<nth_feature_table>', irec.nth_feature_table );
          curstr_partial_gty := replace(curstr_partial_gty, '<nth_feature_shape_column>', irec.nth_feature_shape_column );
          curstr_partial_gty := replace(curstr_partial_gty, '<nth_feature_pk_column>', irec.nth_feature_pk_column );
          curstr_partial_gty := replace(curstr_partial_gty, '<nm_obj_type>', p_obj_type );
          curstr_partial_gty := replace(curstr_partial_gty, '<nm_type>', p_type );

          if p_only_live = 'Y' then
            curstr_partial_gty := curstr_partial_gty||lf||' and nm_end_date is null';
          end if;
