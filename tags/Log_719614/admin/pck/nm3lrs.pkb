...
CURSOR c1 (c_parent_units number
  ...
  SELECT nm_ne_id_of, nm_cardinality, ne_no_start, ne_no_end,
         nm3unit.convert_unit(c_parent_units, nm3net.get_nt_units( nm3net.get_nt_type( ne_id )), nm_slk), ne_length,
   ...
   LOOP
     ...
     IF l_tab_nm_cardinality(i) = 1 THEN
       p_lrefs( lref_index).r_offset :=
	        nm3unit.convert_unit( p_parent_units, p_datum_units, p_offset ) - l_slk + l_tab_nm_begin_mp(i) ;
     ELSE
       p_lrefs( lref_index ).r_offset :=
	        l_tab_nm_end_mp(i) - (nm3unit.convert_unit( p_parent_units, p_datum_units, p_offset) - l_slk);
     END IF;
  END LOOP;
