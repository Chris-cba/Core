CREATE OR REPLACE PACKAGE BODY nm3merge
IS
    --
    --   PVCS Identifiers :-
    --
    --       pvcsid           : $Header:   //new_vm_latest/archives/nm3/admin/pck/nm3merge.pkb-arc   2.28   Mar 26 2021 17:27:12   Rob.Coupe  $
    --       Module Name      : $Workfile:   nm3merge.pkb  $
    --       Date into PVCS   : $Date:   Mar 26 2021 17:27:12  $
    --       Date fetched Out : $Modtime:   Mar 26 2021 17:25:56  $
    ...
    --
    --
    FUNCTION is_node_conistsent (
        p_ne_id_1         IN nm_elements.ne_id%TYPE,
        p_ne_id_2         IN nm_elements.ne_id%TYPE,
        p_merge_at_node   IN nm_nodes.no_node_id%TYPE)
        RETURN VARCHAR2
    IS
        retval    VARCHAR2 (10);
        l_type1   nm_node_usages.nnu_node_type%TYPE;
        l_type2   nm_node_usages.nnu_node_type%TYPE;
    BEGIN
        BEGIN
            SELECT type1, type2
              INTO l_type1, l_type2
              FROM (SELECT rn,
                           nnu_ne_id                                     ne_id_1,
                           nnu_node_type                                 type1,
                           LEAD (nnu_ne_id, 1) OVER (ORDER BY rn)        ne_id_2,
                           LEAD (nnu_node_type, 1) OVER (ORDER BY rn)    type2
                      FROM (SELECT 1     rn,
                                   nnu_ne_id,
                                   nnu_no_node_id,
                                   nnu_node_type
                              FROM nm_node_usages
                             WHERE     nnu_no_node_id = p_merge_at_node
                                   AND nnu_ne_id = p_ne_id_1
                            UNION ALL
                            SELECT 2,
                                   nnu_ne_id,
                                   nnu_no_node_id,
    ...
    ------------------------------------------------------------------------------------------------
    --
    PROCEDURE merge_members_by_in (
        p_ne_id_of_1       nm_elements.ne_id%TYPE,
    ...
    PROCEDURE merge_other_product_stp (
    ...
    IS
        ...
        --
        CURSOR c_get_node (c_ne_id_1 NUMBER, c_ne_id_2 NUMBER)
        IS
    ...
    PROCEDURE organise_nodes (
        p_ne1          IN     nm_elements.ne_id%TYPE,
    ...
    IS
        --
        CURSOR cs_nodes IS
              SELECT nnu_ne_id, nnu_node_type, nnu_no_node_id
                FROM nm_node_usages_all
               WHERE nnu_ne_id IN (p_ne1, p_ne2)
            ORDER BY DECODE (nnu_ne_id, p_ne1, 1, 2),
                     DECODE (nnu_no_node_id, p_no_id, 2, 1);
    ...
    BEGIN
        ...
        --
        v_leg_no :=
            nm3net.get_leg_number (p_ne_id_1,
                                   nm3net.get_start_node (p_ne_id_1));
        ...
        v_leg_no :=
            nm3net.get_leg_number (p_ne_id_2,
                                   nm3net.get_end_node (p_ne_id_2));
    ...
    ------------------------------------------------------------------------------------------------
    --
    PROCEDURE audit_element_history (
        pi_ne_id_new        IN nm_elements.ne_id%TYPE,
    ...
    BEGIN
        ...
        IF HIG.IS_PRODUCT_LICENSED ('LB')
        THEN
            EXECUTE IMMEDIATE 'begin lb_nw_edit.log_transaction( :g_transaction_id, :neh_id ); end; '
                USING g_transaction_id, l_rec_neh.neh_id;
        END IF;
    ...
    IS
        ...
        CURSOR cs_inherited_columns (
            c_nt_type   nm_type_columns.ntc_nt_type%TYPE)
        IS
            SELECT ntc_column_name
              FROM nm_type_columns a
             WHERE ntc_nt_type = c_nt_type AND ntc_inherit = 'Y';

        --
        CURSOR cs_type_inclusion (
            c_nt_type   nm_type_inclusion.nti_nw_child_type%TYPE)
        IS
            SELECT *
    ...
    BEGIN
        ...
        v_merge_at_node :=
            nm3net.get_element_shared_node (p_ne_id_1, p_ne_id_2);
        ...
        THEN
            ...
            BEGIN
                ...
                LOOP
                    l_been_in_loop := TRUE;
                    l_cursor_name :=
                        SUBSTR (LOWER ('cs_' || cs_rec.nti_parent_column),
                                1,
                                30);
                    ...
                    nm3ddl.append_tab_varchar (
                        l_tab_vc,
                        '      IF nm3net.is_node_poe (pi_route_id => l_parent_ne_id');
                    nm3ddl.append_tab_varchar (
                        l_tab_vc,
                           '                            ,pi_node_id  => '
                        || g_package_name
                ...
                nm3ddl.append_tab_varchar (
                    l_tab_vc,
                    '      nm3type.g_exception_code := -20501;');
                nm3ddl.append_tab_varchar (
                    l_tab_vc,
                       '      nm3type.g_exception_msg  := '
                    || nm3flx.string ('Auto Inclusion parent not found')
                    || ';');
                nm3ddl.append_tab_varchar (
                    l_tab_vc,
                ...
                nm3ddl.append_tab_varchar (
                    l_tab_vc,
                    '      nm3type.g_exception_code := -20002;');
                nm3ddl.append_tab_varchar (
                    l_tab_vc,
                       '      nm3type.g_exception_msg  := '
                    || nm3flx.string ('There is a POE at the node')
                    || ';');
                nm3ddl.append_tab_varchar (
                    l_tab_vc,
                    '      RAISE_APPLICATION_ERROR(nm3type.g_exception_code,nm3type.g_exception_msg);');
        ...
        g_rec_ne2.ne_nt_type :=
            NVL (g_rec_ne2.ne_nt_type, g_rec_ne1.ne_nt_type);
        g_rec_ne2.ne_descr := NVL (g_rec_ne2.ne_descr, g_rec_ne1.ne_descr);
        g_rec_ne2.ne_admin_unit :=
            NVL (g_rec_ne2.ne_admin_unit, g_rec_ne1.ne_admin_unit);
        g_rec_ne2.ne_gty_group_type :=
            NVL (g_rec_ne2.ne_gty_group_type, g_rec_ne1.ne_gty_group_type);
        --
...
--            || p_ne_id_new
--            || ' node start '
--            || NVL (g_rec_ne2.ne_no_start, p_ne_no_start_new)
--            || ', node end '
--            || NVL (g_rec_ne2.ne_no_end, p_ne_no_end_new));

        nm3net.insert_element (
            p_ne_id               => p_ne_id_new,
    ...
    PROCEDURE end_date_members_internal (
        p_ne_id_1          nm_elements.ne_id%TYPE,
        p_ne_id_2          nm_elements.ne_id%TYPE,
    ...
    ------------------------------------------------------------------------------------------------
    --
    FUNCTION get_new_chainage (
        pi_rse_length_1           IN nm_elements.ne_length%TYPE,
        ...
        pi_cardinality_flipped    IN VARCHAR2 DEFAULT 'N')
        RETURN NUMBER
    IS
        l_retval              NUMBER;
    ...
    BEGIN
        IF pi_chainage_section = 1
        THEN
            IF pi_new_starting_section = 1
            THEN
                RETURN (l_original_chainage);
            ELSE
                RETURN (l_length_2 + l_original_chainage);
            END IF;
        ELSIF pi_chainage_section = 2
        THEN
            IF pi_new_starting_section = 2
            THEN
                IF pi_cardinality_flipped = 'N'
                THEN
                    RETURN (l_original_chainage);
                ELSE
                    RETURN (l_length_2 - l_original_chainage);
    ...
    BEGIN
        ...
        THEN
            l_block :=
                   'BEGIN'
                || CHR (10)
                || '   stp_network_ops.do_merge(pi_ne_id_new             => :p_ne_id_new'
                || CHR (10)
                || '                           ,pi_ne_id1                => :p_ne_id_1'
                || CHR (10)
                || '                           ,pi_ne_id2                => :p_ne_id_2'
                || CHR (10)
                || '                           ,pi_ne_length_1           => :p_ne_length_1'
                || CHR (10)
                || '                           ,pi_ne_length_2           => :p_ne_length_2'
                || CHR (10)
                || '                           ,pi_effective_date        => :p_effective_date'
                || CHR (10)
                || '                           ,pi_flip_cardinality_of_2 => :p_flip_cardinality_of_2);'
                || CHR (10)
                || 'END;';
    ...
    ------------------------------------------------------------------------------------------------
    --
    PROCEDURE merge_other_products (
        p_ne_id_1                 nm_elements.ne_id%TYPE,
        p_ne_id_2                 nm_elements.ne_id%TYPE,
        p_ne_id_new               nm_elements.ne_id%TYPE,
    ...
    BEGIN
        ...
        IF p_ne_id_1 = p_new_starting_ne_id
        THEN
        ...
        THEN
            ...
            l_block :=
                ...
                || '   maimerge.merge_data'
                || CHR (10)
                || '      (p_rse_he_id_new  => :p_ne_id_new'
                || CHR (10)
                || '      ,p_rse_he_id_1    => :p_ne_id_1'
                || CHR (10)
                || '      ,p_rse_he_id_2    => :p_ne_id_2'
                || CHR (10)
                || '      ,p_new_starting_section  => :p_new_starting_section'
                || CHR (10)
                || '      ,p_flip_cardinality_of_2    => :p_flip_cardinality_of_2'
                || CHR (10)
                || '      ,p_effective_date => :p_effective_date'
        ...
        THEN
            ...
            l_block :=
                   'BEGIN'
                || CHR (10)
                || '   prowmerge.merge_data( p_new_id     => :p_ne_id_new'
                || CHR (10)
                || '                        ,p_old_id1    => :p_ne_id_1'
                || CHR (10)
                || '                        ,p_old_id2    => :p_ne_id_2'
        ...
        THEN
            ...
            l_block :=
                   'BEGIN'
                || CHR (10)
                || '   ukpmerge.merge( p_rse            => :p_ne_id_new'
                || CHR (10)
                || '                  ,p_rse_1          => :p_ne_id_1'
                || CHR (10)
                || '                  ,p_rse_2          => :p_ne_id_2'
            ...
            EXECUTE IMMEDIATE l_block
                USING IN p_ne_id_new, p_ne_id_1, p_ne_id_2;
        END IF;

        --NSG
        IF Hig.Is_Product_Licensed (Nm3type.c_Nsg)
        THEN
            EXECUTE IMMEDIATE   'Begin'
                             || CHR (10)
                             || '  Nsg_Merge.Merge_Esu ('
                             || CHR (10)
                             || '                      p_Old_Ne_Id_1     =>  :p_Ne_Id_1,'
                             || CHR (10)
                             || '                      p_Old_Ne_Id_2     =>  :p_Ne_Id_2,'
                             || CHR (10)
                             || '                      p_New_Ne_Id       =>  :p_Ne_Id_New,'
                             || CHR (10)
                             || '                      p_Effective_Date  =>  :p_Effective_Date'
                             || CHR (10)
                             || '                      );'
    ...
    ------------------------------------------------------------------------------------------------
    --
    PROCEDURE check_other_products (p_ne_id1           IN     nm_elements.ne_id%TYPE,
    ...
    BEGIN
        ...
        THEN
            ...
            l_block :=
                ...
                || '    maimerge.check_data'
                || CHR (10)
                || '              (p_rse_he_id_1    => :p_ne_id1'
                || CHR (10)
                || '              ,p_rse_he_id_2    => :p_ne_id2'
                || CHR (10)
                || '              ,p_sect_no        => :p_sectno'
                || CHR (10)
                || '              ,p_effective_date => :p_effective_date'
    ...
    IS
        ...
        --
        l_ne_rec_1                nm_elements%ROWTYPE
                                      := nm3net.get_ne (p_ne_id_1);
        l_ne_rec_2                nm_elements%ROWTYPE
                                      := nm3net.get_ne (p_ne_id_2);
        ...
        l_node_consistency        VARCHAR2 (10);
        ...
        IS
            ...
            IS
                  SELECT a2.nnu_node_type,
                         Nm3net.get_type_sign (a1.nnu_node_type),
                         Nm3net.get_type_sign (a2.nnu_node_type),
                         SUM (1) OVER (ORDER BY 1)
        ...
        BEGIN
            ...
            FETCH c_connectivity
                INTO l_type1,
                     l1,
                     l2,
                     lc;

            IF c_connectivity%NOTFOUND
            THEN
                retval := 0;
            ELSE
                IF lc > 1
    ...
    BEGIN
        ...
        --check that if a node is supplied and there is a choice as to which node is dissolved then raise exception
        -- if the node supplied is inconsistent

        --
        l_connectivity :=
            defrag_connectivity (pi_ne_id1   => p_ne_id_1,
                                 pi_ne_id2   => p_ne_id_2);

        l_node_consistency :=
            is_node_conistsent (p_ne_id_1, p_ne_id_2, p_merge_at_node);

        IF l_connectivity != -99 AND p_merge_at_node IS NOT NULL
        THEN
            IF l_node_consistency = 'FALSE'
            THEN
                hig.raise_ner (nm3type.c_net, 561);
            ELSIF l_node_consistency = 'TRUE'
            THEN
                NULL;
            ELSIF l_node_consistency = 'INVALID'
            THEN
        ...
        THEN
            ...
            --RC - Enhance to allow a merge at one of the common nodes. Inherit the attributes relating to the first element but if the
            --     node is supplied, then use the node to determine the direction - if the node is not supplied then use the direction
            --     of the first element - therefore the node to be dissolved is the end-node of the first element. If the supplied node
            --     is not a common node then raise an error
--            nm_debug.debug ('The two elements form a loop');

            IF p_merge_at_node IS NULL
            THEN
                --              loop and no node supplied - go with the original code and dissolve the end-node of the first element
                --              i.e. maintaing direction of first element

                l_ne_no_start := l_ne_rec_1.ne_no_start;
                l_ne_no_end := l_ne_rec_1.ne_no_start;

                IF l_ne_rec_1.ne_no_end = l_ne_rec_2.ne_no_start
                THEN
                    l_flip_cardinality_of_2 := 'N';
                    g_ne_id_to_flip := NULL;
                ELSE
                    l_flip_cardinality_of_2 := 'Y';
                    g_ne_id_to_flip := p_ne_id_2;
                END IF;
                ...
                l_starting_ne_id := p_ne_id_1; -- used by merge members to determine how to determine the new overall measures
--                nm_debug.debug (
--                       'No node supplied - flip = '
--                    || l_flip_cardinality_of_2
--                    || ' - flipping '
--                    || g_ne_id_to_flip);
            ELSE
--                nm_debug.debug ('A node has been supplied ');

                IF l_node_consistency = 'TRUE'
                THEN
--                    nm_debug.debug (
--                        'Direction and node are consistent -  we can preserve direction of first element');

                    l_ne_no_start := l_ne_rec_1.ne_no_start;
                    l_ne_no_end := l_ne_rec_1.ne_no_start;

                    l_starting_ne_id := p_ne_id_1;
                    ...
                    END IF;
                ELSE
                    --the node is not the one that is naturally selected (i.e. end node of first element), test to see
                    --if the user has the ability to choose the direction and node from the first element
--                    nm_debug.debug (
--                        'Direction and node are inconsistent - check if the user can set the direction by choosing start element');

                    IF l_ne_rec_1.ne_no_end = l_ne_rec_2.ne_no_start
                    THEN
                        --the two datums are in the same direction. The choice of dissolved node can be made by the user
                        --and we need to maintain the direction of the first - so they may need to choose a different node
                        --or choose a different order of elements
                        hig.raise_ner (nm3type.c_net, 561);
                    ELSE
                        -- the two datums form a loop and the user can only dissolve the
                        -- same node irrespective of the order of the elements.
                        -- we need to use the specified node as the one to be dissolved,
                        -- maintain the direction of the first - by appending the
                        -- first to the second (after reversing the direction of the second if needed)

                        l_ne_no_start := l_ne_rec_1.ne_no_end;
                        l_ne_no_end := l_ne_rec_1.ne_no_end;
                        ...
                        l_starting_ne_id := p_ne_id_2; -- used by merge members to determine how to determine the new overall measures
--                        nm_debug.debug (
--                               'Loop and opposite directions with node - flip no 2, ne_id = '
--                            || g_ne_id_to_flip
--                            || ' this is also the start');
        ...
        --
        g_ne_1_datum_length := l_ne_rec_1.ne_length; --nm3net.get_datum_element_length (p_ne_id_1);
        g_ne_2_datum_length := l_ne_rec_2.ne_length; --nm3net.get_datum_element_length (p_ne_id_2);
        --
        ...
        -- Change made by AE
        DECLARE
            l_ne_id_1   nm_elements.ne_id%TYPE := p_ne_id_1;
            l_ne_id_2   nm_elements.ne_id%TYPE := p_ne_id_2;
        BEGIN
            IF g_starting_ne_id = p_ne_id_1
            THEN
                NULL;
            ELSIF nm3merge.g_starting_ne_id = p_ne_id_2
            THEN
                l_ne_id_1 := p_ne_id_2;
                l_ne_id_2 := p_ne_id_1;
            ELSE
                raise_application_error (
                    -20001,
                    'Problem in arguments in merge geometry');
            END IF;
        ...
        merge_members (p_ne_id_1          => p_ne_id_1,
                       ...
                       p_merge_at_node    => p_merge_at_node);
        --
        end_date_members_internal (p_ne_id_1,
                                   p_ne_id_2,
                                   p_ne_id_new,
                                   p_effective_date);
        ...
        THEN
            ...
            BEGIN
                l_block :=
                    'begin lb_nw_edit.lb_merge(:p_ne1, :p_ne2, :p_ne, :p_effective_date, :g_transaction_id); end; ';
        ...
        IF nm3nwad.ad_data_exist (p_ne_id_1)
        THEN
    ...
    EXCEPTION
        ...
        THEN
            set_for_return;
            RAISE_APPLICATION_ERROR (nm3type.g_exception_code,
                                     nm3type.g_exception_msg);
    --   WHEN OTHERS
    --    THEN
    --      set_for_return;
    ...
    ------------------------------------------------------------------------------------------------
    --
    FUNCTION elements_on_same_route (pi_ne_id_1 NUMBER, pi_ne_id_2 NUMBER)
        RETURN VARCHAR2
    IS
        --
        CURSOR cs_elements_in_same_route (c_ne_id_1 NUMBER, c_ne_id_2 NUMBER)
        IS
            SELECT 1
        ...
        --
        CURSOR element_in_route (c_ne_id NUMBER)
        IS
            SELECT 1
              FROM nm_members
             WHERE nm_ne_id_of = c_ne_id AND nm_type = 'G';
    ...
    BEGIN
        ...
        --
        IF l_tab_in_route (1) AND l_tab_in_route (2)
        THEN
    ...
    BEGIN
        ...
        INSERT INTO nm_element_history (neh_id,
                                        neh_ne_id_old,
                                        neh_ne_id_new,
                                        neh_operation,
                                        neh_effective_date,
    ...
    -----------------------------------------------------------------------------
    --
    PROCEDURE end_date_members (
        p_nm_ne_id_of_old   nm_members.nm_ne_id_of%TYPE,
    ...
    BEGIN
        ...
        --
        OPEN cs_nm (p_nm_ne_id_of_old, l_nm_type);

        FETCH cs_nm
            BULK COLLECT INTO l_tab_nmh_nm_ne_id_in,
    ...
    BEGIN
        ...
        FORALL i IN 1 .. g_tab_nmh_nm_ne_id_in.COUNT
            INSERT INTO nm_member_history (nmh_nm_ne_id_in,
                                           nmh_nm_ne_id_of_old,
                                           nmh_nm_ne_id_of_new,
                                           nmh_nm_begin_mp,
                                           nmh_nm_start_date,
              ...
              SELECT nm_ne_id_in
               ...
               WHERE     nm_ne_id_of IN (c_ne_id_of_1, c_ne_id_of_2)
                     AND NOT (    nm_type = 'G'
                              ...
                              AND nm3net.is_gty_partial (nm_obj_type) = 'N') -- KA/GJ 694396 filter out non-partial group memberships of the second original datum which we are effectively ignoring.
    ...
    IS
        ...
        --
        PROCEDURE populate_data_local (
            p_ne_id_of     nm_members.nm_ne_id_of%TYPE,
        ...
        IS
            ...
            CURSOR cs_members (c_ne_id_of nm_members.nm_ne_id_of%TYPE)
            IS
                  SELECT *
        ...
        BEGIN
            ...
            LOOP
                ...
                THEN
                    ...
                    THEN
                        l_gty_is_partial :=
                            nm3net.gty_is_partial (cs_rec.nm_obj_type);
                    ELSIF l_is_inv
                    THEN
                        IF cs_rec.nm_obj_type !=
                           NVL (g_rec_nit.nit_inv_type, nm3type.c_nvl)
                        THEN
                            g_rec_nit :=
                                nm3inv.get_inv_type (cs_rec.nm_obj_type);
                        END IF;

                        l_inv_is_excl_point :=
                            (    g_rec_nit.nit_exclusive = 'Y' -- If this inv type is exclusive
                ...
                IF l_is_group AND NOT l_gty_is_partial
                THEN
                    IF cs_rec.nm_ne_id_of != p_starting_ne_id --RAC - the order of the arguments is not important, we need to keep the data on the first element by direction
                    ...
                    ELSE
                        ...
                        l_rec_nm.nm_end_mp :=
                            p_datum_length_1 + p_datum_length_2;
                    END IF;
                ELSE
                    l_rec_nm.nm_begin_mp :=
                        l_rec_nm.nm_begin_mp + p_len_to_add;
        ...
        --
        PROCEDURE duplicate_members_local (
            l_ne_id_of    nm_members.nm_ne_id_of%TYPE,
        ...
        BEGIN
            ...
            INSERT INTO nm_merge_members
                SELECT *
                  FROM nm_members
                 WHERE nm_ne_id_in = p_ne_id_in AND nm_ne_id_of = l_ne_id_of;
    ...
    BEGIN
        ...
        THEN
            ...
            populate_data_local (p_ne_id_of_2,
                                 p_datum_length_2,
                                 p_datum_length_1);
        --
        ELSE
            --
            populate_data_local (p_ne_id_of_1,
                                 p_datum_length_1,
                                 p_datum_length_2);
        ...
        THEN
            ...
            LOOP
                ...
                --
                IF l_tab_rec_nm (i).nm_begin_mp =
                   l_tab_rec_nm (i - 1).nm_end_mp
                THEN
                    -- Tweak this record to extend the nm_end_mp
                    --
                    l_tab_rec_nm_final (l_tab_rec_nm_final.COUNT).nm_end_mp :=
                        l_tab_rec_nm (i).nm_end_mp;

                    --
                    IF l_tab_rec_nm_final (l_tab_rec_nm_final.COUNT).nm_cardinality =
                       -1
                    THEN
                        l_tab_rec_nm_final (l_tab_rec_nm_final.COUNT).nm_slk :=
                            l_tab_rec_nm (i).nm_slk;
                        l_tab_rec_nm_final (l_tab_rec_nm_final.COUNT).nm_true :=
                            l_tab_rec_nm (i).nm_true;
                    END IF;
                --
                -- Task 0108613
                -- Deal merging of partial members with differing cardinalities
                --
                ELSIF l_tab_rec_nm (i).nm_end_mp =
                      l_tab_rec_nm (i - 1).nm_begin_mp
                THEN
                    --
                    l_tab_rec_nm_final (l_tab_rec_nm_final.COUNT).nm_begin_mp :=
                        l_tab_rec_nm (i).nm_begin_mp;
                    l_tab_rec_nm_final (l_tab_rec_nm_final.COUNT).nm_end_mp :=
                        l_tab_rec_nm (i - 1).nm_end_mp;
                ...
                ELSE
                    ...
                    l_tab_rec_nm_final (l_tab_rec_nm_final.COUNT + 1) :=
                        l_tab_rec_nm (i);
                END IF;
            END LOOP;
        END IF;
        ...
        --
        IF l_is_inv AND l_inv_is_excl_point
        THEN
            ...
            l_rec_excl_check.nit_x_sect_allow_flag :=
                g_rec_nit.nit_x_sect_allow_flag;
    ...
    -----------------------------------------------------------------------------
    --
    FUNCTION is_nw_operation_in_progress
        RETURN BOOLEAN
    IS
    BEGIN
    ...
    -----------------------------------------------------------------------------
    --
    PROCEDURE set_nw_operation_in_progress (
        p_in_progress   BOOLEAN DEFAULT TRUE)
    IS
    BEGIN
    ...
    ------------------------------------------------------------------------------------------------
    --
    FUNCTION route_has_valid_SE_nodes (
    ...
    ------------------------------------------------------------------------------------------------
    --
    FUNCTION can_elements_be_merged (
        pi_ne_id_1          IN nm_elements.ne_id%TYPE,
        pi_ne_id_2          IN nm_elements.ne_id%TYPE,
    ...
    IS
        ...
        PROCEDURE set_output_params (p_ner_appl             IN nm_errors.ner_appl%TYPE,
                                     ...
                                     p_supplimentary_info   IN VARCHAR2)
        IS
        BEGIN
    ...
    BEGIN
        ...
        THEN
            ...
            BEGIN
                l_block :=
                    'BEGIN LB_NW_EDIT.CHECK_OPERATION(:p_op, :p_ne1, :p_ne2, :p_start_m, :p_shift_m, :p_effective_date, :p_length ); end; ';
        ...
        THEN
            ...
            RETURN (FALSE);
        ELSIF NVL (pi_effective_date, TRUNC (SYSDATE)) > TRUNC (SYSDATE)
        THEN
            set_output_params (nm3type.c_net, 165, NULL);
            RETURN (FALSE);
        ELSIF NOT nm3inv_security.can_usr_see_all_inv_on_element (pi_ne_id_1)
        THEN
            ...
            RETURN (FALSE);
        ELSIF NOT nm3inv_security.can_usr_see_all_inv_on_element (pi_ne_id_2)
        THEN
            set_output_params (nm3type.c_net,
                               172 -- User does not have access to all inventory on element
                                  ,
            ...
            RETURN (FALSE);
        ELSIF l_ne_rec_1.ne_id = l_ne_rec_2.ne_id
        THEN
            set_output_params (p_ner_appl             => nm3type.c_net,
            ...
            RETURN (FALSE);
        ELSIF l_ne_rec_1.ne_nt_type != l_ne_rec_2.ne_nt_type
        THEN
            set_output_params (
                p_ner_appl   => nm3type.c_net,
                p_ner_id     => 363               -- elements cannot be merged
                                   ,
                p_supplimentary_info   =>
                    'Network Type of both elements must match');
            RETURN (FALSE);
        ELSIF NVL (l_ne_rec_1.ne_gty_group_type, -1) !=
              NVL (l_ne_rec_2.ne_gty_group_type, -1)
        THEN
            set_output_params (
                p_ner_appl   => nm3type.c_net,
                p_ner_id     => 363               -- elements cannot be merged
                                   ,
                p_supplimentary_info   =>
                    'Group Type of both elements must match');
            RETURN (FALSE);
        ELSIF l_ne_rec_1.ne_admin_unit != l_ne_rec_2.ne_admin_unit
        THEN
            set_output_params (
                p_ner_appl   => nm3type.c_net,
                p_ner_id     => 363               -- elements cannot be merged
                                   ,
                p_supplimentary_info   =>
                    'Admin Unit of both elements must match');
            RETURN (FALSE);
        ELSIF     NVL (l_ne_rec_1.ne_sub_class, -1) !=
                  NVL (l_ne_rec_2.ne_sub_class, -1)
              AND nm3net.subclass_is_used (pi_ne_id_1)
        THEN
            set_output_params (
                p_ner_appl   => nm3type.c_net,
                p_ner_id     => 363               -- elements cannot be merged
                                   ,
                p_supplimentary_info   =>
                    'Subclass of both elements must match');

            RETURN (FALSE);
        ELSIF nm3net.is_nt_inclusion (l_ne_rec_1.ne_nt_type)
        THEN
            set_output_params (
                nm3type.c_net,
                363                                      -- cannot merge group
                   ,
                   CHR (10)
                || 'Auto-Inclusion detected between Network Type '
                || l_ne_rec_1.ne_nt_type
                || ' and datum network.');
            RETURN (FALSE);
        ELSIF     nm3net.element_is_a_datum (pi_ne_type => l_ne_rec_1.ne_type)
              AND nm3net.element_is_a_datum (pi_ne_type => l_ne_rec_2.ne_type)
        THEN
            IF elements_on_same_route (pi_ne_id_1, pi_ne_id_2) = 'N'
            THEN
                l_suppl := hig.get_ner (nm3type.c_net, 189).ner_descr;
                set_output_params (nm3type.c_net, 363    -- cannot merge group
                                                     , CHR (10) || l_suppl); -- elements are not in same group
                RETURN (FALSE);
            ELSE
                RETURN (TRUE);
            END IF;
        ELSIF     nm3net.element_is_a_group (pi_ne_type => l_ne_rec_1.ne_type)
              AND nm3net.element_is_a_group (pi_ne_type => l_ne_rec_2.ne_type)
        THEN
            IF    NOT route_has_valid_SE_nodes (pi_ne_id_1)
               OR NOT route_has_valid_SE_nodes (pi_ne_id_2)
            THEN
        ...
        ELSE
            set_output_params (
                ...
                p_ner_id     => 363               -- elements cannot be merged
                                   ,
                p_supplimentary_info   =>
                    'Select two Datum elements or two Group elements');
            RETURN (FALSE);
        END IF;
    ...
    ------------------------------------------------------------------------------------------------
    --
    PROCEDURE check_elements_can_be_merged (
        pi_ne_id_1          IN nm_elements.ne_id%TYPE,
        pi_ne_id_2          IN nm_elements.ne_id%TYPE,
        pi_effective_date   IN DATE DEFAULT NULL)
    IS
    ...
    FUNCTION get_route_common_node (
        pi_route_ne_id_1   IN nm_elements.ne_id%TYPE,
        pi_route_ne_id_2   IN nm_elements.ne_id%TYPE)
    ...
    IS
        ...
        IS
            SELECT nt_node_type
             ...
             WHERE ne_nt_type = nt_type AND ne_id = c_route_ne_id_1;

        CURSOR c1 IS
            SELECT ne1.ne_no_end, 'E', 'S'
              FROM nm_elements ne1, nm_elements ne2
             WHERE     ne1.ne_id = pi_route_ne_id_1
                   AND ne2.ne_id = pi_route_ne_id_2
                   AND ne1.ne_no_end = ne2.ne_no_start
            UNION
            SELECT ne1.ne_no_start, 'S', 'E'
              FROM nm_elements ne1, nm_elements ne2
             WHERE     ne1.ne_id = pi_route_ne_id_1
                   AND ne2.ne_id = pi_route_ne_id_2
                   AND ne1.ne_no_start = ne2.ne_no_end
            UNION
            SELECT ne1.ne_no_end, 'E', 'E'
              FROM nm_elements ne1, nm_elements ne2
             WHERE     ne1.ne_id = pi_route_ne_id_1
                   AND ne2.ne_id = pi_route_ne_id_2
                   AND ne1.ne_no_end = ne2.ne_no_end
            UNION
            SELECT ne1.ne_no_start, 'S', 'S'
              FROM nm_elements ne1, nm_elements ne2
             WHERE     ne1.ne_id = pi_route_ne_id_1
                   AND ne2.ne_id = pi_route_ne_id_2
                   AND ne1.ne_no_start = ne2.ne_no_start;
    ...
    BEGIN
        ...
        -----------------------------------
        l_tab_rec_common_nodes :=
            nm3merge.get_route_common_node (
                pi_route_ne_id_1   => pi_route_ne_rec_1.ne_id,
        ...
        ---------------------------------------------------------
        IF l_tab_rec_common_nodes.COUNT = 0
        THEN
        ...
        --------------------------------------------------------------------
        IF l_tab_rec_common_nodes.COUNT > 1
        THEN
        ...
        IF     l_tab_rec_common_nodes (1).on_ne_id_1 = 'E'
           AND l_tab_rec_common_nodes (1).on_ne_id_2 = 'S'
        THEN
            --                ROUTE 1                   ROUTE 2
            --        *---------------------*--------------------------*
            --        S                    E S                         E
        ...
        ELSIF     l_tab_rec_common_nodes (1).on_ne_id_1 = 'S'
              AND l_tab_rec_common_nodes (1).on_ne_id_2 = 'E'
        THEN
            --                ROUTE 1                   ROUTE 2
            --        *---------------------*--------------------------*
            --        E                    S E                         S
        ...
        ELSIF     l_tab_rec_common_nodes (1).on_ne_id_1 = 'E'
              AND l_tab_rec_common_nodes (1).on_ne_id_2 = 'E'
        THEN
            --                ROUTE 1                   ROUTE 2
            --        *---------------------*--------------------------*
        ...
        IF     po_flip_cardinality_of_2 = 'Y'
           AND nm3net.get_gty (pi_route_ne_rec_2.ne_gty_group_type).ngt_reverse_allowed =
               'N'
        THEN
    ...
    ------------------------------------------------------------------------------------------------
    --
    PROCEDURE merge_group_elements (
        pi_route_ne_rec_1      IN     nm_elements%ROWTYPE,
    ...
    BEGIN
        ...
        g_transaction_id := lb_transaction_id_seq.NEXTVAL;
        ...
        g_rec_ne2.ne_length :=
            pi_route_ne_rec_1.ne_length + pi_route_ne_rec_2.ne_length;
        g_rec_ne2.ne_admin_unit := pi_route_ne_rec_1.ne_admin_unit;
        g_rec_ne2.ne_gty_group_type := pi_route_ne_rec_1.ne_gty_group_type;
        g_rec_ne2.ne_owner :=
            NVL (pi_ne_owner_new, pi_route_ne_rec_1.ne_owner);
        g_rec_ne2.ne_name_1 :=
            NVL (pi_ne_name_1_new, pi_route_ne_rec_1.ne_name_1);
        g_rec_ne2.ne_name_2 :=
            NVL (pi_ne_name_2_new, pi_route_ne_rec_1.ne_name_2);
        g_rec_ne2.ne_prefix :=
            NVL (pi_ne_prefix_new, pi_route_ne_rec_1.ne_prefix);
        g_rec_ne2.ne_number :=
            NVL (pi_ne_number_new, pi_route_ne_rec_1.ne_number);
        g_rec_ne2.ne_sub_type :=
            NVL (pi_ne_sub_type_new, pi_route_ne_rec_1.ne_sub_type);
        g_rec_ne2.ne_group :=
            NVL (pi_ne_group_new, pi_route_ne_rec_1.ne_group);
        g_rec_ne2.ne_no_start := pi_ne_no_start;
        g_rec_ne2.ne_no_end := pi_ne_no_end;
        g_rec_ne2.ne_sub_class :=
            NVL (pi_ne_sub_class_new, pi_route_ne_rec_1.ne_sub_class);
        g_rec_ne2.ne_nsg_ref :=
            NVL (pi_ne_nsg_ref_new, pi_route_ne_rec_1.ne_nsg_ref);
        g_rec_ne2.ne_version_no :=
            NVL (pi_ne_version_no_new, pi_route_ne_rec_1.ne_version_no);


        nm3net.insert_element (
            p_ne_id               => g_rec_ne2.ne_id,
            p_ne_unique           => g_rec_ne2.ne_unique,
            p_ne_length           => g_new_element_length,
    ...
    ------------------------------------------------------------------------------------------------
    --
    PROCEDURE merge_group_memberships (
        pi_route_ne_id_1           IN nm_elements.ne_id%TYPE,
        pi_route_ne_id_2           IN nm_elements.ne_id%TYPE,
    ...
    IS
        CURSOR c_in_memb IS
            ...
            SELECT 2,
                   ...
                   nm_end_mp,
                   nm_slk,
                   DECODE (pi_flip_cardinality_of_2,
                           'Y', nm_cardinality * -1,
        ...
        TYPE memb_rec IS RECORD
        (
            nm_route_1_or_2    VARCHAR2 (1),
        ...
        TYPE tab_memb_rec IS TABLE OF memb_rec
        ...
        PROCEDURE end_date_existing_IN_members
        IS
        BEGIN
            UPDATE nm_members
        ...
        PROCEDURE end_date_existing_OF_members
        IS
        BEGIN
            UPDATE nm_members
               SET nm_end_date = pi_effective_date
    ...
    BEGIN
        ...
        FOR i IN 1 .. l_tab_in_memb_rec.COUNT
        LOOP
        ...
        FOR i IN 1 .. l_tab_of_memb_rec.COUNT
        LOOP
            INSERT INTO nm_members (nm_ne_id_in,
                                    nm_ne_id_of,
                                    nm_type,
                ...
                SELECT l_tab_of_memb_rec (i).nm_ne_id_in,
                  ...
                  FROM DUAL
                 WHERE    (    nm3net.get_group_exclusive_flag (
                                   l_tab_of_memb_rec (i).nm_ne_id_in) =
                               'Y'
                           AND NOT EXISTS
                                   (SELECT 'x'
                                      FROM nm_members
                                     WHERE     nm_obj_type =
                                           ...
                                           AND nm_ne_id_of =
                                               pi_route_new_ne_id))
                       OR (    nm3net.get_group_exclusive_flag (
                                   l_tab_of_memb_rec (i).nm_ne_id_in) =
                               'N'
                           AND NOT EXISTS
                                   (SELECT 'x'
    ...
    IS
        ...
        l_ne_rec_1                nm_elements%ROWTYPE
                                      := nm3net.get_ne (pi_ne_id => pi_route_ne_id_1);
        l_ne_rec_2                nm_elements%ROWTYPE
                                      := nm3net.get_ne (pi_ne_id => pi_route_ne_id_2);
    ...
    BEGIN
        ...
        merge_group_memberships (
            pi_route_ne_id_1           => pi_route_ne_id_1,
        ...
        IF nm3nwad.ad_data_exist (pi_route_ne_id_1)
        THEN
    ...
    ------------------------------------------------------------------------------------------------
    --
    PROCEDURE do_merge_datum_or_group (
        pi_ne_id_1             IN     nm_elements.ne_id%TYPE,
        pi_ne_id_2             IN     nm_elements.ne_id%TYPE,
        pi_effective_date      IN     DATE DEFAULT TO_DATE (
                                                       SYS_CONTEXT (
    ...
    BEGIN
        ...
        --
        IF nm3net.element_is_a_datum (pi_ne_type => l_ne_rec_1.ne_type)
        THEN
