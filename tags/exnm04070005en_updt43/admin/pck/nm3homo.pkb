...
AS
   ...
   --   PVCS Identifiers :-
   --
   --       pvcsid           : $Header:   //new_vm_latest/archives/nm3/admin/pck/nm3homo.pkb-arc   2.23.1.2   Jul 19 2016 09:50:08   Rob.Coupe  $
   --       Module Name      : $Workfile:   nm3homo.pkb  $
   --       Date into PVCS   : $Date:   Jul 19 2016 09:50:08  $
   --       Date fetched Out : $Modtime:   Jul 19 2016 09:43:06  $
   --       PVCS Version     : $Revision:   2.23.1.2  $
   --
   --
   ...
   -----------------------------------------------------------------------------
   --
   TYPE t_ne_arr IS TABLE OF nm_elements%ROWTYPE
      INDEX BY PLS_INTEGER;

   TYPE t_date_arr IS TABLE OF PLS_INTEGER
   ...
   TYPE t_chunk_arr IS TABLE OF t_chunk_rec
      INDEX BY PLS_INTEGER;

   -- Log 713421
   TYPE l_child_ne IS RECORD
   (
   ...
   TYPE l_child_ne_table IS TABLE OF l_child_ne
   ...
   g_body_sccsid        CONSTANT VARCHAR2 (2000) := '"$Revision:   2.23.1.2  $"';
   ...
   c_hist_loc_enabled   CONSTANT BOOLEAN
                                    := NVL (
                                          hig.get_sysopt (
                                             p_option_id   => c_hist_enabled_opt),
                                          'N') = 'Y' ;
   ...
   g_homo_exc_msg                VARCHAR2 (2000)
      := 'Unspecified error within ' || g_package_name;
   g_homo_exception              EXCEPTION;
   --
   -- <Used in Exclusivity Checking>
   g_tab_vc                      nm3type.tab_varchar32767;
   g_tab_vc_inv_type             NM_INV_TYPES.nit_inv_type%TYPE
                                    := SUBSTR (nm3type.c_nvl, 1, 4);
   -- </Used in Exclusivity Checking>
   --
   ...
   -----------------------------------------------------------------------------
   --
   FUNCTION has_children (pi_iit_ne_id IN NUMBER)
      RETURN BOOLEAN;

   --
   -----------------------------------------------------------------------------
   --
   FUNCTION has_parent (pi_iit_ne_id IN NUMBER)
      RETURN BOOLEAN;
   ...
   -----------------------------------------------------------------------------
   --
   PROCEDURE deal_with_hierarchical (p_ne_id NUMBER, p_effective_date DATE);

   --
   -----------------------------------------------------------------------------
   --
   PROCEDURE check_item_has_no_future_locs (
      p_iit_ne_id         nm_inv_items.iit_ne_id%TYPE,
      p_effective_date    DATE DEFAULT TO_DATE (
                                          SYS_CONTEXT ('NM3CORE',
                                                       'EFFECTIVE_DATE'),
                                          'DD-MON-YYYY'));
   ...
   IS
      ...
      IS
         SELECT DISTINCT nm.*
           FROM NM_MEMBERS nm, NM_NW_TEMP_EXTENTS nte
      ...
      --
      l_inv_orig_location                    NM_PLACEMENT_ARRAY
                                                := nm3pla.initialise_placement_array;
      l_new_inv_location                     NM_PLACEMENT_ARRAY
                                                := nm3pla.initialise_placement_array;
      --
      ...
      --
      c_nte_id_for_unchanged_locs   CONSTANT NM_NW_TEMP_EXTENTS.nte_job_id%TYPE
         := nm3net.get_next_nte_id ;
      c_nte_id                      CONSTANT NM_NW_TEMP_EXTENTS.nte_job_id%TYPE
         := nm3net.get_next_nte_id ;
      l_remnant_job_id                       NM_NW_TEMP_EXTENTS.nte_job_id%TYPE;
      --
      l_impacted_inv                         int_array_type
         := int_array_type (p_iit_ne_id);
      --
      l_valid_xsp                            BOOLEAN;
   ...
   BEGIN
      ...
      --
      INSERT INTO NM_NW_TEMP_EXTENTS (nte_job_id,
                                      nte_ne_id_of,
                                      nte_begin_mp,
                                      nte_end_mp,
                                      nte_cardinality,
      ...
      -- #############################################################################
      --
      check_temp_ne_for_pnt_or_cont (
         pi_nte_job_id    => c_nte_id,
      ...
      THEN
         ...
         EXCEPTION
            ...
            THEN
               ...
               g_homo_exc_msg :=
                  'Inventory types not marked as "multiple allowed" must only have a single location';
               RAISE g_homo_exception;
         END;
      END IF;
      ...
      BEGIN
         ...
         THEN
            hig.raise_ner (
               ...
               pi_id                   => 67,
               pi_supplementary_info   =>    'NM_ELEMENTS : Effective Date : '
                                          || TO_CHAR (
                                                p_effective_date,
                                                SYS_CONTEXT ('NM3CORE',
                                                             'USER_DATE_MASK')));
         ...
         IF (   does_relation_exist (l_rec_nit.nit_inv_type,
             ...
             OR does_relation_exist (l_rec_nit.nit_inv_type,
         ...
         THEN
            ...
            BEGIN
               ...
               --
               FOR i
                  IN (SELECT DISTINCT nm_ne_id_in
                        FROM nm_members, nm_inv_items
                       WHERE     nm_obj_type = l_parent_rec.iit_inv_type
                             AND nm_ne_id_in = iit_ne_id
                             AND DECODE (l_nit_rec.NIT_X_SECT_ALLOW_FLAG,
                                         'Y', NVL (iit_x_sect, '-'),
                                         '-') =
                                    DECODE (
                                       l_nit_rec.NIT_X_SECT_ALLOW_FLAG,
                                       'Y', NVL (l_iit_rec.iit_x_sect, '-'),
                                       '-')
                             AND nm_ne_id_of IN (SELECT x.pl_ne_id
                                                   FROM TABLE (
                                                           l_new_inv_pl_arr.npa_placement_array) x))
               LOOP
                  DECLARE
                     --
                     l_ne_id   NUMBER;
                        ...
                        BEGIN
                           ...
                           THEN
                              ...
                              l_iig_rec.iig_top_id :=
                                 nm3invval.get_iig_top_id (i.nm_ne_id_in);
               ...
               THEN
                  ...
                  ELSE
                     ...
                     g_homo_exc_msg :=
                        'Cannot locate Child Asset outside the bounds of Parent Location';
                     RAISE g_homo_exception;
                  END IF;
               END IF;
            END;
         ...
         THEN
            ...
            g_homo_exc_msg :=
               'Continuous Inventory Items must not be placed at a single point';
            RAISE g_homo_exception;
         ...
         THEN
            ...
            g_homo_exc_msg :=
                  'XSP not allowed for Inventory type "'
               || l_rec_iit.iit_inv_type
               || '"';
            RAISE g_homo_exception;
         ...
         THEN
            ...
            THEN
               ...
               g_homo_exc_msg :=
                     'XSP must be specified for Inventory type "'
                  || l_rec_iit.iit_inv_type
                  || '"';
               RAISE g_homo_exception;
            ELSE
               ...
               --
               IF NOT valid_xsp_on_pl (
                         pi_pl         => l_new_inv_pl_arr,
               ...
               THEN
                  ...
                  g_homo_exc_msg :=
                        'XSP "'
                     || l_rec_iit.iit_x_sect
                     || '" is not allowed for Inventory type "'
         ...
         BEGIN
            ...
            LOOP
               IF is_affected_by_exclusivity (
                     p_iit_ne_id   => l_future_ne_id_in (i),
                     p_rec_iit     => l_rec_iit,
               ...
               THEN
                  ...
                  g_homo_exc_msg :=
                     'There are future inventory locations which would be affected';
                  RAISE g_homo_exception;
               END IF;
            END LOOP;
         END;
      ...
      DECLARE
         --
         CURSOR cs_nte_ele (c_nte_job_id NM_NW_TEMP_EXTENTS.nte_job_id%TYPE)
         IS
         ...
         --
         CURSOR cs_nte_nm (c_nte_job_id NM_NW_TEMP_EXTENTS.nte_job_id%TYPE)
         IS
                SELECT /*+ INDEX (nm nm_ne_id_of_ind) */
                       nm.ROWID nm_rowid
                  FROM nm_members nm,
                       (  SELECT nte.nte_ne_id_of -- get the distinct list of NE_IDs
                            FROM nm_nw_temp_extents nte
                           WHERE nte_job_id = c_nte_job_id
                        GROUP BY nte.nte_ne_id_of)
                 WHERE nte_ne_id_of = nm.nm_ne_id_of AND nm.nm_type = 'G'
      ...
      BEGIN
         ...
         FETCH cs_nte_ele BULK COLLECT INTO l_tab_ne_rowid;
         ...
         ELSE
            ...
            FETCH cs_nte_nm BULK COLLECT INTO l_tab_nm_rowid;
      ...
      THEN
         ...
         THEN
            FOR cs_rec
               IN cs_affected_inv (l_remnant_job_id,
                                   l_rec_iit.iit_inv_type,
                                   p_iit_ne_id)
            LOOP -- This cursor returns affected membership records, independent of XSP
               ...
               DECLARE
                  ...
                  l_placement_arr          NM_PLACEMENT_ARRAY
                     := nm3pla.initialise_placement_array;
                  l_member_arr             NM_PLACEMENT_ARRAY
                     := nm3pla.initialise_placement_array;
               ...
               BEGIN
                  --
                  IF is_affected_by_exclusivity (
                        p_iit_ne_id   => cs_rec.nm_ne_id_in,
                        p_rec_iit     => l_rec_iit,
                        p_exclusive   => (l_rec_nit.nit_exclusive = 'Y'),
                        p_x_sect      => (l_rec_nit.nit_x_sect_allow_flag =
                                             'Y'))
                  THEN
                     NULL;
                  ELSE
                  ...
                  deal_with_hierarchical (
                     p_ne_id            => cs_rec.nm_ne_id_in,
                     p_effective_date   => p_effective_date);
                  --
                  --
                  -- Record this iit_ne_id as having had it's location changed
                  l_tab_iit_ne_id (l_tab_iit_ne_id.COUNT + 1) :=
                     cs_rec.nm_ne_id_in;

                  --
                  -- Get all of the MPs for this NE_ID_OF
                  FOR cs_mp
                     IN (SELECT nte_begin_mp mp
                           FROM NM_NW_TEMP_EXTENTS
                         ...
                         SELECT nte_end_mp mp
                          ...
                          WHERE     nte_job_id = l_remnant_job_id
                                AND nte_ne_id_of = cs_rec.nm_ne_id_of
                         UNION
                         SELECT cs_rec.nm_begin_mp mp FROM DUAL
                         UNION
                         SELECT cs_rec.nm_end_mp mp FROM DUAL)
                  LOOP
                  ...
                  FOR cs_placed
                     IN (  SELECT nte_begin_mp, nte_end_mp
                             FROM NM_NW_TEMP_EXTENTS
                            WHERE     nte_job_id = l_remnant_job_id
                                  AND nte_ne_id_of = cs_rec.nm_ne_id_of
                  ...
                  LOOP
                     ...
                     IF l_placement_arr.npa_placement_array (l_pl_pos).pl_start =
                           cs_placed.nte_begin_mp
                     THEN
                     ...
                     LOOP
                        EXIT WHEN l_placement_arr.npa_placement_array (
                                     l_pl_pos).pl_start >=
                                     cs_placed.nte_begin_mp;
                        l_pl_pos := l_pl_pos + 1;
                     END LOOP;
                     ...
                     LOOP
                        IF l_placement_arr.npa_placement_array (l_pl_pos).pl_end <=
                              cs_placed.nte_end_mp
                        THEN
                           l_tab_dont_do (l_pl_pos) := l_pl_pos;
         ...
         ELSE                                          -- This is a POINT item
            FOR cs_rec
               IN cs_affected_point (l_remnant_job_id,
                                     l_rec_iit.iit_inv_type,
                                     p_iit_ne_id)
            LOOP
               --
         ...
         FORALL l_count IN 1 .. l_tab_iit_ne_id.COUNT
            UPDATE nm_inv_items
             ...
             WHERE     iit_ne_id = l_tab_iit_ne_id (l_count)
                   AND NOT EXISTS
                          (SELECT 1          -- item has no location currently
                             FROM nm_members
                            WHERE nm_ne_id_in = iit_ne_id)
                   AND NOT EXISTS
                          (SELECT 1 -- item has no pre-existing future location
                             FROM nm_members_all
                            WHERE     nm_ne_id_in = iit_ne_id
      ...
      LOOP
         ...
         DECLARE
            l_found_par   BOOLEAN;
            l_nit_rec     nm_inv_types%ROWTYPE
               := nm3get.get_nit (l_child_ne_rec.child_obj_type);
         BEGIN
            l_found_par := FALSE;
            ...
            FOR i
               IN (SELECT DISTINCT nm_ne_id_in
                    ...
                    WHERE     nm_obj_type = l_child_ne_rec.parent_obj_type
                          AND nm_ne_id_in = iit_ne_id
                          AND (nm_ne_id_of,
                               DECODE (l_nit_rec.NIT_X_SECT_ALLOW_FLAG,
                                       'Y', NVL (iit_x_sect, '-'),
                                       '-')) IN (SELECT nm_ne_id_of,
                                                        DECODE (
                                                           l_nit_rec.NIT_X_SECT_ALLOW_FLAG,
                                                           'Y', NVL (
                                                                   iit_x_sect,
            ...
            THEN
               IF     l_child_ne_rec.child_location = 'N'
                  ...
                  AND l_child_ne_rec.itg_relation =
                         nm3invval.c_derived_relation
               THEN
                  UPDATE nm_inv_item_groupings
                     SET iig_end_date = p_effective_date
      ...
      THEN
         ...
         DECLARE
            --
            CURSOR cs_elements (p_job_id NUMBER)
            IS
               SELECT ne_id, ne_length, ne_unique
                 FROM NM_ELEMENTS
            ...
            LOOP
               ...
               THEN
                  IF    l_placement_arr.npa_placement_array.COUNT > 1
                     ...
                     OR l_placement_arr.npa_placement_array (1).pl_end !=
                           cs_rec.ne_length
                  THEN
      ...
      IF l_inv_orig_location.placement_count > 0 AND c_xattr_status
      THEN
         DECLARE
            l_pl_end_dated_portion   NM_PLACEMENT_ARRAY
                                        := nm3pla.initialise_placement_array;
         BEGIN
            l_pl_end_dated_portion :=
               nm3pla.subtract_pl_from_pl (
            ...
            THEN
               ...
               nm3inv_xattr.x_item_check_pl (
                  pi_obj_type      => l_rec_iit.iit_inv_type,
      ...
      BEGIN
         ...
         LOOP
            ...
            IF NM_INV_SDO_AGGR.IS_TYPE_AGGREGATED (l_nit_type)
            THEN
               NM_INV_SDO_AGGR.AGGREGATE_INV_GEOMETRY (l_nit_type,
                                                       l_tab_iit_ne_id (i));
            END IF;
         END LOOP;
      END;
   ...
   -----------------------------------------------------------------------------
   --
   FUNCTION create_duplicate_nte (
      pi_nte_id   IN nm_nw_temp_extents.nte_job_id%TYPE)
   ...
   -----------------------------------------------------------------------------
   --
   FUNCTION move_edited_datums_to_new_nte (
      pi_nte_id   IN nm_nw_temp_extents.nte_job_id%TYPE)
      RETURN nm_nw_temp_extents.nte_job_id%TYPE
   IS
      c_nte_id_for_edited_datums   CONSTANT NM_NW_TEMP_EXTENTS.nte_job_id%TYPE
         := nm3net.get_next_nte_id ;
   BEGIN
      nm_debug.proc_start (
      ...
      UPDATE nm_nw_temp_extents nte
         SET nte.nte_job_id = c_nte_id_for_edited_datums
       WHERE     nte.nte_job_id = pi_nte_id
             AND EXISTS
                    (SELECT 1
                       FROM nm_element_history neh
                      WHERE     nte.nte_ne_id_of = neh.neh_ne_id_old
   ...
   -----------------------------------------------------------------------------
   --
   PROCEDURE lock_datums_membs_for_nte (
   ...
   -----------------------------------------------------------------------------
   --
   PROCEDURE end_date_existing_locations (
   ...
   BEGIN
      ...
      THEN
         ...
         THEN
            FOR cs_rec
               IN cs_affected_inv (pi_nte_job_id,
            ...
            LOOP -- This cursor returns affected membership records, independent of XSP
               ...
               BEGIN
                  ...
                  IF is_affected_by_exclusivity (
                        ...
                        p_x_sect      => (pi_nit_rec.nit_x_sect_allow_flag =
                                             'Y'))
                  THEN
                     NULL;
                  ELSE
                  ...
                  --
                  -- Record this iit_ne_id as having had it's location changed
                  pio_iit_ne_id_arr (pio_iit_ne_id_arr.COUNT + 1) :=
                     cs_rec.nm_ne_id_in;
         ...
         ELSE                                          -- This is a POINT item
            FOR cs_rec
               IN cs_affected_point (pi_nte_job_id,
                                     pi_iit_rec.iit_inv_type,
                                     pi_iit_rec.iit_ne_id)
            LOOP
               --
   ...
   -----------------------------------------------------------------------------
   --
   FUNCTION get_unprocessed_chunks (
   ...
   IS
      l_pl_arr   nm_placement_array
         := nm3pla.initialise_placement_array (pi_ne_id   => pi_ne_id,
   ...
   BEGIN
      ...
      LOOP
         ...
         THEN
            ...
            IF (    pi_begin_mp BETWEEN pi_chunk_arr (i).begin_mp
                                    AND pi_chunk_arr (i).end_mp
                AND pi_end_mp BETWEEN pi_chunk_arr (i).begin_mp
                                  AND pi_chunk_arr (i).end_mp)
            THEN
            ...
            l_pl_arr :=
               nm3pla.remove_pl_from_pl_arr (pi_pl_arr   => l_pl_arr,
                                             pi_pl       => l_pl);
   ...
   IS
      l_pl_arr               nm_placement_array
         := nm3pla.initialise_placement_array (pi_ne_id   => pi_pl.pl_ne_id,
      ...
      l_pl                   nm_placement
         := nm3pla.initialise_placement (pi_ne_id => pi_pl.pl_ne_id);

      l_unprocessed_pl_arr   nm_placement_array
                                := nm3pla.initialise_placement_array;
   ...
   BEGIN
      ...
      THEN
           ...
           SELECT nm.nm_ne_id_in,
                  ...
                  nm.nm_begin_mp,
                  nm.nm_end_mp
             BULK COLLECT INTO l_ne_id_in_arr,
                  l_start_date_arr,
                  l_begin_mp_arr,
                  l_end_mp_arr
             FROM nm_members_all nm
            WHERE     nm.nm_ne_id_in <> pi_iit_rec.iit_ne_id
                  AND nm.nm_ne_id_of = pi_pl.pl_ne_id
                  AND nm.nm_type = 'I'
                  AND nm.nm_obj_type = pi_nit_rec.nit_inv_type
                  AND pi_pl.pl_start < nm.nm_end_mp
                  AND pi_pl.pl_end > nm.nm_begin_mp
                  AND (   nm.nm_end_date IS NULL
                       OR (    nm.nm_end_date > pi_nm_rec.nm_start_date
                           AND nm.nm_start_date <> nm.nm_end_date))
         ORDER BY nm.nm_start_date;

         db (
               ' got '
            || l_ne_id_in_arr.COUNT
            || ' potential exclusive conflicts');
         ...
         LOOP
            ...
            THEN
               ...
               db (
                     '  start date= '
                  || TO_CHAR (l_nm_rec.nm_start_date, 'dd/mon/yyyy')
                  || ' end date= '
                  || TO_CHAR (l_nm_rec.nm_end_date, 'dd/mon/yyyy'));

               IF l_nm_rec.nm_start_date < l_nm_rec.nm_end_date
               THEN
                  --get all the bits that haven't already been processed
                  l_unprocessed_pl_arr :=
                     get_unprocessed_chunks (
                        pi_ne_id       => l_nm_rec.nm_ne_id_of,
                  ...
                  nm3pla.dump_placement_array (
                     p_pl_arr   => l_unprocessed_pl_arr);
               ...
               db (
                  '--' || l_nm_rec.nm_begin_mp || ' to' || l_nm_rec.nm_end_mp);
   ...
   BEGIN
      ...
      SELECT *
        ...
        FROM nm_nw_temp_extents
       WHERE     nte_job_id = pi_nte_job_id
             AND (pi_ne_id_of IS NULL OR nte_ne_id_of = pi_ne_id_of);
      ...
      LOOP
         ...
         BEGIN
            ...
            l_nm_seq_no :=
               NVL (
                  get_seq_no_from_orig (l_nte_arr (i).nte_ne_id_of,
                                        l_nte_arr (i).nte_begin_mp,
                                        l_nte_arr (i).nte_end_mp),
            ...
            THEN
               ...
               l_pl_arr :=
                  cr8_mems_for_excl_affected (pi_pl        => l_pl,
                                              pi_iit_rec   => pi_iit_rec,
                                              pi_nit_rec   => pi_nit_rec,
                                              pi_nm_rec    => l_rec_nm);

               IF pi_maintain_nte AND l_pl_arr.placement_count <> 1
               THEN
                  l_nte_to_maintain := pi_nte_job_id;
                  ...
                  DELETE nm_nw_temp_extents nte
                   WHERE     nte.nte_job_id = l_nte_to_maintain
                         AND nte.nte_ne_id_of = l_nte_arr (i).nte_ne_id_of
                         AND nte.nte_begin_mp = l_nte_arr (i).nte_begin_mp
                         AND nte.nte_end_mp = l_nte_arr (i).nte_end_mp;
   ...
   -----------------------------------------------------------------------------
   --
   PROCEDURE close_no_longer_located (
               ...
               THEN
                  ...
                  THEN
                     ...
                     pio_warning_msg :=
                        ...
                        || cs_rec.ne_unique
                        || ' ('
                        || TO_CHAR (pi_effective_date, 'DD-MON-YYYY')
                        || ')';
                  END IF;
               END IF;
            --
   ...
   -----------------------------------------------------------------------------
   --
   FUNCTION get_locs_on_element (
      pi_ne_id        IN nm_elements.ne_id%TYPE,
      pi_nte_job_id   IN nm_nw_temp_extents.nte_job_id%TYPE)
      RETURN nm3extent.tab_nte
   IS
   ...
   BEGIN
      ...
      SELECT nte.*
        ...
        FROM nm_nw_temp_extents nte
       WHERE nte.nte_job_id = pi_nte_job_id AND nte.nte_ne_id_of = pi_ne_id;
   ...
   BEGIN
      nm_debug.proc_start (p_package_name     => g_package_name,
        ...
        SELECT ne.*
          BULK COLLECT INTO l_retval
          FROM nm_element_history neh, nm_elements_all ne
         WHERE     neh.neh_ne_id_old = pi_old_ne_id
               AND neh.neh_operation = pi_neh_operation
               AND neh.neh_ne_id_new = ne.ne_id
      ORDER BY neh.neh_id;
   ...
   -----------------------------------------------------------------------------
   --
   PROCEDURE apply_non_closing_edits (
      pio_pl                    IN OUT nm_placement,
   ...
   BEGIN
      ...
      LOOP
         db (
               'applying operation '
            || pi_non_closing_hist_arr (i).neh_operation);

         CASE pi_non_closing_hist_arr (i).neh_operation
            WHEN nm3net_history.c_neh_op_shift
            THEN
               pio_pl.pl_start :=
                  nm3recal.get_mp_translation_shift (
                     pi_mp               => pio_pl.pl_start,
            ...
            THEN
               pio_pl.pl_start :=
                  nm3recal.get_mp_translation_recal (
                     pi_mp                    => pio_pl.pl_start,
               ...
               ELSE
                  pio_pl.pl_end :=
                     nm3recal.get_mp_translation_recal (
                        pi_mp                    => pio_pl.pl_end,
                        pi_orig_element_length   => pi_non_closing_hist_arr (
                                                      i).neh_old_ne_length,
                        pi_begin_mp              => pi_non_closing_hist_arr (
                                                      i).neh_param_1,
                        pi_new_length_to_end     => pi_non_closing_hist_arr (
                                                      i).neh_param_2);
               END IF;
            ELSE
               hig.raise_ner (
                  pi_appl                 => nm3type.c_hig,
                  pi_id                   => 110,
                  pi_supplementary_info   =>    'apply_non_closing_edits: non-closing neh operation "'
                                             || pi_non_closing_hist_arr (i).neh_operation
                                             || '"');
         END CASE;
   ...
   -----------------------------------------------------------------------------
   --
   PROCEDURE coalesce_members_chunks (
      pi_iit_ne_id        IN nm_inv_items.iit_ne_id%TYPE,
      pi_effective_date   IN DATE)
   IS
      ...
      BEGIN
         BEGIN
                SELECT nm.ROWID, nm.nm_end_mp
                  INTO po_rowid, po_new_end_mp
                  FROM nm_members_all nm
                 WHERE     nm.nm_ne_id_in = pi_iit_ne_id
                       AND nm.nm_ne_id_of = pi_nm_ne_id_of
                       AND nm.nm_start_date = pi_start_date
                       AND NVL (nm.nm_end_date, nm3type.c_nvl_date) =
                              NVL (pi_end_date, nm3type.c_nvl_date)
                       AND nm.nm_begin_mp = pi_end_mp
            FOR UPDATE NOWAIT;
         EXCEPTION
            WHEN NO_DATA_FOUND
            THEN
   ...
   BEGIN
      ...
      SELECT nm1.nm_ne_id_of,
             ...
             nm1.nm_start_date,
             nm1.nm_end_date
        BULK COLLECT INTO l_ne_id_of_arr,
             l_begin_mp_arr,
             l_end_mp_arr,
             l_start_date_arr,
             l_end_date_arr
        FROM nm_members_all nm1
       WHERE     nm1.nm_ne_id_in = pi_iit_ne_id
             AND nm1.nm_start_date >= pi_effective_date
             AND NOT EXISTS
                    (SELECT 1
                       FROM nm_members_all nm2
                      WHERE     nm2.nm_ne_id_in = pi_iit_ne_id
                            AND nm2.nm_start_date >= pi_effective_date
                            AND nm2.nm_ne_id_of = nm1.nm_ne_id_of
                            AND nm2.nm_end_mp = nm1.nm_begin_mp);
      ...
      LOOP
         ...
         LOOP
            ...
            l_nm_to_delete_rowid_arr (l_nm_to_delete_rowid_arr.COUNT + 1) :=
               l_rowid;
         END LOOP;
         ...
         THEN
            ...
            UPDATE nm_members_all nm
               SET nm.nm_end_mp = l_new_end_mp
             WHERE     nm.nm_ne_id_in = pi_iit_ne_id
                   AND nm.nm_ne_id_of = l_ne_id_of_arr (i)
                   AND nm.nm_begin_mp = l_begin_mp_arr (i)
                   AND nm.nm_start_date = l_start_date_arr (i);
         END IF;
      END LOOP;
      ...
      db (
            'deleting '
         || l_nm_to_delete_rowid_arr.COUNT
         || ' redundant members');

      FORALL i IN 1 .. l_nm_to_delete_rowid_arr.COUNT
         DELETE nm_members_all nm
          WHERE nm.ROWID = l_nm_to_delete_rowid_arr (i);
   ...
   IS
      ...
      c_nte_id                      CONSTANT NM_NW_TEMP_EXTENTS.nte_job_id%TYPE
         := create_duplicate_nte (pi_nte_id => p_temp_ne_id_in) ;
      c_nte_id_for_unchanged_locs   CONSTANT NM_NW_TEMP_EXTENTS.nte_job_id%TYPE
         := nm3net.get_next_nte_id ;
   ...
   BEGIN
               ...
               BEGIN
                  ...
                  LOOP
                     ...
                     THEN -- Only check for NE_IDs we haven't already checked - no point wasting time
                        ...
                        IF    l_local_rec_ne.ne_nt_type !=
                                 l_last_rec_ne.ne_nt_type
                           OR NVL (l_local_rec_ne.ne_sub_class,
                                   nm3type.c_nvl) !=
                                 NVL (l_last_rec_ne.ne_sub_class,
                                      nm3type.c_nvl)
                           ...
                           EXCEPTION
                              ...
                              THEN
                                 ...
                                 g_homo_exc_msg :=
                                    ...
                                    || l_rec_iit.iit_inv_type
                                    || '" on NW_TYPE "'
                                    || l_local_rec_ne.ne_nt_type
                                    || '"'
                                    || ', SUB_CLASS "'
                                    || l_local_rec_ne.ne_sub_class
                                    || '"';
                                 RAISE g_homo_exception;
                           END;
                        END IF;
      ...
      nm_debug.debug_sql_string (
            'select * from nm_nw_temp_extents where nte_job_id = '
         || l_remnant_job_id);
      ...
      nm3net_history.get_edited_datums_in_nte (
         pi_nte_id                 => l_remnant_job_id,
      ...
      l_nte_for_eff_date_loc :=
         create_duplicate_nte (pi_nte_id => l_remnant_job_id);

      IF l_edited_ne_id_arr.COUNT > 0
      THEN
         IF NOT c_hist_loc_enabled
         THEN
            hig.raise_ner (pi_appl => 'NET', pi_id => 452);
         END IF;

         --move edited datums into new temp ne
         db ('move edited datums into new temp ne');
         l_nte_for_edited_datums :=
            move_edited_datums_to_new_nte (pi_nte_id => l_remnant_job_id);
         db ('edited nte');
         nm_debug.debug_sql_string (
               'select * from nm_nw_temp_extents where nte_job_id = '
            || l_nte_for_edited_datums);
      ...
      end_date_existing_locations (
         pi_iit_ne_id                   => p_iit_ne_id,
      ...
      close_no_longer_located (
         pi_end_loc_only     => l_rec_nit.nit_end_loc_only,
      ...
      THEN
         ...
         LOOP
            ...
            db (
               ...
               || l_element_arr_idx
               || ' ne_id='
               || l_edited_ne_id_arr (l_element_arr_idx)
               || ' operation='
               || l_edited_operation_arr (l_element_arr_idx)
               || ' date='
               || TO_CHAR (l_edited_date_arr (l_element_arr_idx),
                           'DD-MON-YYYY'));

            IF l_edited_operation_arr (l_element_arr_idx) <>
                  nm3net_history.c_neh_op_close
            THEN
               -------------------------
               ...
               l_closing_hist_arr :=
                  nm3net_history.get_neh_for_closing_op (
               ...
               THEN
                  hig.raise_ner (
                     pi_appl                 => 'HIG',
                     pi_id                   => 67,
                     pi_supplementary_info   =>    'NM_ELEMENT_HISTORY closing op for ne_id '
                                                || TO_CHAR (
                                                      l_edited_ne_id_arr (
                                                         l_element_arr_idx)));
               END IF;
               ...
               nm3user.set_effective_date (
                  l_edited_date_arr (l_element_arr_idx));
               ...
               l_new_ne_1_rec :=
                  nm3get.get_ne (
                     pi_ne_id   => l_closing_hist_arr (1).neh_ne_id_new);
               --get non-closing edits to apply on new element
               db ('get non-closing edits to apply on new element');
               l_ne_1_non_closing_hist_arr :=
                  nm3net_history.get_neh_for_non_closing_ops (
                     pi_ne_id   => l_closing_hist_arr (1).neh_ne_id_new);
               --get closing op if they exist
               l_ne_1_closing_hist_arr :=
                  nm3net_history.get_neh_for_closing_op (
                     pi_ne_id   => l_closing_hist_arr (1).neh_ne_id_new);
               ...
               THEN
                  ...
                  db (
                        'add to end of element array - '
                     || l_closing_hist_arr (1).neh_ne_id_new);
                  l_edited_ne_id_arr (l_edited_ne_id_arr.COUNT + 1) :=
                     l_ne_1_closing_hist_arr (1).neh_ne_id_old;
                  l_edited_operation_arr (l_edited_ne_id_arr.COUNT) :=
                     l_ne_1_closing_hist_arr (1).neh_operation;
                  l_edited_date_arr (l_edited_ne_id_arr.COUNT) :=
                     l_ne_1_closing_hist_arr (1).neh_effective_date;
               END IF;

               IF l_closing_hist_arr (1).neh_operation =
                     nm3net_history.c_neh_op_split
               THEN
                  db ('split so get second new element');
                  l_new_ne_2_rec :=
                     nm3get.get_ne (
                        pi_ne_id   => l_closing_hist_arr (2).neh_ne_id_new);
                  --get non-closing edits to apply for 2nd element
                  l_ne_2_non_closing_hist_arr :=
                     nm3net_history.get_neh_for_non_closing_ops (
                        pi_ne_id   => l_closing_hist_arr (2).neh_ne_id_new);

                  --get closing op if they exist
                  l_ne_2_closing_hist_arr :=
                     nm3net_history.get_neh_for_closing_op (
                        pi_ne_id   => l_closing_hist_arr (2).neh_ne_id_new);
                  ...
                  THEN
                     ...
                     db (
                           'add to end of element array - '
                        || l_closing_hist_arr (2).neh_ne_id_new);
                     l_edited_ne_id_arr (l_edited_ne_id_arr.COUNT + 1) :=
                        l_ne_2_closing_hist_arr (2).neh_ne_id_old;
                     l_edited_operation_arr (l_edited_ne_id_arr.COUNT) :=
                        l_ne_2_closing_hist_arr (2).neh_operation;
                     l_edited_date_arr (l_edited_ne_id_arr.COUNT) :=
                        l_ne_2_closing_hist_arr (2).neh_effective_date;
                  END IF;
               END IF;
               ...
               LOOP
                  db (
                        'processing chunk - '
                     || l_locs_on_element_arr (i).nte_ne_id_of
                     || ':'
                  ...
                  CASE
                     WHEN l_edited_operation_arr (l_element_arr_idx) =
                             nm3net_history.c_neh_op_split
                     THEN
                        process_split (
                           pi_begin_mp          => l_locs_on_element_arr (i).nte_begin_mp,
                     ...
                     THEN
                        l_new_pl_1 :=
                           nm3pla.initialise_placement (
                              pi_ne_id   => l_closing_hist_arr (1).neh_ne_id_new,
                              pi_start   =>   l_closing_hist_arr (1).neh_old_ne_length
                                            - l_locs_on_element_arr (i).nte_end_mp,
                              pi_end     =>   l_closing_hist_arr (1).neh_old_ne_length
                                            - l_locs_on_element_arr (i).nte_begin_mp);
                     ...
                     ELSE
                        hig.raise_ner (
                           pi_appl                 => nm3type.c_hig,
                           pi_id                   => 110,
                           pi_supplementary_info   =>    'nm3homo: closing neh operation "'
                                                      || l_edited_operation_arr (
                                                            l_element_arr_idx)
                                                      || '"');
                  END CASE;
                  ...
                  l_pl_arr :=
                     cr8_mems_for_excl_affected (pi_pl        => l_new_pl_1,
                                                 pi_iit_rec   => l_rec_iit,
                                                 pi_nit_rec   => l_rec_nit,
                                                 pi_nm_rec    => l_nm_rec);
                  ...
                  THEN
                     ...
                     l_pl_arr :=
                        cr8_mems_for_excl_affected (pi_pl        => l_new_pl_2,
                                                    pi_iit_rec   => l_rec_iit,
                                                    pi_nit_rec   => l_rec_nit,
                                                    pi_nm_rec    => l_nm_rec);
   ...
   PROCEDURE homo_update (
      ...
      p_warning_msg         OUT VARCHAR2)
   IS
      c_use_orig_proc   CONSTANT BOOLEAN
                                    := NVL (
                                          hig.get_sysopt (
   ...
   IS
      --
      CURSOR cs_iig (c_parent_id NUMBER)
      IS
         SELECT 1
           FROM DUAL
          WHERE EXISTS
   ...
   IS
      --
      CURSOR cs_iig (c_child_id NUMBER)
      IS
         SELECT 1
           FROM DUAL
          WHERE EXISTS
   ...
   -----------------------------------------------------------------------------
   --
   FUNCTION get_contiguous_warning_const
      RETURN VARCHAR2
   IS
   BEGIN
      --
      ...
      THEN
         FOR cs_rec
            IN (WITH items
                ...
                FOR UPDATE OF iit.iit_ne_id NOWAIT) -- Get details of all the inv which is located lower in the hierarchy
         LOOP
            --
            DECLARE
            ...
            BEGIN
               ...
               ELSIF cs_rec.itg_relation NOT IN (nm3invval.c_at_relation,
                                                 nm3invval.c_in_relation,
                                                 nm3invval.c_derived_relation)
               THEN
                  g_homo_exc_code := -20511;
                  g_homo_exc_msg :=
                     'There are child inventory records which would be affected';
                  RAISE g_homo_exception;
               END IF;

               -- LOG 713421
               IF     cs_rec.itg_relation IN (nm3invval.c_in_relation,
                                              nm3invval.c_derived_relation)
                     ...
                     THEN
                        ...
                        BEGIN
                           ...
                           SELECT ne_id
                             ...
                             FROM (SELECT x.pl_ne_id ne_id
                                     FROM TABLE (
                                             nm3pla.get_ne_intersection (
                                                p_ne_id,
                                                cs_rec.iit_ne_id).npa_placement_array) x)
                            WHERE ne_id IS NOT NULL;
                        EXCEPTION
                           WHEN OTHERS
                           THEN
                              ...
                              l_child_ne_rec.child_location :=
                                 NVL (cs_rec.nin_loc_mandatory, 'N');
                              l_child_ne_rec.child_obj_type :=
                                 cs_rec.iit_inv_type;
                              l_child_ne_rec.parent_obj_type :=
                                 cs_rec.parent_obj;
                              l_child_ne_rec.rel_mandatory :=
                                 NVL (cs_rec.itg_mandatory, 'N');
                              l_child_ne_rec.itg_relation :=
                                 cs_rec.itg_relation;
                              l_child_ne_tab (l_child_ne_tab.COUNT + 1) :=
                                 l_child_ne_rec;
                        END;
   ...
   -----------------------------------------------------------------------------
   --
   PROCEDURE end_inv_location (
      pi_iit_ne_id              IN     NM_INV_ITEMS.iit_ne_id%TYPE,
      pi_effective_date         IN     DATE DEFAULT TO_DATE (
                                                       SYS_CONTEXT (
                                                          'NM3CORE',
                                                          'EFFECTIVE_DATE'),
                                                       'DD-MON-YYYY'),
   ...
   BEGIN
      ...
      EXCEPTION
         WHEN future_dated
         THEN
            hig.raise_ner (pi_appl => nm3type.c_net, pi_id => 178);
      END;
   ...
   -----------------------------------------------------------------------------
   --
   PROCEDURE end_inv_location_by_temp_ne (
      pi_iit_ne_id              IN     NM_INV_ITEMS.iit_ne_id%TYPE,
      pi_nte_job_id             IN     NM_NW_TEMP_EXTENTS.nte_job_id%TYPE,
      pi_effective_date         IN     DATE DEFAULT TO_DATE (
                                                       SYS_CONTEXT (
   ...
   IS
      ...
      IS
         SELECT /*+ INDEX (nm nm_pk) */
                1
           FROM NM_MEMBERS_ALL nm, NM_NW_TEMP_EXTENTS nte
          WHERE     nte.nte_job_id = p_nte_job_id
                AND nm.nm_ne_id_in = p_iit_ne_id
                AND nm.nm_ne_id_of = nte.nte_ne_id_of
      ...
      --
      CURSOR cs_future_affected_children (
         p_iit_ne_id    NM_INV_ITEMS.iit_ne_id%TYPE,
         p_eff_date     DATE)
      IS
         SELECT 1
      ...
      CURSOR cs_children (
         p_iit_ne_id    NM_INV_ITEMS.iit_ne_id%TYPE)
      IS
         SELECT iig.iig_item_id child_id, iit_inv_type, nin.nin_loc_mandatory
           FROM NM_INV_ITEM_GROUPINGS iig, NM_INV_ITEMS, nm_inv_nw nin
      ...
      l_dummy                             PLS_INTEGER;
      --
      l_pl_current                        NM_PLACEMENT_ARRAY
         := nm3pla.initialise_placement_array;
      l_pl_new                            NM_PLACEMENT_ARRAY
         := nm3pla.initialise_placement_array;
      l_pl_to_remove                      NM_PLACEMENT_ARRAY
         := nm3pla.initialise_placement_array;
      --
   ...
   BEGIN
      ...
      THEN
         g_homo_exc_code := -20502;
         g_homo_exc_msg :=
            'No NM_INV_ITEMS record found iit_ne_id= ' || pi_iit_ne_id;
         RAISE g_homo_exception;
      END IF;
      ...
      BEGIN
         l_nit_rec :=
            nm3inv.get_inv_type (pi_inv_type => l_iit_rec.iit_inv_type);
      EXCEPTION
         WHEN e_no_nit_found
         THEN
            g_homo_exc_code := -20509;
            g_homo_exc_msg :=
                  'No NM_INV_TYPES record found for Inventory type "'
               || l_iit_rec.iit_inv_type
               || '"';
            RAISE g_homo_exception;
      END;
      ...
      IF     pi_check_for_parent
         AND (   does_relation_exist (p_inv_type   => l_iit_rec.iit_inv_type,
              ...
              OR does_relation_exist (p_inv_type   => l_iit_rec.iit_inv_type,
                                      p_relation   => nm3invval.c_in_relation))
         AND has_parent (pi_iit_ne_id => l_iit_rec.iit_ne_id)
      THEN
         g_homo_exc_code := -20531;
         g_homo_exc_msg :=
            'Cannot remove inventory locations which are in a Child AT or IN relationship';
         RAISE g_homo_exception;
      END IF;
      ...
      ELSE
         ...
         nm3extent_o.create_temp_ne_from_pl (l_pl_new,
                                             l_new_location_temp_ne);
      ...
      THEN
         ...
         LOOP
            ...
            BEGIN
               ...
               IF     l_rec_itg.itg_relation IN (nm3invval.c_derived_relation,
                                                 nm3invval.c_in_relation)
   ...
   -----------------------------------------------------------------------------
   --
   FUNCTION future_affected_inv_exists (
      p_nte_job_id        NM_NW_TEMP_EXTENTS.nte_job_id%TYPE,
   ...
   BEGIN
      RETURN get_future_affected_inv (p_nte_job_id,
                                      p_inv_type,
                                      p_effective_date).COUNT > 0;
   END future_affected_inv_exists;

   --
   -----------------------------------------------------------------------------
   --
   FUNCTION get_future_affected_inv (
      p_nte_job_id        NM_NW_TEMP_EXTENTS.nte_job_id%TYPE,
      p_inv_type          NM_MEMBERS.nm_obj_type%TYPE,
      p_effective_date    DATE DEFAULT TO_DATE (
                                          SYS_CONTEXT ('NM3CORE',
   ...
   BEGIN
      ...
      THEN
         ...
         FETCH cs_future_affected_inv_cont BULK COLLECT INTO l_tab_ne_id_in;
      ...
      ELSE
         ...
         FETCH cs_future_affected_inv_point BULK COLLECT INTO l_tab_ne_id_in;
   ...
   -----------------------------------------------------------------------------
   --
   FUNCTION is_affected_by_exclusivity (
      p_iit_ne_id    NM_INV_ITEMS.iit_ne_id%TYPE,
   ...
   BEGIN
      ...
      BEGIN
         ...
         THEN
            ...
            append (
               '   WHERE  iit_ne_id  = ' || g_package_name || '.g_iit_ne_id');
            ...
            THEN
               append (
                     '     AND   iit_x_sect = '
                  || g_package_name
                  || '.g_rec_iit.iit_x_sect');
            END IF;

            --
            l_tab_ita :=
               nm3inv.get_tab_ita_exclusive (
                  p_inv_type   => p_rec_iit.iit_inv_type);
   ...
   BEGIN
      ...
      THEN
         ...
         IF l_found
         THEN
            hig.raise_ner (pi_appl => nm3type.c_net, pi_id => 86);
         END IF;
      ELSE
         ...
         IF l_found
         THEN
            hig.raise_ner (pi_appl => nm3type.c_net, pi_id => 105);
         END IF;
      END IF;
   ...
   IS
      --
      CURSOR cs_future_locs (
         c_nm_ne_id_in       nm_members_all.nm_ne_id_in%TYPE,
   ...
   BEGIN
      ...
      IF l_found
      THEN
         hig.raise_ner (pi_appl => nm3type.c_net, pi_id => 355);
      END IF;
   ...
   -----------------------------------------------------------------------------
   --
   PROCEDURE historic_locate_validation (
      pi_nte_job_id     IN nm_nw_temp_extents.nte_job_id%TYPE,
   ...
   BEGIN
      ...
      IF     c_hist_loc_enabled
         AND nm3net_history.nte_has_non_blocking_edits (
                pi_nte_job_id   => pi_nte_job_id)
      THEN
         IF nm3net_history.nte_has_blocking_edits (
               pi_nte_job_id   => pi_nte_job_id)
         THEN
         ...
         IF     pi_user_ne_id IS NOT NULL
            ...
            AND nm3net_history.group_has_blocking_edits (
                   pi_ne_id   => pi_user_ne_id)
         THEN
            RAISE e_locate_blocked;
         END IF;
      END IF;
   ...
   EXCEPTION
      ...
      THEN
         hig.raise_ner (pi_appl => nm3type.c_net, pi_id => 451);
   ...
   -----------------------------------------------------------------------------
   --
   PROCEDURE remove_element_from_nte (
   ...
   BEGIN
      ...
      DELETE nm_nw_temp_extents nte
       WHERE nte.nte_job_id = pi_job_id AND nte.nte_ne_id_of = pi_ne_id;
   ...
   -----------------------------------------------------------------------------
   --
   FUNCTION translate_location_in_time (
      pi_job_id        IN nm_nw_temp_extents.nte_job_id%TYPE,
   ...
   IS
      ...
      l_target_job_id               nm_nw_temp_extents.nte_job_id%TYPE
                                       := nm3seq.next_nte_id_seq;
      ...
      THEN
         ...
         LOOP
            ...
            THEN
               ...
               ---------------------------------
               remove_element_from_nte (
                  pi_job_id   => l_target_job_id,
                  pi_ne_id    => l_edited_ne_id_arr (l_element_arr_idx));
            END IF;
      ...
      SELECT 1
       ...
       WHERE EXISTS
                (SELECT t.pl_ne_id
                   FROM TABLE (
                           CAST (
                              pi_pl.npa_placement_array AS nm_placement_array_type)) t
                  WHERE NOT EXISTS
                           (SELECT 1
                              FROM nm_elements, nm_members, xsp_restraints
