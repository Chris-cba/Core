...
----------------------------------------------------------------------------------------------
--
PROCEDURE create_gis_theme_for_inv_type(p_inv_type    IN nm_inv_types.nit_inv_type%TYPE
                                       ,pi_theme_name IN gis_themes.gt_theme_name%TYPE DEFAULT NULL
                                       ,pi_where      IN gis_themes.gt_where%TYPE      DEFAULT NULL
                                       ) IS
--
   CURSOR cs_roles (c_inv_type nm_inv_types.nit_inv_type%TYPE) IS
   SELECT *
    FROM  nm_inv_type_roles
   WHERE  itr_inv_type = c_inv_type
   AND EXISTS (SELECT 'x'
                FROM  hig_roles
               WHERE  hro_role = itr_hro_role
              );
--
   l_rec_nit  nm_inv_types%ROWTYPE;
   l_rec_gt   nm_themes_all%ROWTYPE;
   l_rec_gtf  nm_theme_functions_all%ROWTYPE;
   l_rec_gthr gis_theme_roles%ROWTYPE;
--
BEGIN
--
   nm_debug.proc_start(g_package_name,'create_gis_theme_for_inv_type');
--
   l_rec_nit              := nm3inv.get_inv_type(p_inv_type);
   l_rec_gt.nth_theme_name := NVL(pi_theme_name, UPPER(SUBSTR(p_inv_type||' - '||l_rec_nit.nit_descr,1,30)));
--
   l_rec_gt.nth_theme_id   := higgis.get_theme_id(l_rec_gt.nth_theme_name);
--
   IF NVL(l_rec_gt.nth_theme_id,-1) != -1
    THEN
   --
--      nm_debug.debug('This theme already exists, don'||CHR(39)||'t bother');
      UPDATE nm_themes_all
       SET   nth_feature_table = c_application_owner||'.'||nm3inv_sde.get_inv_sde_view_name(p_inv_type),
             nth_feature_pk_column = 'IIT_NE_ID',
             nth_feature_fk_column = 'NE_ID_OF'
      WHERE  nth_theme_id      = l_rec_gt.nth_theme_id
       AND   nth_feature_table IS NULL;
   --
   ELSE
      --
      -- Create the GIS_THEMES_ALL record
      --
      l_rec_gt.nth_theme_id            := higgis.next_theme_id;
      l_rec_gt.nth_table_name          := c_application_owner||'.'||derive_nw_inv_type_view_name(p_inv_type);
      l_rec_gt.nth_where               := pi_where;
      l_rec_gt.nth_pk_column           := 'IIT_NE_ID';
      l_rec_gt.nth_label_column        := 'IIT_PRIMARY_KEY';
      l_rec_gt.nth_rse_table_name      := c_application_owner||'.'||'NM_ELEMENTS';
      l_rec_gt.nth_rse_fk_column       := 'NE_ID_OF';
      l_rec_gt.nth_st_chain_column     := 'NM_BEGIN_MP';
      IF l_rec_nit.nit_pnt_or_cont = 'C'
       THEN
         l_rec_gt.nth_end_chain_column := 'NM_END_MP';
      ELSE
         l_rec_gt.nth_end_chain_column := NULL;
      END IF;
      l_rec_gt.nth_x_column            := NULL;
      l_rec_gt.nth_y_column            := NULL;
      l_rec_gt.nth_offset_field        := NULL;
      l_rec_gt.nth_feature_table       := c_application_owner||'.'||nm3inv_sde.get_inv_sde_view_name(p_inv_type);

-- GJ 01/10/2003 for log 694854
--      l_rec_gt.nth_feature_pk_column   := NULL;
--      l_rec_gt.nth_feature_fk_column   := NULL;
      l_rec_gt.nth_feature_pk_column   := 'IIT_NE_ID';
      l_rec_gt.nth_feature_fk_column   := 'NE_ID_OF';

--      l_rec_gt.nth_route_theme         := 'N';
      IF l_rec_nit.nit_x_sect_allow_flag = 'Y'
       THEN
         l_rec_gt.nth_xsp_column       := 'IIT_X_SECT';
      ELSE
         l_rec_gt.nth_xsp_column       := NULL;
      END IF;
      higgis.ins_gt(l_rec_gt);
      --
      -- Create the GIS_THEME_FUNCTIONS record
      --
      l_rec_gtf.ntf_nth_theme_id := l_rec_gt.nth_theme_id;
      l_rec_gtf.ntf_hmo_module  := 'NM0510';
      l_rec_gtf.ntf_parameter   := 'GIS_SESSION_ID';
      l_rec_gtf.ntf_menu_option := SUBSTR(NVL(pi_theme_name, p_inv_type||' - '||l_rec_nit.nit_descr),1,70);
      higgis.ins_gtf (l_rec_gtf);
      --
      -- Create the GIS_THEME_ROLES records
      --
      l_rec_gthr.gthr_theme_id  := l_rec_gt.nth_theme_id;
      FOR cs_rec IN cs_roles (p_inv_type)
       LOOP
         l_rec_gthr.gthr_role   := cs_rec.itr_hro_role;
         l_rec_gthr.gthr_mode   := cs_rec.itr_mode;
         higgis.ins_gthr (l_rec_gthr);
      END LOOP;
--
   END IF;
--
   nm_debug.proc_end(g_package_name,'create_gis_theme_for_inv_type');
--
END create_gis_theme_for_inv_type;
--
-----------------------------------------------------------------------------
--
