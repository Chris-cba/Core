...
--   PVCS Identifiers :-
--
--       PVCS id          : $Header:   //vm_latest/archives/nm3/admin/pck/nm3flx.pkb-arc   2.13.1.0   Apr 02 2012 14:04:26   Steve.Cooper  $
--       Module Name      : $Workfile:   nm3flx.pkb  $
--       Date into PVCS   : $Date:   Apr 02 2012 14:04:26  $
--       Date fetched Out : $Modtime:   Apr 02 2012 13:46:18  $
--       Version          : $Revision:   2.13.1.0  $
...
Begin
  ...
  Elsif l_Password_Length = 0 Then
    --This doesn't return false due to 4400 and earlier versions of hig1832 always doing this check even if the password was not changed,
    --If we returned false here then those forms would start raising password errors when the users tried to change anything else on the form.
    --from 4500 hig1832 is fixed to only change passwords when the user initiated a change on the front end.       
    Nm_Debug.Debug('Password was Null');
    po_Reason:=  'Password was Null';
    --should really return false..see above.
    --l_Ret_Val:=  False;
  Else
    l_Ret_Val:=  False;
