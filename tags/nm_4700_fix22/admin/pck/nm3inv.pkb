...
--   PVCS Identifiers :-
--
--       sccsid           : $Header:   //new_vm_latest/archives/nm3/admin/pck/nm3inv.pkb-arc   2.31.1.0   Jun 26 2015 11:21:06   Vikas.Mhetre  $
--       Module Name      : $Workfile:   nm3inv.pkb  $
--       Date into SCCS   : $Date:   Jun 26 2015 11:21:06  $
--       Date fetched Out : $Modtime:   Jun 26 2015 12:21:24  $
--       SCCS Version     : $Revision:   2.31.1.0  $
--       Based on --
--
--   nm3inv package body
--
...
--  g_body_sccsid is the SCCS ID for the package body
   g_body_sccsid        CONSTANT varchar2(2000) := '$Revision:   2.31.1.0  $';
...
FUNCTION get_inv_domain_meaning(pi_domain IN NM_INV_ATTRI_LOOKUP_ALL.ial_domain%TYPE
                               ...
                               ,pi_iit_start_date IN NM_INV_ITEMS_ALL.iit_start_date%TYPE
                               ) RETURN NM_INV_ATTRI_LOOKUP_ALL.ial_meaning%TYPE IS

  CURSOR c1(p_domain IN NM_INV_ATTRI_LOOKUP_ALL.ial_domain%TYPE
           ,p_value  IN NM_INV_ATTRI_LOOKUP_ALL.ial_value%TYPE
           ,p_iit_start_date IN NM_INV_ITEMS_ALL.iit_start_date%TYPE) IS
    SELECT
      ial.ial_meaning
    FROM
    ...
    AND 
      ...
      OR
      (ial.ial_end_date IS NOT NULL AND
      pi_iit_start_date BETWEEN ial.ial_start_date AND ial.ial_end_date)
      )
    AND
...
BEGIN
  OPEN c1(p_domain => pi_domain
         ,p_value  => pi_value
         ,p_iit_start_date => pi_iit_start_date);
    FETCH c1 INTO l_retval;
    IF c1%NOTFOUND
    THEN
      CLOSE c1;
   ...
   --
         IF cs_rec.ita_id_domain IS NOT NULL
          THEN
            ...
            append ('  l_lup := '||g_package_name||'.get_inv_domain_meaning('||Nm3flx.string(cs_rec.ita_id_domain)||','||g_package_name||'.g_rec_iit.'||cs_rec.ita_attrib_name||','||g_package_name||'.g_rec_iit.iit_start_date'||');');           
            IF NOT l_mandatory
             THEN
               append (' END IF;');
            END IF;
