...
--   PVCS Identifiers :-
--
--       PVCS id          : $Header:   //new_vm_latest/archives/nm3/admin/views/nm_inv_type_groupings.vw-arc   2.3   Apr 13 2018 11:47:16   Gaurav.Gaurkar  $
...
BEGIN
--
   append ('CREATE OR replace force view nm_inv_type_groupings AS',FALSE);
   append ('SELECT');
   append ('--');
   append ('--   SCCS Identifiers :-');
   append ('--');
   append ('--       sccsid           : @(#)nm_inv_type_groupings.vw	1.7 03/24/05');
   append ('--       Module Name      : nm_inv_type_groupings.vw');
   append ('--       Date into SCCS   : 05/03/24 16:22:41');
   ...
   append ('--');
   append ('       *');
   append (' FROM  nm_inv_type_groupings_all');
   append ('WHERE  itg_start_date <= To_Date(Sys_Context(''NM3CORE'',''EFFECTIVE_DATE''),''DD-MON-YYYY'')');
   append (' AND   NVL(itg_end_date,TO_DATE('||CHR(39)||'99991231'||CHR(39)||','||CHR(39)||'YYYYMMDD'||CHR(39)||')) >  To_Date(Sys_Context(''NM3CORE'',''EFFECTIVE_DATE''),''DD-MON-YYYY'')');
--
...
--
   IF NOT l_enterprise_edn
    ...
    THEN
      ...
      --
      append (' AND chk_inv_type_valid_for_role(itg_inv_type)!='||CHR(39)||'FALSE'||CHR(39));
      EXECUTE IMMEDIATE l_view;
   END IF;
--
END;
