CREATE OR REPLACE PACKAGE BODY higole AS
--   SCCS Identifiers :-
--
...
--
--
--   Author : Ian SmithCREATE OR REPLACE PACKAGE BODY higole AS
	...
	FUNCTION strget_template_sql (strtemplate IN VARCHAR2, strpkvalue IN VARCHAR2) RETURN VARCHAR2 IS
	...
	BEGIN
		...
		IF strtempcolumns IS NULL THEN
			RAISE column_error;
		END IF;
		...
		IF strpkcolumn IS NULL THEN
			RAISE pk_error;
		END IF;

		strreturn := strbuildsql||strtempcolumns||' FROM '||strtablename||' WHERE '||strpkcolumn||'='''||strpkvalue||'''';

		RETURN strreturn;

	EXCEPTION
		WHEN column_error THEN
			RAISE_APPLICATION_ERROR(-20001,'Unable to find columns for template <<<'||strtemplate||'>>>');
		WHEN synonym_error THEN
			RAISE_APPLICATION_ERROR(-20001,'Unable to find synonym for template <<<'||strtemplate||'>>>');
		WHEN pk_error THEN
			RAISE_APPLICATION_ERROR(-20001,'Unable to find PK Column for template <<<'||strtemplate||'>>>');

	END strget_template_sql;

-----------------

	PROCEDURE get_template_sql (strtemplate IN VARCHAR2, strpkvalue IN VARCHAR2,sql_out OUT VARCHAR2) IS
...
--
--------
	FUNCTION strget_sccs_id RETURN VARCHAR2 IS
	BEGIN
		RETURN c_sccs_id;
	END;
--------
...
--------
	...
	FUNCTION get_default_report_type RETURN VARCHAR2 IS
		CURSOR c1 IS
			SELECT hop_value
			FROM hig_options
			WHERE hop_product = 'HIG'
			AND hop_id = 'DEFREPTYPE';
...
--------
	...
	FUNCTION strget_template_pk_column (strtemplatename IN VARCHAR2) RETURN VARCHAR2 IS
...
--
--------
	FUNCTION strget_template_where (strtemplatename IN VARCHAR2) RETURN VARCHAR2 IS
		strreturn VARCHAR2(32767);
...
--
--------
	FUNCTION strget_template_columns (strtemplate IN VARCHAR2) RETURN VARCHAR2 IS

		strreturn VARCHAR2(32767);
		strbuildwhere VARCHAR2(32767);

		CURSOR c1 IS
			SELECT * FROM DOC_TEMPLATE_COLUMNS
	...
	BEGIN
		FOR recs IN c1 LOOP
			...
			IF UPPER(recs.dtc_col_type) = c_strvarchar THEN
				strbuildwhere := strbuildwhere ||recs.dtc_col_name||' '||recs.dtc_col_alias||',';
			ELSIF UPPER(recs.dtc_col_type) = c_strdate THEN
				strbuildwhere := strbuildwhere ||'TO_CHAR('||recs.dtc_col_name||','''||c_strdateformat||''') '
									 ||recs.dtc_col_alias||',';
			ELSIF UPPER(recs.dtc_col_type) = c_strnumeric THEN
				strbuildwhere := strbuildwhere ||'TO_CHAR('||recs.dtc_col_name||') '||recs.dtc_col_alias||',';
			ELSE
		...
		IF strbuildwhere IS NOT NULL THEN
...
--
--------
	FUNCTION strget_procedure_columns (strtemplate IN VARCHAR2,strpkvalue IN VARCHAR2) RETURN VARCHAR2 IS
	...
	BEGIN
		...
		IF strcolumns IS NOT NULL THEN
			---Strip off the last comma
		...
		strsql := 'SELECT '||strcolumns||' FROM '||strtablename||' WHERE '||strpkcolumn||'='''||strpkvalue||'''';
		...
		FOR recs IN c1 LOOP
			IF UPPER(recs.dtc_col_type) = c_strvarchar THEN
				dbms_sql.define_column(source_cursor,intcol,varchar_col,255);
			ELSIF UPPER(recs.dtc_col_type) = c_strdate THEN
		...
		IF dbms_sql.fetch_rows(source_cursor) > 0 THEN
			...
			FOR recs IN c1 LOOP
				...
				ELSIF UPPER(recs.dtc_col_type) = c_strdate THEN
					...
					strcolvalues := strcolvalues ||TO_CHAR(date_col,c_strdateformat)||',';
				...
				intcol := intcol +1;
			END LOOP;
			IF strcolvalues IS NOT NULL THEN
				---Rip off the last comma
				strcolvalues := SUBSTR(strcolvalues,1,LENGTH(strcolvalues)-1);
			END IF;
		END IF;
	...
	EXCEPTION
		WHEN no_columns_found OR synonym_error OR pk_error THEN
			---RETURN 'No Columns found for template <<<'||strTemplate||'>>>';
		...
		WHEN OTHERS THEN
			IF dbms_sql.is_open(source_cursor) THEN
				dbms_sql.close_cursor(source_cursor);
			END IF;
			RAISE_APPLICATION_ERROR(-20000,SQLERRM||' At line'||TO_CHAR(intlineno));
			RETURN NULL;
...
--------
	...
	FUNCTION strget_template_from (strtemplate IN VARCHAR2) RETURN VARCHAR2 IS
...
--
--------
	FUNCTION strget_template_synonym (strtemplate IN VARCHAR2) RETURN VARCHAR2 IS
		strreturn VARCHAR2(2000);
...
--
--------
	FUNCTION strget_template_procedure (strtemplate IN VARCHAR2) RETURN VARCHAR2 IS
		strreturn VARCHAR2(2000);
...
--------
	...
	FUNCTION strget_template_path (strtemplate IN VARCHAR2) RETURN VARCHAR2 IS
	...
	FUNCTION strget_template_path (strtemplate IN VARCHAR2, strui IN VARCHAR2) RETURN VARCHAR2 IS
	...
	BEGIN
		OPEN c1;
		FETCH c1 INTO strreturn, strreturn2;
		CLOSE c1;
...
--
--------
	FUNCTION bolhas_templates(strtable IN VARCHAR2) RETURN BOOLEAN IS
		intcount INTEGER;
...
--------
	...
	FUNCTION intget_template_media (strtemplate IN VARCHAR2) RETURN NUMBER IS
		intreturn NUMBER;
		CURSOR c1 IS
			SELECT dtg_dmd_id
			FROM DOC_TEMPLATE_GATEWAYS
...
--
--------
	FUNCTION intget_template_location (strtemplate IN VARCHAR2) RETURN NUMBER IS
		intreturn NUMBER;
		CURSOR c1 IS
			SELECT dtg_dlc_id
			FROM DOC_TEMPLATE_GATEWAYS
...
--
--------
	PROCEDURE copytemplate(strfromtemplate IN VARCHAR2, strtotemplate IN VARCHAR2) IS
		CURSOR c1 IS
			SELECT 'EXISTS'
			FROM DOC_TEMPLATE_GATEWAYS
	...
	EXCEPTION
		WHEN template_error THEN
			CLOSE c1;
			RAISE_APPLICATION_ERROR(-20000,'Template ['||strtotemplate||'] Already Exists !');
	END;
...
--------
	...
	FUNCTION strget_function_type(strfunction IN VARCHAR2) RETURN VARCHAR2 IS
	...
	BEGIN
		...
		ELSIF inttype IN (2,3) THEN
			strreturn := c_strnumeric;
		ELSIF inttype = 12 THEN
			strreturn := c_strdate;
		END IF;
...
--
--------
	FUNCTION bolparsesql(strsql IN VARCHAR2, strerror OUT VARCHAR2) RETURN BOOLEAN IS
...
--
--------
	FUNCTION bolparsesql(strsql IN VARCHAR2, strerror OUT VARCHAR2, interrorpos OUT INTEGER) RETURN BOOLEAN IS
		source_cursor INTEGER := dbms_sql.open_cursor;
	BEGIN
		---Now build the dynamic sql to return to values of each column
		dbms_sql.parse(source_cursor,strsql,dbms_sql.v7);
...
--------
	...
	PROCEDURE initialise_template_data (strtemplate IN VARCHAR2, strpkvalue IN VARCHAR2) IS
	...
	BEGIN
		...
		IF g_tabtemplatedata.COUNT>0 THEN
		...
		FOR recs IN c1 LOOP
			...
			dbms_sql.define_column(source_cursor,intcol,varchar_col,2000);
			intcol := intcol +1;
		END LOOP;
		intlineno := 4;
	...
	EXCEPTION
		...
		WHEN NO_DATA_FOUND THEN
			IF dbms_sql.is_open(source_cursor) THEN
				dbms_sql.close_cursor(source_cursor);
			END IF;
			RAISE_APPLICATION_ERROR(-20000,'Invalid Primary Key <<<'||strpkvalue||'>>>');
...
--------
	...
	FUNCTION strget_template_column_data(intcolumnpos IN INTEGER) RETURN VARCHAR2 IS
		strreturn 		VARCHAR2(2000);
	...
	BEGIN
		...
		IF intcolumnpos > mc_no_of_cols THEN
			RETURN NULL;
		END IF;
...
--------
	...
	PROCEDURE initialise_pk_data (strsql IN VARCHAR2) IS
...
--------
	...
	FUNCTION strget_template_pk_data(introwpos IN INTEGER) RETURN VARCHAR2 IS
		strreturn 		VARCHAR2(2000);
		intdelimcount	INTEGER;
		intdelimstartpos	INTEGER;
		intdelimendpos	INTEGER;
	...
	BEGIN
		...
		---than are in template data then bomb out with null!!!
		IF mc_template_pk IS NULL OR introwpos > mc_no_of_pks THEN
			RETURN NULL;
		END IF;
		...
		ELSE
			---Find the first delimiter position
			intdelimstartpos :=INSTR(mc_template_pk,mc_delimiter,1,(introwpos -1))+LENGTH(mc_delimiter);
			intdelimendpos :=INSTR(mc_template_pk,mc_delimiter,intdelimstartpos);
			strreturn := 'Start pos '||TO_CHAR(intdelimstartpos)||' end pos'||TO_CHAR(intdelimendpos);
			---Get the substring from these delimiter positions
			strreturn := SUBSTR(mc_template_pk,intdelimstartpos,intdelimendpos - intdelimstartpos);
		END IF;
...
--
--------
	FUNCTION intget_no_of_pks RETURN NUMBER IS
	BEGIN
		RETURN mc_no_of_pks;
	END;
...
--------
	...
	PROCEDURE resequencetemplate(strtemplate IN VARCHAR2) IS

		intseqno INTEGER := 1;
		CURSOR c1 IS
			SELECT dtc_col_seq,ROWNUM,ROWID FROM DOC_TEMPLATE_COLUMNS
			WHERE dtc_template_name = strtemplate
			ORDER BY 1,2;
...
BEGIN
	...
	BEGIN
		...
		IF c_strdateformat IS NULL THEN
