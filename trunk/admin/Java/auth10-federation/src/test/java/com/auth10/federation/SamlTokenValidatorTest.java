...
// - Added DefaultBootstrap.bootstrap() on construction.
// ----------------------------------------------------------------------------------------------
...
public class SamlTokenValidatorTest extends TestCase {
    ...
    public SamlTokenValidatorTest(String name) throws ConfigurationException {
        super(name);
    ...
    public void testOrgIdCertificateShouldBeValid() throws Exception {
        ...
        validator.setThumbprints("3F5DFCDF4B3D0EAB9BA49BEFB3CFD760DA9CCCF1");
        ...
        FederatedPrincipal principal = new FederatedPrincipal(validator.validate(office365Token));
    ...
    public void testShouldValidateSaml11TokenWithRSTRAndReturnClaims()
        ...
        FederatedPrincipal principal = new FederatedPrincipal(validator.validate(validTestToken));
        List<Claim> claims = principal.getClaims();

        assertEquals(6, claims.size());
        assertEquals("http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress", claims.get(0).getClaimType());
        assertEquals("test@company.com", claims.get(0).getClaimValue());
    }
    ...
    public void testShouldValidateSaml20TokenWithRSTRAndReturnClaims()
        ...
        assertEquals(10, claims.size());
        assertEquals("http://schemas.microsoft.com/accesscontrolservice/2010/07/claims/domain",    claims.get(0).getClaimType());
        assertEquals("globalbank.onmicrosoft.com", claims.get(0).getClaimValue());
    }
    ...
    public void testShouldThrowBecauseTheTokenAppliesToUnknownAudience()
        ...
        validator.setThumbprints(invalidTrustedIssuer);
