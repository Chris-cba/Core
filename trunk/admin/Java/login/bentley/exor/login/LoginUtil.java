...
public class LoginUtil {
	...
	public static String changePassword(String connectionString, String userName, String oldPassword, String newPassword) {
		...
		try {
			...
			String error = changePasswordSQLException.getMessage().toLowerCase(Locale.ENGLISH);
			...
			closeOracle("changePassword", null, conn);
		}
	...
	private OracleConnection getOracleConnection() throws Exception {
		...
		//you will need to create a Data Source with JNDI name <TNS_NAME>_LOGINDS
	...
	public static String getRelationship(String formsURL, String dataSource, String emailId) {
		String logMessage = formDebugMessage("getRelationship", "Forms URL", formsURL, "Datasource", dataSource, "Email-ID", emailId);
		IMSLogger.log(logMessage, IMSLogger.DEBUG);
		...
		try {
			...
			sqlStmt = "{call hig_sso_api.get_relationship(?, ?)}";
			cstmt  = conn.prepareCall(sqlStmt);
			...
			while (rset.next()) {
				...
				if(fetchSize > 1) {
					...
					result = rset.getString("ATTRIBUTE1").trim();
				}
			}
			...
			return result;
		} catch(Exception getRelationshipException) {
			logException(getRelationshipException, "getRelationship", IMSLogger.ERROR);
			return errorMessage;
		} finally {
			closeOracle("getRelationship", cstmt, conn);
		}
	}
	...
	private static void closeOracle(String functionName, CallableStatement cstmt, Connection conn) {
		try {
			if(cstmt != null) {
				cstmt.close();
			}
