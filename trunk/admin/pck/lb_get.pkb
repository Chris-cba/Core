...
AS
    ...
    BEGIN
        ...
        l_category := l_nit_row.nit_category;
        ...
        THEN
            ...
            nm_debug.debug (
                   'FT query - '
                || l_nit_row.nit_table_name
                || ','
                || p_obj_type
                || ','
                || l_nit_row.nit_foreign_pk_column
                || ','
                || l_nit_row.nit_lr_st_chain
                || ','
                || NVL (l_nit_row.nit_lr_end_chain,
                        l_nit_row.nit_lr_st_chain));
            --
            retval :=
    ...
    BEGIN
        ...
        RETURN get_refnt_RPt_tab_from_geom (
                   ...
                   p_nlt_ids       => l_nlt_ids,
                   p_constraint    =>
                       lb_nw_cons.get_aggr_constraint (p_inv_type),
    ...
    BEGIN
        ...
        SELECT    'with geom_tab as ( select :geom geoloc from dual ) '
               ...
               || CHR (10)
               || LISTAGG (select_string,
                           CHR (10) || 'UNION ALL' || CHR (10))
                  WITHIN GROUP (ORDER BY nlt_id)
               || CHR (10)
               || ')) s, nm_elements e where e.ne_id = s.ne_id and '
               || p_constraint
               || ')'    aggr_str
          INTO lstr
    ...
    BEGIN
        lstr :=
            ...
            || ' select /*+materialize */ '
            || nth.nth_pk_column
            || ' ne_id, sdo_lrs.geom_segment_start_measure('
            || nth.nth_feature_shape_column
            || ') start_m, sdo_lrs.geom_segment_end_measure('
            || nth.nth_feature_shape_column
            || ') end_m '
            || ' from '
            || nth.nth_feature_table
    ...
    FUNCTION get_refnt_RPt_tab_from_nse (p_nse_id        IN INTEGER,
                                         p_cardinality   IN INTEGER)
        RETURN lb_RPt_tab
    IS
        retval   lb_rpt_tab;
    ...
    FUNCTION get_refnt_RPt_tab_from_nte (p_nte_job_id    IN INTEGER,
                                         p_cardinality   IN INTEGER)
        RETURN lb_RPt_tab
    IS
        retval   lb_rpt_tab;
    BEGIN
        SELECT CAST (COLLECT (lb_rpt (nte_ne_id_of,
                                      nlt_id,
                                      NULL,
                                      NULL,
                                      1,
                                      ...
                                      nlt_units)) AS lb_rpt_tab)
          INTO retval
          FROM nm_linear_types, nm_elements, nm_nw_temp_extents
    ...
    BEGIN
        ...
        SELECT CAST (COLLECT (lb_rpt (nm_ne_id_of,
          ...
          FROM (SELECT nm_ne_id_of,
                       ...
                       nm_seq_no,
                       nm_dir_flag,
                       GREATEST (m.nm_begin_mp, g.nm_begin_mp)
                           nm_begin_mp,
                       LEAST (m.nm_end_mp, g.nm_end_mp)
                           nm_end_mp,
                       nlt_units
                                 ...
                                 MAX (end_slk)
                                     OVER (
                                         PARTITION BY route_id,
                                                      ...
                                                      inv_segment_id)
                                     inv_end_slk,
                                 FIRST_VALUE (nm_seg_no)
                                     OVER (
                                         PARTITION BY route_id,
                                                      inv_id,
                                                      inv_segment_id)
                                     start_seg,
                                 LAST_VALUE (nm_seg_no)
                                     OVER (
                                         PARTITION BY route_id,
                                                      inv_id,
                                                      inv_segment_id)
                ...
                SELECT DECODE (nit_table_name, NULL, 'N', 'Y'), nit_category
                 ...
                 WHERE     nit_inv_type = pi_obj_type
                       AND NOT EXISTS
                               (SELECT 1
                                  FROM nm_group_types
                                 WHERE ngt_group_type = pi_obj_type);
            EXCEPTION
                WHEN NO_DATA_FOUND
                THEN
                    BEGIN
                                ...
                                AS
                                    (SELECT nm_ne_id_in,
                                     ...
                                     SELECT c.nm_ne_id_in,
                                            ...
                                            c.nm_begin_mp,
                                            c.nm_end_mp
                                       FROM group_membs p, nm_members c
                                      WHERE c.nm_ne_id_in = p.nm_ne_id_of)
                            SELECT g.*, nlt_id, nlt_units
                              FROM group_membs  g,
                                   nm_elements,
                                   nm_linear_types
    ...
    BEGIN
        l_str :=
               ' SELECT /*+INDEX(e, NE_PK) */ cast ( collect(lb_rpt( ft.'
            || p_ne_key
            || ', t.nlt_id, '
            || ''''
            || p_inv_type
            ...
            || ', ft.'
            || p_end_col
            || ', t.nlt_units ) ) as lb_rpt_tab ) '
            || ' from '
            || p_table_name
            || ' ft, table (:p_rpt_tab) rtab, nm_elements e, nm_linear_types t'
            || ' where rtab.refnt = ft.'
            || p_ne_key
            || ' and ( rtab.start_m < ft.'
            || p_end_col
            ...
            || p_start_col
            || ')'
            || ' and t.nlt_nt_type = e.ne_nt_type '
            || ' and e.ne_id = rtab.refnt ';
    ...
    IS
        retval   NUMBER;
    BEGIN
        SELECT COUNT (*) INTO retval FROM TABLE (pi_rpt_tab);
    ...
    BEGIN
        ...
        --    nm_debug.debug_on;
        --    nm_debug.debug('referent type = '||l_ref_type||','||l_type_chk );
        ...
        ELSE
            OPEN retval FOR
                WITH
                    locs
                    AS
                        (SELECT nlt_g_i_d, get_count (pi_rpt_tab) rowcnt, t.*
