/* Formatted on 30/07/2018 10:03:22 (QP5 v5.326) */
CREATE OR REPLACE PACKAGE BODY lb_reg
--
--   PVCS Identifiers :-
--
--       pvcsid           : $Header:   //new_vm_latest/archives/lb/admin/pck/lb_reg.pkb-arc   1.15   Jul 30 2018 10:44:54   Rob.Coupe  $
--       Module Name      : $Workfile:   lb_reg.pkb  $
--       Date into PVCS   : $Date:   Jul 30 2018 10:44:54  $
--       Date fetched Out : $Modtime:   Jul 30 2018 10:44:30  $
...
AS
    ...
    --
    g_body_sccsid    CONSTANT VARCHAR2 (30) := '"$Revision:   1.15  $"';
    ...
    BEGIN
        ...
        BEGIN
            ...
            THEN
                raise_application_error (
                    -20004,
                    'LB Assets must be on a datum network');
            END IF;
        --
        EXCEPTION
            WHEN NO_DATA_FOUND
        ...
        IF    pi_role IS NULL
           OR pi_role_mode IS NULL
           OR pi_role_mode NOT IN ('NORMAL', 'READONLY')
        THEN
            raise_application_error (
                -20005,
                'Inventory type must be supplied with a role and mode with a value of NORMAL or READONLY');
        END IF;
        ...
        EXCEPTION
            ...
            THEN
                raise_application_error (
                    -20006,
                    'The supplied Exor role does not exist');
        END;

        -- create views to represent the LB assettype data
        ...
        THEN
            BEGIN
                INSERT INTO LB_TYPES (LB_OBJECT_TYPE,
                                      LB_ASSET_GROUP,
                                      LB_EXOR_INV_TYPE)
                         VALUES (pi_lb_object_type,
                                 pi_LB_asset_class,
                                 pi_exor_type);
            EXCEPTION
                WHEN DUP_VAL_ON_INDEX
    ...
    BEGIN
        ...
        DELETE FROM lb_types
              WHERE     lb_object_type = pi_lb_object_type
                    AND lb_exor_inv_type =
                        NVL (l_exor_type, lb_exor_inv_type);
    END;
    ...
    BEGIN
        ...
        BEGIN
            EXECUTE IMMEDIATE   'drop view V_LB_'
                             || pi_exor_type
                             || '_AGGR_SDO';
        EXCEPTION
            WHEN not_exists
