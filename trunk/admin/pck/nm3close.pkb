CREATE OR REPLACE PACKAGE BODY nm3close
AS
   --
   -----------------------------------------------------------------------------
   --
   --   PVCS Identifiers :-
   --
   --       pvcsid           : $Header:   //new_vm_latest/archives/nm3/admin/pck/nm3close.pkb-arc   2.24   Jun 25 2019 14:26:02   Rob.Coupe  $
   --       Module Name      : $Workfile:   nm3close.pkb  $
   --       Date into PVCS   : $Date:   Jun 25 2019 14:26:02  $
   --       Date fetched Out : $Modtime:   Jun 25 2019 14:25:14  $
   ...
   --
   TYPE tab_char_1 IS TABLE OF VARCHAR2 (1)
   ...
   -----------------------------------------------------------------------------
   --
   PROCEDURE gis_multi_element_close (
      pi_gdo_session_id    gis_data_objects.gdo_session_id%TYPE,
      pi_effective_date    DATE DEFAULT TO_DATE (
                                           SYS_CONTEXT ('NM3CORE',
                                                        'EFFECTIVE_DATE'),
                                           'DD-MON-YYYY'));
   ...
   -----------------------------------------------------------------------------
   --
   PROCEDURE route_close (
      pi_route_id           NM_ELEMENTS.ne_id%TYPE,
      pi_effective_date     DATE DEFAULT TO_DATE (
                                            SYS_CONTEXT ('NM3CORE',
                                                         'EFFECTIVE_DATE'),
                                            'DD-MON-YYYY'),
   ...
   -----------------------------------------------------------------------------
   --
   PROCEDURE close_temp_extent (
      pi_job_id            NM_NW_TEMP_EXTENTS.nte_job_id%TYPE,
      pi_effective_date    DATE DEFAULT TO_DATE (
                                           SYS_CONTEXT ('NM3CORE',
                                                        'EFFECTIVE_DATE'),
                                           'DD-MON-YYYY'));
   ...
   --
   FUNCTION multi_element_check (pi_type         IN VARCHAR2,
                                 pi_element_id   IN NUMBER)
      RETURN BOOLEAN;

   --

   PROCEDURE close_lb_data (
      p_ne_id            IN nm_elements.ne_id%TYPE,
      p_effective_date   IN DATE,
      p_neh_id           IN nm_element_history.neh_id%TYPE DEFAULT NULL,
      p_close_datums     IN BOOLEAN DEFAULT TRUE);
   ...
   -----------------------------------------------------------------------------
   --
   PROCEDURE insert_nm_element_history (
      p_ne_id            IN NM_ELEMENTS.ne_id%TYPE,
   ...
   PROCEDURE close_lb_data (
      ...
      p_close_datums     IN BOOLEAN DEFAULT TRUE)
   IS
      l_transaction_id   INTEGER; -- this is a local transaction ID, used when there is no NEH record to link to.

      get_lb_ne_rows     SYS_REFCURSOR;
      g_lb_ne_rows_sql   VARCHAR2 (4000)
         :=    'select nm_ne_id_of from nm_locations l where l.nm_ne_id_of in ( '
            || ' select nm_ne_id_of from nm_members_all where nm_ne_id_in = '
            || p_ne_id
            || ' union '
            || ' select '
            || p_ne_id
            || ' from dual ) group by nm_ne_id_of';
      l_ne_array         int_array_type;
   BEGIN
   
      nm_debug.proc_start (g_package_name, 'close_lb_data');
...
--            nm_debug.debug_on;
            ...
            IF p_neh_id IS NULL OR p_neh_id = g_neh_id
            THEN -- we are calling the close of LB stuff locally - i.e. where no core element history is being created.
               EXECUTE IMMEDIATE
                  'select lb_transaction_id_seq.nextval from dual'
...
--               || l_transaction_id);
            ...
            IF p_close_datums
            THEN
               OPEN get_lb_ne_rows FOR g_lb_ne_rows_sql;

               FETCH get_lb_ne_rows BULK COLLECT INTO l_ne_array;

               CLOSE get_lb_ne_rows;

               FOR i IN 1 .. l_ne_array.COUNT
               LOOP
                  --
                  l_block :=
                     'begin lb_nw_edit.lb_close(:p_ne, :p_effective_date, :p_transaction_id); end; ';

                  --
                  EXECUTE IMMEDIATE l_block
                     USING l_ne_array (i), p_effective_date, l_transaction_id;
               END LOOP;
            ...
            ELSE
                  ...
                  --
                  EXECUTE IMMEDIATE l_block
                     USING p_ne_id, p_effective_date, l_transaction_id;
            
            END IF;
--
--            nm_debug.debug (
--                  'lb - Transaction - local transaction_id = '
--               || l_transaction_id);

            EXECUTE IMMEDIATE
               'begin lb_nw_edit.log_transaction( :transaction_id, :neh_id ); end; '
               USING l_transaction_id, p_neh_id;
         END;
      END IF;

      nm_debug.proc_end (g_package_name, 'close_lb_data');
      
   END;



   PROCEDURE check_contiguous_inv_type (
      p_nm_ne_id_of    NM_MEMBERS.nm_ne_id_of%TYPE)
   IS
      -- get ne_id of any inv that is contiguous on an element
      CURSOR c1 (c_nm_ne_id_of NM_MEMBERS.nm_ne_id_of%TYPE)
      IS
         SELECT nm_ne_id_in
           FROM NM_MEMBERS
      ...
      CURSOR c2 (c_nm_ne_id_in NM_MEMBERS.nm_ne_id_in%TYPE)
      IS
         SELECT nm_ne_id_of
           FROM NM_MEMBERS
   ...
   BEGIN
      ...
      FOR c1rec IN c1 (p_nm_ne_id_of)
      LOOP
         FOR c2rec IN c2 (c1rec.nm_ne_id_in)
         LOOP
            IF c2rec.nm_ne_id_of != p_nm_ne_id_of
            THEN
               RAISE_APPLICATION_ERROR (
                  -20100,
                  'Contiguous inv_item over more than current element');
            END IF;
         END LOOP;
      END LOOP;
   ...
   -----------------------------------------------------------------------------
   --
   PROCEDURE check_replaceable_inv_type (
      p_nm_ne_id_of    NM_MEMBERS.nm_ne_id_of%TYPE)
   IS
   ...
   BEGIN
      ...
      THEN
         RAISE_APPLICATION_ERROR (
            -20110,
            'Unable to close Element it has un-replaceable inventory');
      END IF;
   ...
   -----------------------------------------------------------------------------
   --
   FUNCTION inv_type_end_loc_only (
   ...
   IS
      ...
      IS
         SELECT 1
           FROM NM_INV_TYPES
          WHERE nit_inv_type = p_nit_inv_type AND nit_end_loc_only = 'Y';
   ...
   PROCEDURE close_groups (
      ...
      p_effective_date    DATE DEFAULT TO_DATE (
                                          ...
                                          'DD-MON-YYYY'))
   IS
      TYPE l_rec_nmh_tab IS TABLE OF nm_member_history%ROWTYPE
         INDEX BY BINARY_INTEGER;
   ...
   BEGIN
      nm_debug.proc_start (g_package_name, 'close_groups');
         ...
         UPDATE NM_MEMBERS
          ...
          WHERE nm_type = 'G' AND nm_ne_id_of = p_ne_id AND nm_end_date IS NULL
   ...
   -----------------------------------------------------------------------------
   --
   PROCEDURE close_other_products (
      p_ne_id             NM_ELEMENTS.ne_id%TYPE,
   ...
   BEGIN
      ...
      THEN
         ...
         THEN
            EXECUTE IMMEDIATE
               ...
               || CHR (10)
               || 'END;'
               USING IN p_ne_id, p_effective_date;
         ELSE
            EXECUTE IMMEDIATE
                  'BEGIN'
               || CHR (10)
               || '   accclose.do_close_from_temp_extent (:p_ne_id'
               || CHR (10)
               || '                                      ,:p_effective_date'
               || CHR (10)
               || '                                      );'
      ...
      THEN
         ...
         EXECUTE IMMEDIATE
               'BEGIN'
            || CHR (10)
            || '   maiclose.close_data (p_id             => :p_ne_id'
            || CHR (10)
      ...
      THEN
         ...
         EXECUTE IMMEDIATE
               'BEGIN'
            || CHR (10)
            || '   stp_network_ops.do_close(pi_ne_id          => :pi_ne_id'
            || CHR (10)
   ...
   -----------------------------------------------------------------------------
   --
   PROCEDURE close_other_products_route (
      p_ne_id             NM_ELEMENTS.ne_id%TYPE,
      p_effective_date    NM_ELEMENTS.ne_start_date%TYPE)
   IS
   BEGIN
   ...
   IS
      --
      CURSOR c_check_close
      IS
         SELECT 1
           FROM nm_elements_all
          WHERE ne_id = p_ne_id AND ne_end_date IS NOT NULL;
   ...
   BEGIN
      ...
      THEN
         ...
         RAISE_APPLICATION_ERROR (
            -20001,
            'Cannot close an element that has already been closed');
      --
      END IF;
      ...
      -- ** Will be removed **
      -- *********************
      IF p_effective_date > TRUNC (SYSDATE)
      THEN
      ...
      nm3nwval.network_operations_check (nm3nwval.c_close, p_ne_id);
      --
      ...
      BEGIN
         ...
         LOOP
            IF     nm3merge.g_tab_nmh_nm_type (i) = 'I'
               AND nm3inv.get_inv_type (nm3merge.g_tab_nmh_nm_obj_type (i)).nit_end_loc_only =
                      'N'
            THEN
      ...
      --
      --
      insert_nm_element_history (p_ne_id, p_effective_date, p_neh_descr --CWS 0108990 12/03/2010
                                                                       ); -- Insert the stored NM_MEMBER_HISTORY records

--      nm_debug.debug ('Global neh_id is ' || g_neh_id);

      close_lb_data (p_ne_id            => p_ne_id,
                     p_effective_date   => p_effective_date,
                     p_neh_id           => g_neh_id,
                     p_close_datums     => FALSE ); 
      ...
      close_other_products (p_ne_id, p_effective_date);
      ...
      FOR i IN 1 .. l_aggr_list.COUNT
      LOOP
         nm_inv_sdo_aggr.aggregate_inv_geometry (l_aggr_list (i).ptr_value,
                                                 l_aggr_list (i).ptr_id);
      END LOOP;
   ...
   IS
      --
      CURSOR cs_gdo (
         c_session_id    gis_data_objects.gdo_session_id%TYPE)
      IS
           SELECT ne_id, nm_seq_no
             FROM gis_data_objects, NM_ELEMENTS, NM_MEMBERS
   ...
   BEGIN
      ...
      IF pi_type = c_route
      THEN
         nm3nwval.network_operations_check (nm3nwval.c_close, pi_id);
      ELSIF pi_type = 'EXT' AND g_multi_check
      THEN
         IF NOT multi_element_check (pi_type         => pi_type,
                                     pi_element_id   => pi_id)
         THEN
      ...
      IF pi_type = nm3close.c_gis
      THEN
         gis_multi_element_close (pi_id, pi_effective_date);
      ELSIF pi_type = nm3close.c_extent
      THEN
         close_temp_extent (pi_id, pi_effective_date);
      ELSIF pi_type = nm3close.c_route
      THEN                                -- Group of Groups/Group of Sections
      ...
      ELSE
         RAISE_APPLICATION_ERROR (
            -20001,
               'Invalid type "'
            || pi_type
            || '" passed to nm3close.Multi_Element_Close');
      END IF;
   ...
   -----------------------------------------------------------------------------
   --
   FUNCTION is_group_to_be_closed (
      pi_parent_ne_type       IN nm_elements.ne_type%TYPE,
   ...
   BEGIN
      ...
      BEGIN
         v_nti_record :=
            nm3get.get_nti (pi_nti_nw_parent_type   => pi_nti_nw_parent_type,
   ...
   -----------------------------------------------------------------------------
   --
   PROCEDURE get_membership_hierarchy (
      pi_route_id                   IN NM_ELEMENTS.ne_id%TYPE,
      pi_end_date_all               IN VARCHAR2 DEFAULT 'N' --                                  ,pi_end_date_datums               IN     VARCHAR2 DEFAULT 'N'
                                                           ,
   ...
   IS
      ...
      PROCEDURE append (pi_text IN VARCHAR2, pi_nl IN BOOLEAN DEFAULT TRUE)
      IS
      ------------------------------------------
   ...
   BEGIN
      ...
      append (
         '        LEVEL                                                                 depth');
      append (
         '       ,nm3net.get_ne_unique(nm_ne_id_of)                                     label');
      append (
         '       ,nm3net.get_ne_type(nm_ne_id_of)                                       child_ne_type');
      append (
         '       ,nm_ne_id_of                                                           child_ne_id');
      append (
         '       ,nm3close.is_group_to_be_closed(nm3net.get_ne_type(nm_ne_id_in)');
      append (
         '                                      ,nm3net.get_nt_type(nm_ne_id_in)');
      append (
         '                                      ,nm3net.get_nt_type(nm_ne_id_of)');
      append (
         '                                      ,:pi_end_date_all)               end_date_flag');
      append ('    FROM');
      append ('        nm_members');
      append ('    WHERE');
      append ('        nm3net.get_ne_gty( nm_ne_id_of ) IS NOT NULL');

      IF NOT pi_include_non_closed_nodes
      THEN
         append (
            '    AND nm3close.is_group_to_be_closed(nm3net.get_ne_type(nm_ne_id_in)');
         append (
            '                                        ,nm3net.get_nt_type(nm_ne_id_in)');
         append (
            '                                        ,nm3net.get_nt_type(nm_ne_id_of)');
      ...
      THEN
         ...
         OPEN l_ref_cursor FOR l_qry
            USING pi_end_date_all,
                  pi_end_date_all,
                  pi_end_date_all,
                  pi_route_id;
      ELSE
         --      nm_debug.debug('Our query will return all of the nodes');
         OPEN l_ref_cursor FOR l_qry USING pi_end_date_all, pi_route_id;
      END IF;
      ...
      LOOP
         ...
         FETCH l_ref_cursor
            INTO g_tab_rte_memb_depth (v_counter),
   ...
   -----------------------------------------------------------------------------
   --
   FUNCTION datums_are_to_be_closed (
   ...
   BEGIN
      ...
      --------------------------------------------------------------
      IF     pi_ne_type = 'G'
         AND (   pi_end_date_datums = 'Y'
              OR nm3net.is_nt_inclusion (
                    pi_nt   => nm3net.get_nt_type (pi_ne_id)))
   ...
   BEGIN
      ...
      THEN
         ...
         BEGIN
            nm3extent.create_temp_ne (pi_source_id   => pi_ne_id,
                                      ...
                                      po_job_id      => l_job_id);
            g_multi_check := FALSE;
            multi_element_close (nm3close.c_extent,
                                 l_job_id,
                                 pi_effective_date);
            g_multi_check := TRUE;
         EXCEPTION
      ...
      END IF;
         ...
         UPDATE NM_MEMBERS
          ...
          WHERE nm_ne_id_in = pi_ne_id AND nm_end_date IS NULL
      RETURNING NM_NE_ID_IN,
                NM_NE_ID_OF,
                NM_NE_ID_OF,
                NM_BEGIN_MP,
      ...
      close_lb_data (p_ne_id            => pi_ne_id,
                     p_effective_date   => pi_effective_date,
                     p_neh_id           => g_neh_id,
                     p_close_datums     => l_close_datums);

      -----------------------------------------------------------------
      -- end date any Additional Data that is associated to this element
      ------------------------------------------------------------------
      nm3nwad.end_date_all_ad_for_element (
         pi_ne_id            => pi_ne_id,
         pi_effective_date   => pi_effective_date);
   ...
   PROCEDURE route_close (
      ...
      pi_end_date_datums    VARCHAR2 DEFAULT 'N')
   IS
      CURSOR is_route_id (c_ne_id NM_ELEMENTS.ne_id%TYPE)
      IS
             SELECT *
               FROM NM_ELEMENTS
              WHERE ne_id = c_ne_id AND ne_type IN ('P', 'G')
      ...
      */
      --
      PROCEDURE close_shape (p_route_id         IN NM_ELEMENTS.ne_id%TYPE,
                             p_effective_date   IN DATE)
      IS
         CURSOR c1 (
            c_route_id   IN NM_ELEMENTS.ne_id%TYPE)
         IS
      ...
      BEGIN
         ...
         FOR irec IN c1 (p_route_id)
         LOOP
            NM3SDO_EDIT.END_DATE_SHAPE (irec.nth_theme_id,
                                        p_route_id,
                                        pi_effective_date);
         END LOOP;
      END;
   BEGIN
      ...
      get_membership_hierarchy (pi_route_id                   => pi_route_id,
                                pi_end_date_all               => pi_end_date_all --                          ,pi_end_date_datums          => pi_end_date_datums
                                                                                ,
      ...
      g_tab_rte_memb_child_ne_type (l_next_recno) :=
         nm3net.get_ne_type (pi_route_id);
      ...
      ---------------------------------------------------------------------------------------
      FOR i IN 1 .. g_tab_rte_memb_depth.COUNT
      LOOP
         close_element (pi_ne_id             => g_tab_rte_memb_child_ne_id (i),
                        pi_ne_type           => g_tab_rte_memb_child_ne_type (i),
                        pi_effective_date    => pi_effective_date,
                        pi_end_date_datums   => pi_end_date_datums);
      --    nm_debug.debug('lb - Closing LB data if licensed, using ne_id = '||g_tab_rte_memb_child_ne_id(i) );
      --
      --    close_lb_data ( p_ne_id          => g_tab_rte_memb_child_ne_id(i),
      --                    p_effective_date => pi_effective_date);
   ...
   IS
      --
      CURSOR c1 (c_job_id NM_NW_TEMP_EXTENTS.nte_job_id%TYPE)
      IS
         SELECT nte_ne_id_of
      ...
      --
      CURSOR cs_members (
         c_job_id    NM_NW_TEMP_EXTENTS.nte_job_id%TYPE)
      IS
      ...
      --
      CURSOR cs_elements (c_job_id NM_NW_TEMP_EXTENTS.nte_job_id%TYPE)
      IS
             SELECT ne.ROWID ne_rowid, ne_id
               FROM NM_ELEMENTS ne, NM_NW_TEMP_EXTENTS nte
              WHERE nte.nte_job_id = c_job_id AND ne.ne_id = nte.nte_ne_id_of
      ...
      --
      CURSOR cs_inv (c_iit_ne_id NUMBER)
      IS
             SELECT ROWID
               FROM NM_INV_ITEMS
   ...
   BEGIN
      ...
      FETCH cs_members
         BULK COLLECT INTO l_tab_nm_ne_id_in,
              l_tab_nm_type,
              l_tab_nm_obj_type,
              l_tab_nm_rowid;
      ...
      FETCH cs_elements BULK COLLECT INTO l_tab_ne_rowid, l_tab_ne_id;
      ...
      FORALL i IN 1 .. l_tab_nm_rowid.COUNT
            UPDATE NM_MEMBERS
               SET nm_end_date = pi_effective_date
             WHERE ROWID = l_tab_nm_rowid (i) AND NM_END_DATE IS NULL
         RETURNING NM_NE_ID_IN,
                   NM_NE_ID_OF,
                   NM_NE_ID_OF,
                   NM_BEGIN_MP,
      ...
      --      close_lb_data( l_tab_ne_id(i), pi_effective_date );
      --    END LOOP;
      --
      FOR i IN 1 .. l_rec_nmh.COUNT
      LOOP
      ...
      LOOP
         ...
         IF     l_tab_nm_type (l_count) = 'I'
            AND nm3inv.get_inv_type (l_tab_nm_obj_type (l_count)).nit_end_loc_only =
                   'N'
   ...
   PROCEDURE close_node (
      pi_no_node_id        nm_nodes.no_node_id%TYPE,
      pi_effective_date    DATE DEFAULT TO_DATE (
                                           SYS_CONTEXT ('NM3CORE',
                                                        'EFFECTIVE_DATE'),
                                           'DD-MON-YYYY'))
      ...
      BEGIN
         WITH datums
              AS (SELECT nte_ne_id_of datum_ne_id
                    FROM nm_nw_temp_extents
                   WHERE nte_job_id = pi_element_id AND pi_type = 'EXT' --  union all
                                                        --  select nm_ne_id_of
                                                           --  from nm_members
                                                     --  where :c_type = 'ROU'
