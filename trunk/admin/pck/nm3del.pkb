...
--   SCCS Identifiers :-
--
--       sccsid           : @(#)nm3del.pkb	1.50 03/22/07
--       Module Name      : nm3del.pkb
--       Date into SCCS   : 07/03/22 14:11:01
--       Date fetched Out : 07/06/13 14:11:21
--       SCCS Version     : 1.50
--
--
...
-----------------------------------------------------------------------------
--
   g_body_sccsid CONSTANT  VARCHAR2(2000) := '"@(#)nm3del.pkb	1.50 03/22/07"';
...
--
--
--   Procedure to del using GRS_PK constraint
--
PROCEDURE del_grs (pi_grs_line_no       gri_spool.grs_line_no%TYPE
...
BEGIN
--
   nm_debug.proc_start(g_package_name,'del_grs');
--
   -- Lock the row first
   l_rowid := nm3lock_gen.lock_grs
                   (pi_grs_line_no       => pi_grs_line_no
                   ,pi_grs_job_id        => pi_grs_job_id
                   ,pi_raise_not_found   => pi_raise_not_found
                   ,pi_not_found_sqlcode => pi_not_found_sqlcode
                   ,pi_locked_sqlcode    => pi_locked_sqlcode
                   );
--
   IF l_rowid IS NOT NULL
    THEN
      DELETE gri_spool grs
      WHERE ROWID = l_rowid;
   END IF;
--
   nm_debug.proc_end(g_package_name,'del_grs');
--
END del_grs;
--
-----------------------------------------------------------------------------
...
--
PROCEDURE del_iig (pi_iig_top_id        nm_inv_item_groupings.iig_top_id%TYPE
...
--
--
--   Procedure to del using IIG_UK constraint
--
...
--   Procedure to del using IIG_PK constraint
--
PROCEDURE del_iig_all (pi_iig_top_id        nm_inv_item_groupings_all.iig_top_id%TYPE
