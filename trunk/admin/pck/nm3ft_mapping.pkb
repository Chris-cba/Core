...
BEGIN
  /*
  || Get The Column Names For Ta-ble
  ...
  */
  ...
  SELECT column_name
    ...
    case
       when column_name in ('IIT_NE_ID', 'IIT_DESCR', 'IIT_INV_TYPE', 'IIT_PRIMARY_KEY', 'IIT_FOREIGN_KEY' ) then -1
       else 1
    end is_flexible
    BULK COLLECT
    INTO lt_cols
...
--  IF SUBSTR(lt_cols(i).inv_col_name,1,14) = 'IIT_NUM_ATTRIB'
    IF lt_cols(i).is_flexible = 1 and lt_cols(i).inv_col_data_type = 'NUMBER'
     THEN
...
--  ELSIF SUBSTR(lt_cols(i).inv_col_name,1,14) = 'IIT_CHR_ATTRIB'
    ELSIF lt_cols(i).is_flexible = 1 and lt_cols(i).inv_col_data_type = 'VARCHAR2'
     THEN
...
--  ELSIF SUBSTR(lt_cols(i).inv_col_name,1,15) = 'IIT_DATE_ATTRIB'
    ELSIF lt_cols(i).is_flexible = 1 and lt_cols(i).inv_col_data_type = 'DATE'     THEN
