CREATE OR REPLACE PACKAGE BODY nm3inv
AS
  --
  -----------------------------------------------------------------------------
  --
  --   PVCS Identifiers :-
  --
  --       sccsid           : $Header:   //new_vm_latest/archives/nm3/admin/pck/nm3inv.pkb-arc   2.36   Apr 16 2018 09:22:48   Gaurav.Gaurkar  $
  --       Module Name      : $Workfile:   nm3inv.pkb  $
  --       Date into SCCS   : $Date:   Apr 16 2018 09:22:48  $
  ...
  -- 10/10/2014  SJS - Added overloaded functions with effective date for get_attrib_value
  --             and get_all_attrib_values for HA Core Update.
  --
  ------------------------------------------------------------------
  --   Copyright (c) 2018 Bentley Systems Incorporated. All rights reserved.
  ------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------------------------------
  --
FUNCTION get_ita_by_attrib_name(
...
BEGIN
  ...
  ELSE
    ...
    IF c2%NOTFOUND THEN
      CLOSE c2;
      RAISE_APPLICATION_ERROR( -20001 ,Hig.replace_strings_in_message(g_thing_does_not_exist, 'Inventory Type'));
    END IF;
    CLOSE c2;
  END IF;
  --
...
FUNCTION view_exists(
    inv_view_name IN NM_INV_TYPES.nit_view_name%TYPE )
  RETURN BOOLEAN
IS
  --
BEGIN
...
--
FUNCTION view_in_use(
    p_view_name IN NM_INV_TYPES.nit_view_name%TYPE )
  RETURN BOOLEAN
IS
  --
  ...
  --
  CURSOR in_use
  IS -- Dummy cursor for the present.
    SELECT 1 FROM dual;
  --
  v_in_use INTEGER;
  --
...
FUNCTION synonym_exists(
    SYNONYM IN NM_INV_TYPES.nit_view_name%TYPE)
  RETURN BOOLEAN
IS
  --
BEGIN
...
BEGIN
  ...
  -- Code moved into NM3INV_VIEW package
  --
  Nm3inv_View.create_inv_view (p_inventory_type ,p_join_to_network ,p_view_name );
  --
...
----------------------------------------------------------------------------------------------
--
FUNCTION get_create_inv_view_text(
    p_inventory_type  IN NM_INV_TYPES.nit_inv_type%TYPE ,
    p_join_to_network IN BOOLEAN DEFAULT FALSE )
  RETURN VARCHAR2
IS
...
----------------------------------------------------------------------------------------------
--
FUNCTION work_out_inv_type_view_name(
...
----------------------------------------------------------------------------------------------
--
FUNCTION derive_inv_type_view_name(
    pi_inv_type IN NM_INV_TYPES.nit_inv_type%TYPE)
  RETURN VARCHAR2
IS
BEGIN
...
----------------------------------------------------------------------------------------------
--
FUNCTION derive_nw_inv_type_view_name(
    pi_inv_type IN NM_INV_TYPES.nit_inv_type%TYPE)
  RETURN VARCHAR2
IS
BEGIN
...
----------------------------------------------------------------------------------------------
--
FUNCTION get_xsp_descr(
    p_inv_type   IN XSP_RESTRAINTS.xsr_ity_inv_code%TYPE ,
...
BEGIN
  ...
  IF c1%NOTFOUND THEN
    CLOSE c1;
    RAISE_APPLICATION_ERROR( -20001 ,Hig.replace_strings_in_message(g_thing_does_not_exist, 'XSP'));
  END IF;
  CLOSE c1;
  RETURN l_retval;
END;
...
BEGIN
  ...
  SELECT ROWNUM, nit_inv_type FROM NM_INV_TYPES;
EXCEPTION
...
FUNCTION get_ita_domain_sql(
    pi_ita_id_domain IN NM_INV_TYPE_ATTRIBS_ALL.ita_id_domain%TYPE)
  RETURN VARCHAR2
IS
BEGIN
...
----------------------------------------------------------------------------------------------
--
FUNCTION build_ita_lov_sql_string(
...
BEGIN
  --
  l_rec_ita              := Nm3get.get_ita (pi_ita_inv_type => pi_ita_inv_type ,pi_ita_attrib_name => pi_ita_attrib_name ,pi_raise_not_found => FALSE );
  IF l_rec_ita.ita_query IS NOT NULL THEN
    ...
    ELSIF pi_replace_bind_variable_with           IS NOT NULL THEN -- replace bind variable with given value
      IF INSTR(pi_replace_bind_variable_with,'%') != 0 THEN
        Hig.raise_ner(pi_appl => 'NET' ,pi_id => 394 ,pi_supplementary_info => CHR(10)||Nm3flx.string(l_rec_ita.ita_scrn_text));
      END IF;
      l_retval := REPLACE(l_rec_ita.ita_query ,Nm3flx.extract_bind_variable(l_rec_ita.ita_query) ,Nm3flx.string(pi_replace_bind_variable_with) );
    ELSE
    ...
    END IF;
  ELSIF l_rec_ita.ita_id_domain IS NOT NULL THEN
  ...
  IF l_retval IS NOT NULL AND NOT Nm3flx.is_select_statement_valid (p_sql => l_retval) THEN
    Hig.raise_ner(pi_appl => 'NET' ,pi_id => 388 ,pi_supplementary_info => CHR(10)||Nm3flx.string(l_rec_ita.ita_scrn_text)||CHR(10)||l_retval); -- Parse error in query
  END IF;
  RETURN l_retval;
  --
  --
...
BEGIN
  ...
  --
  IF (b_notfound) THEN
    RAISE_APPLICATION_ERROR( -20001 ,Hig.replace_strings_in_message(g_thing_does_not_exist,a_ial_domain));
  END IF;
  --
END valid_fk_ial;
--
...
BEGIN
  --
  valid_fk_ial (a_ial_domain => a_ial_domain ,a_ial_value => LTRIM(TO_CHAR(a_ial_value)) ,a_ial_meaning => a_ial_meaning );
  --
END valid_fk_ial;
--
----------------------------------------------------------------------------------------------
...
IS
  CURSOR c1(p_domain IN NM_INV_ATTRI_LOOKUP_ALL.ial_domain%TYPE ,p_value IN NM_INV_ATTRI_LOOKUP_ALL.ial_value%TYPE)
  IS
    SELECT ial.ial_meaning
    FROM NM_INV_ATTRI_LOOKUP_ALL ial
    WHERE ial.ial_domain = p_domain
      --CWS 0109619 Any end dated domains fail regardless of when the end date is.
    AND ial.ial_end_date IS NULL
      --
    AND ial.ial_value = p_value;
  l_retval NM_INV_ATTRI_LOOKUP_ALL.ial_meaning%TYPE;
BEGIN
  OPEN c1(p_domain => pi_domain ,p_value => pi_value);
  FETCH c1 INTO l_retval;
  IF c1%NOTFOUND THEN
    CLOSE c1;
    RAISE_APPLICATION_ERROR(-20001, 'Inventory domain ' || pi_domain || ', value ' || pi_value || ' not found.');
  END IF;
  CLOSE c1;
  RETURN l_retval;
...
IS
BEGIN
  RETURN get_inv_domain_meaning(pi_domain => pi_domain ,pi_value => LTRIM(TO_CHAR(pi_value)) );
END get_inv_domain_meaning;
--
----------------------------------------------------------------------------------------------
--
...
IS
  ...
  IS
    ...
    FROM NM_INV_ATTRI_LOOKUP_ALL ial
    WHERE ial.ial_domain                                                         = p_domain
    AND TO_CHAR(Hig.date_convert(ial.ial_value),Nm3type.c_full_date_time_format) = TO_CHAR(pi_value,Nm3type.c_full_date_time_format);
  l_retval NM_INV_ATTRI_LOOKUP_ALL.ial_meaning%TYPE;
BEGIN
  OPEN c1(p_domain => pi_domain ,p_value => pi_value);
  FETCH c1 INTO l_retval;
...
--
-- CP 30/11/05
FUNCTION get_inv_domain_value(
    pi_domain  IN NM_INV_ATTRI_LOOKUP_ALL.ial_domain%TYPE ,
    pi_meaning IN OUT NM_INV_ATTRI_LOOKUP_ALL.ial_meaning%TYPE )
  RETURN NM_INV_ATTRI_LOOKUP_ALL.ial_value%TYPE
IS
  CURSOR c1 (p_domain IN NM_INV_ATTRI_LOOKUP_ALL.ial_domain%TYPE ,p_meaning IN NM_INV_ATTRI_LOOKUP_ALL.ial_meaning%TYPE)
  IS
    SELECT ial_value ,
      ial_meaning
...
BEGIN
  OPEN c1(p_domain => pi_domain ,p_meaning => pi_meaning);
  FETCH c1
  INTO l_retval,
    l_retmeaning;
  IF c1%NOTFOUND THEN
    CLOSE c1;
    RAISE_APPLICATION_ERROR(-20001, 'Inventory domain ' || pi_domain || ', meaning ' || pi_meaning || ' not found.');
  END IF;
  CLOSE c1;
...
IS
BEGIN
  validate_flex_inv (p_inv_type => p_inv_type ,p_attrib_name => p_attrib_name ,pi_value => TO_CHAR(pi_value,Nm3type.c_full_date_time_format) ,po_value => po_value ,po_meaning => po_meaning ,pi_validate_domain_dates => pi_validate_domain_dates );
...
BEGIN
  ...
  END IF;
  --
  l_rec_nita := Nm3get.get_ita (pi_ita_inv_type => p_inv_type ,pi_ita_attrib_name => p_attrib_name ,pi_raise_not_found => FALSE );
  --
  l_qry := Nm3inv.build_ita_lov_sql_string (pi_ita_inv_type => p_inv_type ,pi_ita_attrib_name => p_attrib_name ,pi_include_bind_variable => TRUE ,pi_replace_bind_variable_with => NULL);
  --
  ...
  IF Nm3flx.extract_bind_variable(l_qry,1) IS NOT NULL THEN
    l_qry                                  := Nm3inv.build_ita_lov_sql_string (pi_ita_inv_type => p_inv_type ,pi_ita_attrib_name => p_attrib_name ,pi_include_bind_variable => FALSE ,pi_replace_bind_variable_with => pi_bind_variable_value);
  END IF;
  --nm_debug.debug_on;
  ...
  IF l_qry IS NOT NULL THEN
    BEGIN
      Nm3extlov.validate_lov_value (p_statement => l_qry ,p_value => pi_value ,p_meaning => po_meaning ,p_id => l_id ,pi_match_col => 3) ; -- important to match on the correct column in the sql query string
      po_value := l_id;
    EXCEPTION
    WHEN OTHERS THEN
      Hig.raise_ner(pi_appl => 'NET' ,pi_id => 389 ,pi_supplementary_info => CHR(10)||l_rec_nita.ita_scrn_text||CHR(10)||Nm3flx.string(pi_value));
    END;
  END IF;
  --
  IF l_rec_nita.ita_format = Nm3type.c_date THEN
    --
    DECLARE
      l_date DATE;
      --
    ...
    BEGIN
      ...
      IF l_date IS NULL THEN
        Hig.raise_ner (pi_appl => Nm3type.c_hig ,pi_id => 148 ,pi_supplementary_info => pi_value );
  ...
  ELSIF l_rec_nita.ita_format = Nm3type.c_number THEN
    ...
    BEGIN
      ...
      --
      IF NOT Nm3flx.is_numeric(l_value) THEN
        Hig.raise_ner (pi_appl => Nm3type.c_hig ,pi_id => 111 ,pi_supplementary_info => pi_value );
        --            RAISE g_flex_validation_exception;
      END IF;
      --
      ...
      END IF ;
      IF l_raise_len_error THEN
        hig.raise_ner(pi_appl => 'NET' ,pi_id => 29 ,pi_supplementary_info => 'Length ' || l_length || ' is greater than column length ' || l_rec_nita.ita_fld_length );
      END IF ;
      IF l_rec_nita.ita_dec_places                               IS NOT NULL AND Instr(l_num_length,'.') > 0 THEN
        IF LENGTH(SUBSTR(l_num_length,Instr(l_num_length,'.')                                                                                          +1)) > l_rec_nita.ita_dec_places THEN
          hig.raise_ner(pi_appl => 'NET' ,pi_id => 29 ,pi_supplementary_info => 'Decimal Places ' || LENGTH(SUBSTR(l_num_length,Instr(l_num_length,'.')+1)) || ' is greater than column decimal places ' || l_rec_nita.ita_dec_places );
        END IF ;
      END IF ;
      -- Log 702388:Linesh:11-Mar-09:End
      --
      IF NVL(l_rec_nita.ita_min,l_number) > l_number OR NVL(l_rec_nita.ita_max,l_number) < l_number THEN
        Hig.raise_ner (pi_appl => Nm3type.c_net ,pi_id => 29 ,pi_supplementary_info => pi_value||' NOT BETWEEN '||l_rec_nita.ita_min||' AND '||l_rec_nita.ita_max );
        --            RAISE g_flex_validation_exception;
      END IF;
      --
      IF l_rec_nita.ita_units IS NOT NULL THEN
        --
        DECLARE
          ex_value_error EXCEPTION;
          PRAGMA EXCEPTION_INIT( ex_value_error, -6502 );
        BEGIN
          l_number   := TO_NUMBER(Nm3unit.get_formatted_value (p_value => l_number ,p_unit_id => l_rec_nita.ita_units ) );
        ...
        WHEN ex_value_error THEN
          ...
          Hig.raise_ner (pi_appl => Nm3type.c_net ,pi_id => 456 ,pi_supplementary_info => '['||l_rec_nita.ita_fld_length||'-' ||l_rec_nita.ita_dec_places ||'] : ['||nm3get.get_un(pi_un_unit_id => l_rec_nita.ita_units , pi_raise_not_found =>FALSE).un_format_mask||']' );
        END;
        --
      END IF;
      --
      IF l_rec_nita.ita_dec_places  IS NOT NULL THEN
        IF l_rec_nita.ita_dec_places = 0 THEN
          po_value                  := LTRIM(TO_CHAR(l_number ,c_format_string ) );
        ELSE
          po_value := LTRIM(TO_CHAR(l_number ,c_format_string ||'D' ||SUBSTR(c_format_string,1,l_rec_nita.ita_dec_places) ) );
        END IF;
      ELSE
        po_value := LTRIM(TO_CHAR(l_number));
      END IF;
      --
      IF l_rec_nita.ita_format_mask IS NOT NULL THEN
        po_value                    := LTRIM(TO_CHAR(TO_NUMBER(po_value),l_rec_nita.ita_format_mask));
        IF REPLACE(po_value,'#')    IS NULL THEN
          Hig.raise_ner (pi_appl => Nm3type.c_net ,pi_id => 458 );
        END IF ;
      END IF;
      --
    END;
    --
  ELSIF l_rec_nita.ita_format = Nm3type.c_varchar THEN
    --
    DECLARE
    ...
    BEGIN
      ...
      --
      IF LENGTH(l_out_value) > l_rec_nita.ita_fld_length THEN
        Hig.raise_ner (pi_appl => Nm3type.c_net ,pi_id => 275 );
...
IS
BEGIN
  RETURN validate_flex_inv(pi_inv_type => pi_inv_type ,pi_attrib_name => pi_attrib_name ,pi_value => TO_CHAR(pi_value,Nm3type.c_full_date_time_format) ,pi_bind_variable_value => pi_bind_variable_value );
END validate_flex_inv;
--
----------------------------------------------------------------------------------------------
--
...
BEGIN
  --
  BEGIN
    validate_flex_inv (p_inv_type => pi_inv_type ,p_attrib_name => pi_attrib_name ,pi_value => pi_value ,po_value => l_value ,po_meaning => l_meaning ,pi_validate_domain_dates => FALSE ,pi_bind_variable_value => pi_bind_variable_value );
    --
    IF g_validate_flex_inv_func_rtn IN (c_meaning,c_both) THEN
      --
      IF l_meaning IS NOT NULL THEN
        --
        IF g_validate_flex_inv_func_rtn = c_meaning THEN
          l_value                      := l_meaning;
        ELSE
...
BEGIN
  --
  RETURN Nm3get.get_ita (pi_ita_inv_type => pi_inv_type ,pi_ita_attrib_name => pi_attrib_name ,pi_raise_not_found => FALSE );
  --
...
FUNCTION get_attrib_value(
    p_ne_id       IN NM_INV_ITEMS.iit_ne_id%TYPE ,
    p_attrib_name IN NM_INV_TYPE_ATTRIBS.ita_attrib_name%TYPE )
  RETURN VARCHAR2
IS
  --
  CURSOR c1 (p_ne_id NM_INV_ITEMS.iit_ne_id%TYPE ,p_attrib_name NM_INV_TYPE_ATTRIBS.ita_attrib_name%TYPE )
  IS
    SELECT ita_format ,
      ita_format_mask
    FROM NM_INV_TYPE_ATTRIBS ,
...
BEGIN
  ...
  CLOSE c1;
  --
  IF l_format     = Nm3type.c_date AND INSTR(p_attrib_name,l_format,1,1) != 0 THEN
    l_sql_string := 'select to_char(' ||p_attrib_name ||', ' ||Nm3flx.string(NVL(l_mask,Sys_Context('NM3CORE','USER_DATE_MASK'))) ||') from nm_inv_items where iit_ne_id = :p_ne_id';
  ELSIF l_format  = Nm3type.c_number AND l_mask IS NOT NULL THEN
    l_sql_string := 'select to_char(' ||p_attrib_name ||', ' ||Nm3flx.string(l_mask) ||') from nm_inv_items where iit_ne_id = :p_ne_id';
  ELSE
    l_sql_string := 'select ' ||p_attrib_name ||' from nm_inv_items where iit_ne_id = :p_ne_id';
  END IF;
  --
...
BEGIN
  ...
  --
  IF l_format     = Nm3type.c_date AND INSTR(p_attrib_name,l_format,1,1) != 0 THEN
--    l_sql_string := 'select to_char(' ||p_attrib_name ||', ' ||Nm3flx.string(NVL(l_mask,Sys_Context('NM3CORE','USER_DATE_MASK'))) ||') from nm_inv_items_eff where parent_ne_id = :p_ne_id' ||' and :p_effective_date >= iit_start_date and :p_effective_date < nvl(iit_end_date,SYSDATE+1)';
    l_sql_string := 'select to_char(' ||p_attrib_name ||', ' ||Nm3flx.string(NVL(l_mask,Sys_Context('NM3CORE','USER_DATE_MASK'))) ||') from nm_inv_items_eff niie where niie.parent_ne_id = :p_ne_id' 
                  ||' and :p_effective_date between niie.iit_start_date and nvl(niie.iit_end_date,SYSDATE+1)'
                  ||' and niie.iit_start_date = (select max(niie2.iit_start_date) from nm_inv_items_eff niie2 where niie2.parent_ne_id = niie.parent_ne_id and :p_effective_date BETWEEN niie2.iit_start_date and nvl(niie2.iit_end_date,SYSDATE+1))'
                  ||' and nvl(date_created,SYSDATE+1) = (select max(nvl(niie2.date_created,SYSDATE+1)) from nm_inv_items_eff niie2 where niie2.parent_ne_id = niie.parent_ne_id and :p_effective_date BETWEEN niie2.iit_start_date and nvl(niie2.iit_end_date,SYSDATE+1))';

  ELSIF l_format  = Nm3type.c_number AND l_mask IS NOT NULL THEN
    l_sql_string := 'select to_char(' ||p_attrib_name ||', ' ||Nm3flx.string(l_mask) ||') from nm_inv_items_eff niie where niie.parent_ne_id = :p_ne_id' 
                  ||' and :p_effective_date between niie.iit_start_date and nvl(niie.iit_end_date,SYSDATE+1)'
                  ||' and niie.iit_start_date = (select max(niie2.iit_start_date) from nm_inv_items_eff niie2 where niie2.parent_ne_id = niie.parent_ne_id and :p_effective_date BETWEEN niie2.iit_start_date and nvl(niie2.iit_end_date,SYSDATE+1))'
                  ||' and nvl(date_created,SYSDATE+1) = (select max(nvl(niie2.date_created,SYSDATE+1))  from nm_inv_items_eff niie2 where niie2.parent_ne_id = niie.parent_ne_id and :p_effective_date BETWEEN niie2.iit_start_date and nvl(niie2.iit_end_date,SYSDATE+1))';
  ELSE
    l_sql_string := 'select ' ||p_attrib_name ||' from nm_inv_items_eff niie where niie.parent_ne_id = :p_ne_id' 
                  ||' and :p_effective_date between niie.iit_start_date and nvl(niie.iit_end_date,SYSDATE+1)'
                  ||' and niie.iit_start_date = (select max(niie2.iit_start_date) from nm_inv_items_eff niie2 where niie2.parent_ne_id = niie.parent_ne_id and :p_effective_date BETWEEN niie2.iit_start_date and nvl(niie2.iit_end_date,SYSDATE+1))'
                  ||' and nvl(date_created,SYSDATE+1) = (select max(nvl(niie2.date_created,SYSDATE+1)) from nm_inv_items_eff niie2 where niie2.parent_ne_id = niie.parent_ne_id and :p_effective_date BETWEEN niie2.iit_start_date and nvl(niie2.iit_end_date,SYSDATE+1))';
  END IF;
  --
  EXECUTE IMMEDIATE l_sql_string INTO l_retval USING p_ne_id,  p_effective_date, p_effective_date,  p_effective_date;
  --
  RETURN l_retval;
EXCEPTION
WHEN OTHERS THEN
  NM_DEBUG.DEBUG('Exception raised in nm3inv.get_attrib_value: '||SUBSTR(sqlerrm(),1,200));
  RAISE;
  --
END get_attrib_value;
--
-----------------------------------------------------------------------------
--
FUNCTION get_all_attrib_values(
    p_ne_id IN NM_INV_ITEMS.iit_ne_id%TYPE)
  RETURN attrib_table
IS
  --
...
FUNCTION get_all_attrib_values(
    ...
    p_inv_type       IN NM_INV_ITEMS.iit_inv_type%TYPE ,
    p_effective_date IN DATE )
  RETURN nm3inv.attrib_table
IS
 --
  -- Select attributes in force on the Effective Date passed in.
  -- Use table NM_INV_TYPE_ATTRIBS_ALL instead of view NM_INV_TYPE_ATTRIBS
  -- as that already has date restrictions in place using SYS_CONTEXT.
  --
  CURSOR c1
  IS
    SELECT ita_attrib_name
    ...
    AND   nvl(p_effective_date, ita_start_date) >= ita_start_date and nvl(p_effective_date, ita_start_date) < nvl(ita_end_date, SYSDATE+1)
    ORDER BY ita_disp_seq_no;
  --
  l_attrib_name NM_INV_TYPE_ATTRIBS.ita_attrib_name%TYPE;
  l_attrib_list nm3inv.attrib_table;
  l_attrib_count NUMBER := 0;
  --
BEGIN
  --
  ...
  WHEN c1%NOTFOUND;
    l_attrib_list(l_attrib_count).attrib_name  := l_attrib_name;
    l_attrib_list(l_attrib_count).attrib_value := get_attrib_value(p_ne_id, l_attrib_list(l_attrib_count).attrib_name, p_effective_date);
  END LOOP;
  CLOSE c1;
  --
  RETURN l_attrib_list;
...
BEGIN
  --
  RETURN Nm3get.get_ita (pi_ita_inv_type => pi_inv_type ,pi_ita_attrib_name => pi_attrib_name ,pi_raise_not_found => FALSE ).ita_scrn_text;
  --
END get_attrib_scrn_text;
--
-----------------------------------------------------------------------------
--
FUNCTION get_attrib_domain(
    pi_inv_type    IN NM_INV_TYPE_ATTRIBS.ita_inv_type%TYPE ,
    pi_attrib_name IN NM_INV_TYPE_ATTRIBS.ita_attrib_name%TYPE )
...
BEGIN
  --
  RETURN Nm3get.get_ita (pi_ita_inv_type => pi_inv_type ,pi_ita_attrib_name => pi_attrib_name ,pi_raise_not_found => FALSE ).ita_id_domain;
  --
END get_attrib_domain;
--
-----------------------------------------------------------------------------
--
FUNCTION get_attrib_format(
    pi_inv_type    IN NM_INV_TYPE_ATTRIBS.ita_inv_type%TYPE ,
    pi_attrib_name IN NM_INV_TYPE_ATTRIBS.ita_attrib_name%TYPE )
...
BEGIN
  --
  RETURN Nm3get.get_ita (pi_ita_inv_type => pi_inv_type ,pi_ita_attrib_name => pi_attrib_name ,pi_raise_not_found => FALSE ).ita_format;
  --
END get_attrib_format;
--
-----------------------------------------------------------------------------
--
FUNCTION attrib_queryable(
    pi_inv_type    IN NM_INV_TYPE_ATTRIBS.ita_inv_type%TYPE ,
    pi_attrib_name IN NM_INV_TYPE_ATTRIBS.ita_attrib_name%TYPE )
  RETURN BOOLEAN
IS
  CURSOR c1(p_inv_type IN NM_INV_TYPE_ATTRIBS.ita_inv_type%TYPE ,p_attrib_name IN NM_INV_TYPE_ATTRIBS.ita_attrib_name%TYPE)
  IS
    SELECT ita_queryable
    FROM NM_INV_TYPE_ATTRIBS
    WHERE ita_inv_type  = p_inv_type
    AND ita_attrib_name = p_attrib_name;
...
BEGIN
  --
  RETURN (Nm3get.get_ita (pi_ita_inv_type => pi_inv_type ,pi_ita_attrib_name => pi_attrib_name ,pi_raise_not_found => FALSE ).ita_queryable = 'Y' );
  --
END attrib_queryable;
--
-----------------------------------------------------------------------------
--
FUNCTION get_top_item_id(
    p_item_id IN NM_INV_ITEM_GROUPINGS.iig_item_id%TYPE)
  RETURN NM_INV_ITEM_GROUPINGS.iig_top_id%TYPE
IS
  CURSOR c1
  IS
    SELECT iig_top_id FROM NM_INV_ITEM_GROUPINGS WHERE iig_item_id = p_item_id;
  CURSOR c2
  IS
    SELECT iig_top_id FROM NM_INV_ITEM_GROUPINGS WHERE iig_top_id = p_item_id;
...
BEGIN
  --
  RETURN Nm3get.get_nit (pi_nit_inv_type => pi_inv_type ,pi_not_found_sqlcode => -20001 );
  --
END get_inv_type;
--
----------------------------------------------------------------------------------------------
--
FUNCTION get_inv_type_table_name(
    p_nit_inv_type NM_INV_TYPES.nit_inv_type%TYPE)
  RETURN VARCHAR2
IS
  --
BEGIN
...
----------------------------------------------------------------------------------------------
--
FUNCTION inv_type_has_child(
    pi_nit_inv_type NM_INV_TYPES.nit_inv_type%TYPE )
  RETURN BOOLEAN
IS
  CURSOR c1
  IS
    ...
    WHERE itg_parent_inv_type = pi_nit_inv_type;
  l_temp PLS_INTEGER;
  l_retval BOOLEAN := FALSE;
BEGIN
  Nm_Debug.proc_start(p_package_name => g_package_name ,p_procedure_name => 'inv_type_has_child');
  OPEN c1;
  ...
  END IF;
  CLOSE c1;
  Nm_Debug.proc_end(p_package_name => g_package_name ,p_procedure_name => 'inv_type_has_child');
  RETURN l_retval;
...
FUNCTION get_inv_item(
    pi_ne_id NM_INV_ITEMS.iit_ne_id%TYPE )
  RETURN NM_INV_ITEMS%ROWTYPE
IS
BEGIN
  --
  RETURN Nm3get.get_iit (pi_iit_ne_id => pi_ne_id ,pi_raise_not_found => FALSE );
  --
END get_inv_item;
--
----------------------------------------------------------------------------------------------
--
PROCEDURE insert_nm_inv_items(
    pi_inv_rec NM_INV_ITEMS%ROWTYPE )
IS
...
----------------------------------------------------------------------------------------------
--
FUNCTION get_nm_inv_nw(
    pi_inv_type IN NM_INV_NW.nin_nit_inv_code%TYPE ,
    pi_nw_type  IN NM_INV_NW.nin_nw_type%TYPE )
  RETURN NM_INV_NW%ROWTYPE
IS
BEGIN
  --
  RETURN Nm3get.get_nin (pi_nin_nit_inv_code => pi_inv_type ,pi_nin_nw_type => pi_nw_type ,pi_not_found_sqlcode => -20001 );
  --
END get_nm_inv_nw;
--
----------------------------------------------------------------------------------------------
...
IS
  --
  CURSOR cs_mand (c_inv_type NM_INV_TYPES.nit_inv_type%TYPE)
  IS
    SELECT *
    FROM NM_INV_TYPE_ATTRIBS
...
BEGIN
  ...
  END LOOP;
  --
  IF l_been_in_loop THEN
    l_string := l_string||CHR(10)||'    THEN' ||CHR(10)||'      RAISE_APPLICATION_ERROR(-20750,'||CHR(39)||'Not all mandatory attribs populated'||CHR(39)||');' ||CHR(10)||'   END IF;' ||CHR(10)||'END;';
...
BEGIN
  --
  RETURN Nm3get.get_ita (pi_ita_inv_type => pi_inv_type ,pi_ita_attrib_name => pi_attrib_name ,pi_raise_not_found => FALSE ).ita_inv_type IS NOT NULL;
  --
END attrib_in_use;
--
----------------------------------------------------------------------------------------------
--
FUNCTION attrib_in_use_char(
    pi_inv_type    IN NM_INV_TYPE_ATTRIBS.ita_inv_type%TYPE ,
    pi_attrib_name IN NM_INV_TYPE_ATTRIBS.ita_attrib_name%TYPE )
  RETURN VARCHAR2
IS
BEGIN
  RETURN Nm3flx.boolean_to_char(attrib_in_use(pi_inv_type => pi_inv_type ,pi_attrib_name => pi_attrib_name));
END attrib_in_use_char;
--
----------------------------------------------------------------------------------------------
--
FUNCTION get_inv_admin_type(
    pi_inv_type NM_INV_TYPES.nit_inv_type%TYPE )
  RETURN VARCHAR2
IS
BEGIN
  RETURN Nm3get.get_nit (pi_nit_inv_type => pi_inv_type ,pi_raise_not_found => FALSE ).nit_admin_type;
END get_inv_admin_type;
--
----------------------------------------------------------------------------------------------
--
FUNCTION inv_location_is_mandatory(
    pi_inv_type IN NM_INV_TYPES.nit_inv_type%TYPE )
  RETURN BOOLEAN
IS
  CURSOR is_loc_mand(p_inv_type IN NM_INV_TYPES.nit_inv_type%TYPE)
  IS
    SELECT 1
    FROM NM_INV_NW_ALL
  ...
  l_retval BOOLEAN;
BEGIN
  Nm_Debug.proc_start(p_package_name => g_package_name ,p_procedure_name => 'inv_location_is_mandatory');
  OPEN is_loc_mand(p_inv_type => pi_inv_type);
  FETCH is_loc_mand INTO l_dummy;
  l_retval := is_loc_mand%FOUND;
  CLOSE is_loc_mand;
  Nm_Debug.proc_end(p_package_name => g_package_name ,p_procedure_name => 'inv_location_is_mandatory');
  RETURN l_retval;
END inv_location_is_mandatory;
--
----------------------------------------------------------------------------------------------
--
FUNCTION mand_loc_inv_not_located(
...
BEGIN
  --
  Nm_Debug.proc_start(p_package_name => g_package_name ,p_procedure_name => 'mand_loc_inv_not_located');
  Nm_Debug.proc_end(p_package_name => g_package_name ,p_procedure_name => 'mand_loc_inv_not_located');
  RETURN inv_location_is_mandatory(pi_inv_type => get_inv_type(p_ne_id => pi_ne_id)) AND NOT(Nm3ausec.do_locations_exist(p_ne_id => pi_ne_id));
  --
END mand_loc_inv_not_located;
--
----------------------------------------------------------------------------------------------
--
FUNCTION get_const_code
  RETURN VARCHAR2
IS
BEGIN
  RETURN c_code;
...
----------------------------------------------------------------------------------------------
--
PROCEDURE set_val_flx_inv_func_ret_type(
    p_val VARCHAR2)
IS
BEGIN
...
----------------------------------------------------------------------------------------------
--
FUNCTION get_inv_item_all(
    pi_ne_id NM_INV_ITEMS.iit_ne_id%TYPE )
  RETURN NM_INV_ITEMS%ROWTYPE
IS
BEGIN
  --
  RETURN Nm3get.get_iit_all (pi_iit_ne_id => pi_ne_id ,pi_raise_not_found => FALSE );
  --
END get_inv_item_all;
--
----------------------------------------------------------------------------------------------
--
FUNCTION get_inv_type_all(
    p_ne_id IN NM_INV_ITEMS.iit_ne_id%TYPE )
  RETURN NM_INV_ITEMS.iit_inv_type%TYPE
IS
  --
...
IS
  CURSOR c_itg(p_inv_type IN NM_INV_TYPES.nit_inv_type%TYPE)
  IS
    SELECT * FROM NM_INV_TYPE_GROUPINGS itg WHERE itg.itg_inv_type = p_inv_type;
  l_retval NM_INV_TYPE_GROUPINGS%ROWTYPE;
BEGIN
  OPEN c_itg(p_inv_type => pi_inv_type);
  FETCH c_itg INTO l_retval;
  IF c_itg%NOTFOUND THEN
    CLOSE c_itg;
...
BEGIN
  --
  RETURN Nm3get.get_ita (pi_ita_inv_type => pi_inv_type ,pi_ita_view_col_name => pi_view_col_name ,pi_not_found_sqlcode => -20001 );
  --
...
BEGIN
  --
  RETURN Nm3get.get_ita (pi_ita_inv_type => pi_inv_type ,pi_ita_attrib_name => pi_attrib_name );
  --
END get_ita_by_attrib_name;
--
----------------------------------------------------------------------------------------------
--
FUNCTION get_inv_mode_by_role(
    pi_inv_type IN NM_INV_TYPE_ATTRIBS.ita_inv_type%TYPE ,
    pi_username IN user_users.username%TYPE )
  RETURN NM_INV_TYPE_ROLES.itr_mode%TYPE
IS
  --
  CURSOR cs_mode (c_inv_type NM_INV_TYPE_ATTRIBS.ita_inv_type%TYPE ,c_username user_users.username%TYPE )
  IS
    SELECT itr_mode
...
IS
  ...
  l_rec_nit NM_INV_TYPES%ROWTYPE;
  --
  CURSOR cs_all_xsp (c_inv_type VARCHAR2)
  IS
...
BEGIN
  ...
  Nm_Debug.proc_start(g_package_name,'validate_rec_iit');
  --
  IF NOT g_checked_for_xav_proc THEN
    g_xav_procedure_exists := Nm3ddl.does_object_exist (p_object_name => Nm3inv_Xattr_Gen.c_xattr_validation_proc_name ,p_object_type => 'PROCEDURE' );
    g_checked_for_xav_proc := TRUE;
  END IF;
  --
  IF g_last_inv_type_dyn_val IS NULL OR g_last_inv_type_dyn_val != g_rec_iit.iit_inv_type THEN
    --
    ...
    IF l_rec_nit.nit_x_sect_allow_flag = 'Y' THEN
      ...
      --
      IF l_tab_xsp.COUNT = 0 THEN
        Hig.raise_ner (pi_appl => 'NET' ,pi_id => 196 ,pi_sqlcode => -20400 );
      END IF;
      --
    ...
    LOOP
      ...
      -- Log 702388:Linesh:11-Mar-09:Start
      IF cs_rec.ita_format = Nm3type.c_number THEN
        -- AE
        -- Task 0108730 / 0108765
        -- Round the value before continuing
        append (' ');
        --
        IF cs_rec.ita_dec_places IS NOT NULL THEN
          append ('   '||g_package_name||'.g_rec_iit.'||cs_rec.ita_attrib_name ||' := ROUND('||g_package_name||'.g_rec_iit.'||cs_rec.ita_attrib_name||','||cs_rec.ita_dec_places||');');
        END IF;
        --
        append (' ');
        -- AE
      ...
      --
      IF NOT l_mandatory AND cs_rec.ita_attrib_name = 'IIT_PRIMARY_KEY' THEN
        append (' IF '||g_package_name||'.g_rec_iit.'||cs_rec.ita_attrib_name||' IS NULL THEN');
        append ('   '||g_package_name||'.g_rec_iit.'||cs_rec.ita_attrib_name||' := '||g_package_name||'.g_rec_iit.iit_ne_id;');
        append (' END IF;');
      END IF;
      --
      IF cs_rec.ita_id_domain IS NOT NULL THEN
        IF NOT l_mandatory THEN
        ...
        --
      ELSIF cs_rec.ita_query IS NOT NULL THEN -- Validate against query
        append ('   -- ita_query');
        append('DECLARE');
        ...
        append('END;');
      END IF;
      IF cs_rec.ita_format = Nm3type.c_varchar THEN
        ...
        -------------------------------------------------------
        --
        IF cs_rec.ita_id_domain IS NULL THEN
          append (' ');
          append (' '||g_package_name||'.g_rec_iit.'||cs_rec.ita_attrib_name||' := ' ||'nm3inv.format_with_ita_case ( pi_asset_type   => '||nm3flx.string(cs_rec.ita_inv_type)|| ' , pi_attrib_name  => '||nm3flx.string(cs_rec.ita_attrib_name)|| ' , pi_value        => '||g_package_name||'.g_rec_iit.'||cs_rec.ita_attrib_name||');');
          append (' ');
        END IF;
      ELSIF cs_rec.ita_format = Nm3type.c_date AND INSTR(cs_rec.ita_attrib_name,cs_rec.ita_format,1,1) = 0 THEN
        --
        ...
        append (' END IF;');
        --
      ELSIF cs_rec.ita_format = Nm3type.c_number THEN
        nm_debug.debug_on;
        ...
        --
        IF cs_rec.ita_max IS NOT NULL AND cs_rec.ita_min IS NOT NULL THEN
          append (' IF '||g_package_name||'.g_rec_iit.'||cs_rec.ita_attrib_name||' IS NOT NULL');
    ...
    append (' END IF;');
    IF g_xav_procedure_exists THEN
      append ('--');
...
--
PROCEDURE ins_nit(
    p_rec_nit NM_INV_TYPES%ROWTYPE)
IS
...
--
PROCEDURE ins_ita(
    p_rec_ita NM_INV_TYPE_ATTRIBS%ROWTYPE)
IS
...
BEGIN
  ...
  --
  IF l_rec_nit.nit_x_sect_allow_flag = 'Y' AND pi_xsp IS NULL THEN
  ...
  ELSIF l_rec_nit.nit_x_sect_allow_flag = 'N' AND pi_xsp IS NOT NULL THEN
    Nm3type.g_exception_code           := -20779;
    Nm3type.g_exception_msg            := 'XSP must not be specified when XSP not allowed on inv_type';
    RAISE Nm3type.g_exception;
  ELSIF l_rec_nit.nit_table_name IS NOT NULL AND pi_xsp IS NOT NULL THEN
  ...
  --
  IF l_rec_ita.ita_format     = Nm3type.c_varchar AND g_datatype IN (Nm3type.c_date, Nm3type.c_number) THEN
    Nm3type.g_exception_code :=                                   -20781;
    Nm3type.g_exception_msg  := 'Cannot call this function for VARCHAR2 fields';
    RAISE Nm3type.g_exception;
  END IF;
  --
  IF l_rec_ita.ita_format IN (Nm3type.c_date,Nm3type.c_number) THEN
    l_before_format              := 'TO_CHAR(';
    IF l_rec_ita.ita_format_mask IS NOT NULL THEN
      l_format_mask              := l_rec_ita.ita_format_mask;
    ELSIF l_rec_ita.ita_format    = Nm3type.c_date THEN
    ...
    ELSE
      l_after_format := ')';
    END IF;
    IF g_datatype   != Nm3type.c_date THEN
      l_format_mask := NULL;
    ELSE
  ...
  IF (NVL(hig.get_sysopt('ATTRVALUEV'),'N') = 'N') THEN
    ...
    IF l_rec_nit.nit_table_name            IS NULL THEN
      l_sql                                := l_sql ||CHR(10)||' FROM  nm_inv_items' ||CHR(10)||'      ,nm_members' ||CHR(10)||'WHERE  nm_ne_id_of = :ne_id' ||CHR(10)||' AND   nm_type     = '||Nm3flx.string('I') ||CHR(10)||' AND   nm_obj_type = :inv_type' ||CHR(10)||' AND   :mp BETWEEN nm_begin_mp AND nm_end_mp' ||CHR(10)||' AND   nm_ne_id_in = iit_ne_id';
      IF l_rec_nit.nit_x_sect_allow_flag    = 'Y' THEN
        l_sql                              := l_sql ||CHR(10)||' AND   iit_x_sect = :xsp';
      ELSE
        l_sql := l_sql ||CHR(10)||' AND   :xsp IS NULL';
      END IF;
    ELSE -- This is a foreign table
      l_sql := l_sql ||CHR(10)||' FROM  '||l_rec_nit.nit_table_name ||CHR(10)||'WHERE  '||l_rec_nit.nit_lr_ne_column_name||' = :ne_id' ||CHR(10)||' AND   :inv_type IS NOT NULL' ||CHR(10)||' AND   :mp BETWEEN '||l_rec_nit.nit_lr_st_chain||' AND '||l_rec_nit.nit_lr_end_chain ||CHR(10)||' AND   :xsp IS NULL';
    END IF;
    OPEN l_cur FOR l_sql USING pi_ne_id,
    pi_inv_type,
    pi_mp,
    pi_xsp;
  ...
  ELSE
    ...
    IF l_rec_nit.nit_table_name         IS NULL THEN
      l_sql                             := l_sql ||chr(10)||',  row_number() over (partition by case when :mp in (nm_begin_mp, nm_end_mp) then ''Y'' end order by nm_begin_mp desc) row_num' ||chr(10)||',  case when :mp in (nm_begin_mp, nm_end_mp) then ''Y'' end is_touching' ||CHR(10)||' FROM  nm_inv_items' ||CHR(10)||'      ,nm_members' ||CHR(10)||'WHERE  nm_ne_id_of = :ne_id' ||CHR(10)||' AND   nm_type     = '||Nm3flx.string('I') ||CHR(10)||' AND   nm_obj_type = :inv_type' ||CHR(10)||' AND   :mp BETWEEN nm_begin_mp AND nm_end_mp' ||CHR(10)||' AND   nm_ne_id_in = iit_ne_id';
      IF l_rec_nit.nit_x_sect_allow_flag = 'Y' THEN
        l_sql                           := l_sql ||CHR(10)||' AND   iit_x_sect = :xsp';
      ELSE
        l_sql := l_sql ||CHR(10)||' AND   :xsp IS NULL';
      END IF;
    ELSE -- This is a foreign table
      l_sql := l_sql ||chr(10)||',  row_number() over (partition by case when :mp in ('||l_rec_nit.nit_lr_st_chain||', ' ||l_rec_nit.nit_lr_end_chain||') then ''Y'' end order by '||l_rec_nit.nit_lr_st_chain||' desc) row_num' ||chr(10)||',  case when :mp in ('||l_rec_nit.nit_lr_st_chain||', '||l_rec_nit.nit_lr_end_chain||') then ''Y'' end is_touching' ||CHR(10)||' FROM  '||l_rec_nit.nit_table_name ||CHR(10)||'WHERE  '||l_rec_nit.nit_lr_ne_column_name||' = :ne_id' ||CHR(10)||' AND   :inv_type IS NOT NULL' ||CHR(10)||' AND   :mp BETWEEN '||l_rec_nit.nit_lr_st_chain||' AND '||l_rec_nit.nit_lr_end_chain ||CHR(10)||' AND   :xsp IS NULL';
    END IF;
    ...
    l_sql := 'select q.value' ||chr(10)||'from (' ||chr(10)||l_sql ||chr(10)||') q' ||chr(10)||'where not (q.row_num = 2 and q.is_touching = ''Y'')';
    OPEN l_cur FOR l_sql USING pi_mp,
    pi_mp,
    pi_ne_id,
    pi_inv_type,
    pi_mp,
...
IS
BEGIN
  RETURN get_attribute_value (pi_ne_id => pi_ne_id ,pi_mp => pi_mp ,pi_inv_type => pi_inv_type ,pi_xsp => NULL ,pi_attrib => pi_attrib );
END get_attribute_value;
--
----------------------------------------------------------------------------------------------
--
...
BEGIN
  ...
  RETURN TO_NUMBER(get_attribute_value (pi_ne_id => pi_ne_id ,pi_mp => pi_mp ,pi_inv_type => pi_inv_type ,pi_xsp => pi_xsp ,pi_attrib => pi_attrib ) );
...
IS
BEGIN
  RETURN get_attribute_value_number (pi_ne_id => pi_ne_id ,pi_mp => pi_mp ,pi_inv_type => pi_inv_type ,pi_xsp => NULL ,pi_attrib => pi_attrib );
END get_attribute_value_number;
--
----------------------------------------------------------------------------------------------
--
...
BEGIN
  ...
  RETURN TO_DATE(get_attribute_value (pi_ne_id => pi_ne_id ,pi_mp => pi_mp ,pi_inv_type => pi_inv_type ,pi_xsp => pi_xsp ,pi_attrib => pi_attrib ) ,g_last_date_mask );
...
IS
BEGIN
  RETURN get_attribute_value_date (pi_ne_id => pi_ne_id ,pi_mp => pi_mp ,pi_inv_type => pi_inv_type ,pi_xsp => NULL ,pi_attrib => pi_attrib );
END get_attribute_value_date;
--
----------------------------------------------------------------------------------------------
--
...
IS
BEGIN
  RETURN get_attribute_value (pi_ne_id => pi_ne_id ,pi_mp => pi_mp ,pi_inv_type => pi_inv_type ,pi_xsp => pi_xsp ,pi_attrib => get_ita_by_view_col(pi_inv_type, pi_attrib).ita_attrib_name );
...
IS
BEGIN
  RETURN get_attribute_value (pi_ne_id => pi_ne_id ,pi_mp => pi_mp ,pi_inv_type => pi_inv_type ,pi_attrib => get_ita_by_view_col(pi_inv_type, pi_attrib).ita_attrib_name );
END get_attrib_value_view_col;
--
----------------------------------------------------------------------------------------------
--
...
IS
BEGIN
  RETURN get_attribute_value_number (pi_ne_id => pi_ne_id ,pi_mp => pi_mp ,pi_inv_type => pi_inv_type ,pi_xsp => pi_xsp ,pi_attrib => get_ita_by_view_col(pi_inv_type, pi_attrib).ita_attrib_name );
...
IS
BEGIN
  RETURN get_attribute_value_number (pi_ne_id => pi_ne_id ,pi_mp => pi_mp ,pi_inv_type => pi_inv_type ,pi_attrib => get_ita_by_view_col(pi_inv_type, pi_attrib).ita_attrib_name );
END get_attrib_value_view_col_num;
--
----------------------------------------------------------------------------------------------
--
...
IS
BEGIN
  RETURN get_attribute_value_date (pi_ne_id => pi_ne_id ,pi_mp => pi_mp ,pi_inv_type => pi_inv_type ,pi_xsp => pi_xsp ,pi_attrib => get_ita_by_view_col(pi_inv_type, pi_attrib).ita_attrib_name );
...
IS
BEGIN
  RETURN get_attribute_value_date (pi_ne_id => pi_ne_id ,pi_mp => pi_mp ,pi_inv_type => pi_inv_type ,pi_attrib => get_ita_by_view_col(pi_inv_type, pi_attrib).ita_attrib_name );
END get_attrib_value_view_col_date;
--
----------------------------------------------------------------------------------------------
--
PROCEDURE delete_inv_items(
    pi_inv_type        IN NM_INV_TYPES.nit_inv_type%TYPE ,
...
IS
  ...
  IS
    SELECT itg.itg_inv_type child_type
    FROM NM_INV_TYPE_GROUPINGS_ALL itg
    WHERE itg.itg_parent_inv_type = p_parent_type
    AND itg_mandatory             = 'N';
  ...
  l_i PLS_INTEGER;
BEGIN
  Nm_Debug.proc_start(p_package_name => g_package_name ,p_procedure_name => 'delete_inv_items');
  ...
  IF pi_cascade AND pi_where IS NOT NULL THEN
    RAISE e_cascade_and_where;
  END IF;
  ------------------------------
  --check hierarchy for children
  ------------------------------
  IF inv_type_has_child(pi_nit_inv_type => pi_inv_type) THEN
    l_children_exist_qry   := 'SELECT' || c_nl || '  1' || c_nl || 'FROM' || c_nl || '  dual' || c_nl || 'WHERE EXISTS(SELECT' || c_nl || '               1' || c_nl || '             FROM' || c_nl || '               nm_inv_items_all          ' || pi_inv_table_alias || ',' || c_nl || '               nm_inv_item_groupings_all iig' || c_nl || '             WHERE' || c_nl || '               ' || pi_inv_table_alias || '.iit_inv_type = ' || Nm3flx.string(pi_string => pi_inv_type) || c_nl || '             AND' || c_nl || '               iig.iig_parent_id = ' || pi_inv_table_alias || '.iit_ne_id';
    IF pi_where            IS NOT NULL THEN
      l_children_exist_qry := l_children_exist_qry || c_nl || 'AND' || c_nl || '  ' || pi_where;
    END IF;
    ...
    IF l_children AND NOT(pi_cascade) THEN
  ...
  -------------------------------------
  l_lock_qry   := 'SELECT' || c_nl || '  ' || pi_inv_table_alias || '.rowid,' || c_nl || '  iig.rowid,' || c_nl || '  nm.rowid' || c_nl || 'FROM' || c_nl || '  nm_inv_items_all          ' || pi_inv_table_alias || ',' || c_nl || '  nm_inv_item_groupings_all iig,' || c_nl || '  nm_members_all            nm' || c_nl || 'WHERE' || c_nl || '  ' || pi_inv_table_alias || '.iit_inv_type = ' || Nm3flx.string(pi_string => pi_inv_type) || c_nl || 'AND' || c_nl || '  nm.nm_ne_id_in (+) = ' || pi_inv_table_alias || '.iit_ne_id' || c_nl || 'AND' || c_nl || '  iig.iig_item_id (+) = ' || pi_inv_table_alias || '.iit_ne_id';
  IF pi_where  IS NOT NULL THEN
    l_lock_qry := l_lock_qry || c_nl || 'AND' || c_nl || '  ' || pi_where;
  END IF;
  l_lock_qry := l_lock_qry || c_nl || 'FOR UPDATE NOWAIT';
  ...
  LOOP
    l_i := l_i + 1;
    FETCH cs_lock
    INTO l_iit_rowid_tab(l_i),
      l_iig_rowid_tab(l_i),
      l_nm_rowid_tab(l_i);
    EXIT
  WHEN cs_lock%NOTFOUND;
  END LOOP;
  CLOSE cs_lock;
  -----------------------------
  ...
  IF l_children THEN
    ...
    LOOP
      delete_inv_items(pi_inv_type => l_rec.child_type ,pi_cascade => pi_cascade);
    END LOOP;
  END IF;
  -------------------------------
  ...
  DELETE NM_MEMBERS_ALL WHERE ROWID = l_nm_rowid_tab(l_i);
  ------------------------------------------
  --delete nm_inv_item_groupings_all records
  ------------------------------------------
  FORALL l_i IN 1..l_iig_rowid_tab.COUNT
  DELETE NM_INV_ITEM_GROUPINGS_ALL WHERE ROWID = l_iig_rowid_tab(l_i);
  -----------------------------------------
  --lastly, delete nm_inv_items_all records
  -----------------------------------------
  FORALL l_i IN 1..l_iit_rowid_tab.COUNT
  DELETE NM_INV_ITEMS_ALL WHERE ROWID = l_iit_rowid_tab(l_i);
  Nm_Debug.proc_end(p_package_name => g_package_name ,p_procedure_name => 'delete_inv_items');
EXCEPTION
WHEN e_cascade_and_where THEN
  Hig.raise_ner(pi_appl => 'NET' ,pi_id => 198 ,pi_sqlcode => -20711 ,pi_supplementary_info => NULL);
WHEN e_children_and_no_cascade THEN
  Hig.raise_ner(pi_appl => 'NET' ,pi_id => 197 ,pi_sqlcode => -20710 ,pi_supplementary_info => '(' || pi_inv_type || ')');
END delete_inv_items;
--
----------------------------------------------------------------------------------------------
--
PROCEDURE set_inv_warning_msg(
    p_msg VARCHAR2)
IS
BEGIN
...
----------------------------------------------------------------------------------------------
--
FUNCTION nm_inv_domain_exists(
    p_id_domain NM_INV_DOMAINS_ALL.id_domain%TYPE)
  RETURN BOOLEAN
IS
  --
  CURSOR cs_id (c_id NM_INV_DOMAINS_ALL.id_domain%TYPE)
  IS
    SELECT 1 FROM NM_INV_DOMAINS_ALL WHERE id_domain = c_id;
  --
  l_dummy PLS_INTEGER;
  l_retval BOOLEAN;
  --
...
BEGIN
  ...
  --
  IF NOT Hig.domain_exists (p_hdo_domain) THEN
    Hig.raise_ner (pi_appl => Nm3type.c_hig ,pi_id => 67 ,pi_supplementary_info => 'HIG_DOMAINS : '||p_hdo_domain );
  ELSIF nm_inv_domain_exists (l_id_domain) THEN
    Hig.raise_ner (pi_appl => Nm3type.c_hig ,pi_id => 64 ,pi_supplementary_info => 'NM_INV_DOMAINS : '||l_id_domain );
  END IF;
  --
  INSERT
  INTO NM_INV_DOMAINS
...
IS
  --
  CURSOR cs_hdo (c_domain HIG_DOMAINS.hdo_domain%TYPE)
  IS
    SELECT * FROM HIG_DOMAINS WHERE hdo_domain = c_domain;
  --
  CURSOR cs_id (c_domain NM_INV_DOMAINS_ALL.id_domain%TYPE)
  IS
    SELECT * FROM NM_INV_DOMAINS_ALL WHERE id_domain = c_domain;
  --
...
BEGIN
  ...
  IF l_id_found THEN
    IF UPPER(l_rec_id.id_title) = UPPER(l_rec_hdo.hdo_title) THEN -- These domains are the same
      l_create                 := FALSE;
    ELSE
      l_create    := TRUE;
  ...
  --
  IF l_create THEN
    duplicate_hdo_as_id (p_hdo_domain => p_hdo_domain ,p_id_start_date => p_id_start_date ,p_id_domain => l_id_domain );
  END IF;
  --
  RETURN l_id_domain;
  --
...
----------------------------------------------------------------------------------------------
--
PROCEDURE check_inv_format_against_col(
    p_column_name VARCHAR2 ,
    p_format      VARCHAR2 ,
    p_inv_type    VARCHAR2 DEFAULT NULL )
IS
  --
  CURSOR cs_atc (p_table VARCHAR2 ,p_column VARCHAR2 )
  IS
    SELECT data_type
...
BEGIN
  ...
  BEGIN
    --
    IF p_column_name IS NULL OR p_format IS NULL THEN
    ...
    --
    OPEN cs_atc (l_table_name ,p_column_name );
    FETCH cs_atc INTO l_data_type;
    IF cs_atc%NOTFOUND THEN
      CLOSE cs_atc;
      RAISE l_col_not_found;
    END IF;
    CLOSE cs_atc;
    --
    IF p_format = Nm3type.c_varchar AND l_data_type != Nm3type.c_varchar THEN
      RAISE l_invalid_format;
    ELSIF p_format = Nm3type.c_date AND l_data_type = Nm3type.c_number THEN
      RAISE l_invalid_format;
    ELSIF p_format = Nm3type.c_number AND l_data_type = Nm3type.c_date THEN
      RAISE l_invalid_format;
    END IF;
    --
  EXCEPTION
  WHEN l_col_not_found THEN
    Hig.raise_ner(pi_appl => Nm3type.c_net ,pi_id => 223 ,pi_supplementary_info => l_table_name||'.'||p_column_name );
  WHEN l_data_not_passed THEN
    NULL;
  WHEN l_invalid_format THEN
    Hig.raise_ner(pi_appl => Nm3type.c_net ,pi_id => 222 ,pi_supplementary_info => p_format||':'||l_data_type );
  END;
  --
...
----------------------------------------------------------------------------------------------
--
PROCEDURE instantiate_flex_cols
IS
  --
...
----------------------------------------------------------------------------------------------
--
FUNCTION is_column_allowable_for_flex(
...
BEGIN
  ...
  WHILE i IS NOT NULL
  LOOP
    IF g_tab_flex_col(i) = p_column_name THEN
...
--
FUNCTION is_column_allowable_for_flex(
    p_column_id user_tab_columns.column_id%TYPE)
  RETURN VARCHAR2
IS
  --
  l_retval VARCHAR2(5);
...
BEGIN
  --
  IF g_tab_flex_col.EXISTS(p_column_id) THEN
    l_retval := Nm3type.c_true;
  ELSE
...
----------------------------------------------------------------------------------------------
--
FUNCTION get_tab_ita(
    p_inv_type nm_inv_types.nit_inv_type%TYPE)
  RETURN tab_nita
IS
  --
  CURSOR cs_ita (c_inv_type nm_inv_types.nit_inv_type%TYPE)
  IS
    SELECT *
    FROM nm_inv_type_attribs
    WHERE ita_inv_type = c_inv_type
    ORDER BY ita_disp_seq_no,
      ita_mandatory_yn DESC,
      ita_attrib_name;
...
----------------------------------------------------------------------------------------------
--
FUNCTION get_tab_ita_exclusive(
    p_inv_type NM_INV_TYPES.nit_inv_type%TYPE)
  RETURN tab_nita
IS
  --
...
BEGIN
  ...
  LOOP
    IF l_tab_ita(i).ita_exclusive             = 'Y' THEN
...
----------------------------------------------------------------------------------------------
--
FUNCTION get_tab_ita_displayed(
    p_inv_type NM_INV_TYPES.nit_inv_type%TYPE)
  RETURN tab_nita
IS
  --
...
----------------------------------------------------------------------------------------------
--
FUNCTION inv_type_is_hierarchical(
...
IS
  ...
  l_retval BOOLEAN;
BEGIN
  Nm_Debug.proc_start(p_package_name => g_package_name ,p_procedure_name => 'inv_type_is_hierarchical');
  ...
  BEGIN
    ...
    l_child    := TRUE;
  EXCEPTION
  WHEN e_no_relation THEN
    l_child := FALSE;
  END;
  l_retval := l_nit_rec.nit_top = 'Y' OR (l_child AND (NOT(pi_ignore_derived) OR (pi_ignore_derived AND l_relation <> c_derived_relation)));
  Nm_Debug.proc_end(p_package_name => g_package_name ,p_procedure_name => 'inv_type_is_hierarchical');
  RETURN l_retval;
END inv_type_is_hierarchical;
--
----------------------------------------------------------------------------------------------
--
FUNCTION does_child_exist_diff_au_type(
    pi_inv_type IN NM_INV_TYPES.nit_inv_type%TYPE)
  RETURN BOOLEAN
IS
  --
  CURSOR cs_hier (c_start_inv_type NM_INV_TYPES.nit_inv_type%TYPE)
  IS
    SELECT itg_inv_type ,
      LEVEL
    FROM NM_INV_TYPE_GROUPINGS
...
----------------------------------------------------------------------------------------------
--
FUNCTION is_xsp_valid_on_inv_type(
    pi_inv_type IN NM_INV_TYPES.nit_inv_type%TYPE ,
...
IS
  ...
  --
  CURSOR cs_xsr (c_inv_type NM_INV_TYPES.nit_inv_type%TYPE ,c_xsp NM_XSP.nwx_x_sect%TYPE )
  IS
    SELECT 1
    FROM XSP_RESTRAINTS
    WHERE xsr_ity_inv_code = c_inv_type
...
----------------------------------------------------------------------------------------------
--
PROCEDURE check_ita_id_domain(
    pi_ita_id_domain IN NM_INV_TYPE_ATTRIBS_ALL.ita_id_domain%TYPE ,
    pi_ita_format    IN NM_INV_TYPE_ATTRIBS_ALL.ita_format%TYPE)
IS
  l_rec_id NM_INV_DOMAINS_ALL%ROWTYPE;
BEGIN
  IF pi_ita_id_domain       IS NOT NULL THEN
    l_rec_id                := Nm3get.get_id_all (pi_id_domain => pi_ita_id_domain);
    IF (l_rec_id.id_datatype = Nm3type.c_varchar AND pi_ita_format != l_rec_id.id_datatype) OR (l_rec_id.id_datatype = Nm3type.c_number AND pi_ita_format = Nm3type.c_date) OR (l_rec_id.id_datatype = Nm3type.c_date AND pi_ita_format = Nm3type.c_number) THEN
      Hig.raise_ner (pi_appl => Nm3type.c_net ,pi_id => 309 ,pi_supplementary_info => l_rec_id.id_datatype||':'||pi_ita_format );
    END IF;
  END IF;
...
----------------------------------------------------------------------------------------------
--
PROCEDURE check_ita_query(
    pi_ita_query       IN NM_INV_TYPE_ATTRIBS_ALL.ita_query%TYPE ,
    pi_ita_inv_type    IN NM_INV_TYPE_ATTRIBS_ALL.ita_inv_type%TYPE ,
    pi_ita_attrib_name IN NM_INV_TYPE_ATTRIBS_ALL.ita_attrib_name%TYPE)
IS
BEGIN
  IF pi_ita_query IS NOT NULL THEN
    ...
    --                          ,pi_id   => 422);
    --END IF;
    Nm3flx.validate_ita_query(pi_query => pi_ita_query ,pi_ita_inv_type => pi_ita_inv_type ,pi_ita_attrib_name => pi_ita_attrib_name);
  END IF;
END check_ita_query;
--
----------------------------------------------------------------------------------------------
--
PROCEDURE check_ita_domain_query_excl(
    pi_ita_id_domain IN NM_INV_TYPE_ATTRIBS_ALL.ita_id_domain%TYPE ,
    pi_ita_query     IN NM_INV_TYPE_ATTRIBS_ALL.ita_query%TYPE)
IS
BEGIN
  IF pi_ita_id_domain IS NOT NULL AND pi_ita_query IS NOT NULL THEN
    Hig.raise_ner (pi_appl => Nm3type.c_net ,pi_id => 396 -- Specify either a domain or a query - not both
    );
  END IF;
...
IS
BEGIN
  IF NVL(pi_ita_displayed,'N')  = 'Y' THEN
    IF NVL(pi_ita_disp_width,0) = 0 THEN
      Hig.raise_ner (pi_appl => Nm3type.c_net ,pi_id => 50 ,pi_supplementary_info => 'Displayed width not set when Displayed is switched on' );
    END IF ;
  END IF;
...
----------------------------------------------------------------------------------------------
--
PROCEDURE check_ita_view_col_name(
    pi_ita_view_col_name IN nm_inv_type_attribs_all.ita_view_col_name%TYPE)
IS
BEGIN
  --nm_debug.debug_on ;
  --nm_debug.debug('pi_ita_view_col_name ' || pi_ita_view_col_name);
  --nm_debug.debug_off ;
  IF nm3flx.string_contains_special_chars(pi_string => pi_ita_view_col_name) THEN
    hig.raise_ner( pi_appl => 'NET' , pi_id => 30 , pi_supplementary_info => pi_ita_view_col_name);
  ...
  --
  IF nm3flx.is_reserved_word(p_name => pi_ita_view_col_name) THEN
    hig.raise_ner(pi_appl => 'NET' , pi_id => 455 , pi_supplementary_info => pi_ita_view_col_name);
...
BEGIN
  --
  IF g_tab_ita.COUNT > 0 THEN
    --
    FOR i IN 1..g_tab_ita.COUNT
    LOOP
      --
      -- Check Domain
      check_ita_id_domain ( pi_ita_id_domain => g_tab_ita(i).ita_id_domain , pi_ita_format => g_tab_ita(i).ita_format);
      --
      check_ita_query ( pi_ita_query => g_tab_ita(i).ita_query , pi_ita_inv_type => g_tab_ita(i).ita_inv_type , pi_ita_attrib_name => g_tab_ita(i).ita_attrib_name);
      --
      check_ita_domain_query_excl ( pi_ita_id_domain => g_tab_ita(i).ita_id_domain , pi_ita_query => g_tab_ita(i).ita_query);
      --
      check_disp_width ( pi_ita_disp_width => g_tab_ita(i).ita_disp_width , pi_ita_displayed => g_tab_ita(i).ita_displayed , pi_ita_fld_length => g_tab_ita(i).ita_fld_length) ;
      --
    ...
    check_ngqs ( pi_ita_inv => g_tab_ita );
    --
  END IF;
  --
...
PROCEDURE create_ft_asset_from_table(
    ...
    pi_admin_type   IN NM_INV_TYPES.nit_admin_type%TYPE DEFAULT Nm3get.get_nau(
      pi_nau_admin_unit => Nm3get.get_hus(
        pi_hus_username=>Sys_Context(
          'NM3_SECURITY_CTX',
          'USERNAME')).hus_admin_unit).nau_admin_type ,
...
TYPE tab_utc
IS
  TABLE OF user_tab_columns%ROWTYPE INDEX BY BINARY_INTEGER;
  -- nm_inv_types_all
  l_tab_utc tab_utc;
  l_rec_nit NM_INV_TYPES%ROWTYPE;
...
BEGIN
  ...
  --
  IF NOT does_column_exist (pi_pk_column) THEN
  ...
  --
  IF LENGTH(pi_asset_type) > 4 THEN
  ...
  LOOP --nm_inv_type_attribs
    ...
    --
    IF l_tab_utc(i).data_type    = nm3type.c_number THEN
      l_rec_ntc.ita_disp_width  := 8;
    ELSIF l_tab_utc(i).data_type = nm3type.c_varchar THEN
      l_rec_ntc.ita_disp_width  := 20;
    ELSIF l_tab_utc(i).data_type = nm3type.c_date THEN
...
--
PROCEDURE check_asset_location(
    pi_nit_inv_type IN nm_inv_types_all.nit_inv_type%TYPE ,
...
BEGIN
  IF is_cont_asset_type(pi_nit_inv_type => pi_nit_inv_type) THEN
    IF pi_begin_mp = pi_end_mp THEN
      hig.raise_ner(pi_appl => 'NET' ,pi_id => 86); -- Continuous asset cannot have start and end points that are the same.
    END IF;
    IF pi_begin_mp > pi_end_mp THEN
      hig.raise_ner(pi_appl => 'NET' ,pi_id => 80); -- Continuous asset cannot have start and end points that are the same.
    END IF;
  END IF;
...
--
PROCEDURE bypass_inv_items_all_trgs(
    pi_mode IN BOOLEAN )
IS
  --
BEGIN
...
FUNCTION get_ita_case(
    pi_asset_type  IN nm_inv_type_attribs.ita_inv_type%TYPE ,
    pi_attrib_name IN nm_inv_type_attribs.ita_attrib_name%TYPE )
  RETURN nm_inv_type_attribs.ita_case%TYPE
IS
  CURSOR get_case ( cp_asset_type IN nm_inv_type_attribs.ita_inv_type%TYPE , cp_attrib_name IN nm_inv_type_attribs.ita_attrib_name%TYPE )
  IS
    SELECT DECODE (ita_case,'MIXED',NULL,ita_case)
    FROM nm_inv_type_attribs
...
BEGIN
  ...
  -- Ensure formatting of Case works with single quotes (chr(39))
  IF pi_asset_type IS NOT NULL AND pi_attrib_name IS NOT NULL AND pi_value IS NOT NULL THEN
    l_asset_type   := nm3get.get_ita ( pi_ita_inv_type => pi_asset_type , pi_ita_attrib_name => pi_attrib_name , pi_raise_not_found => FALSE );
    --
    IF l_asset_type.ita_inv_type IS NOT NULL
      --
      -- Task 0110838
      -- Reintroduce case based domain attributes
      AND ( l_asset_type.ita_id_domain IS NULL OR ( NOT (b_ignore_domains) AND l_asset_type.ita_id_domain IS NOT NULL ) )
      --
      AND l_asset_type.ita_case != 'MIXED' THEN
      l_sql                     := ' SELECT REPLACE('||get_ita_case (pi_asset_type, pi_attrib_name) ||'(REPLACE (:l_value,:l_quote,:l_dummy)),:l_dummy,:l_quote)'|| ' FROM DUAL';
      --
      EXECUTE IMMEDIATE l_sql INTO retval USING IN pi_value,
                                                IN l_quote,
                                                IN l_dummy,
                                                IN l_dummy,
                                                IN l_quote;
