...
--   PVCS Identifiers :-
--
--       pvcsid           : $Header:   //new_vm_latest/archives/nm3/admin/pck/nm3pkh.sql-arc   2.53   Mar 26 2020 15:36:52   Chris.Baugh  $
--       Module Name      : $Workfile:   nm3pkh.sql  $
--       Date into PVCS   : $Date:   Mar 26 2020 15:36:52  $
--       Date fetched Out : $Modtime:   Mar 26 2020 15:35:28  $
...
--
SET TERM ON 
PROMPT nm3user_admin.pkh
SET TERM OFF
SET DEFINE ON 
SELECT '&exor_base'||'nm3'||'&terminator'||'admin'||'&terminator'||'pck'||'&terminator'||'nm3user_admin.pkh' run_file
FROM dual 
/ 
start '&run_file'
--
...
--
SET TERM ON 
PROMPT hig_router_params_utils.pkh
SET TERM OFF
SET DEFINE ON 
...
--
SET TERM ON 
PROMPT nm3sdo_wms.pkh
SET TERM OFF
SET DEFINE ON 
SELECT '&exor_base'||'nm3'||'&terminator'||'admin'||'&terminator'||'pck'||'&terminator'||'nm3sdo_wms.pkh' run_file
FROM dual 
/ 
start '&run_file'
--
...
--
SET TERM ON 
PROMPT nm3_msv_util.pkh
SET TERM OFF
SET DEFINE ON 
SELECT '&exor_base'||'nm3'||'&terminator'||'admin'||'&terminator'||'pck'||'&terminator'||'nm3_msv_util.pkh' run_file
FROM dual 
/ 
start '&run_file'
--
...
--
SET TERM ON 
PROMPT hig_sso_api.pkh
SET TERM OFF
SET DEFINE ON 
SELECT '&exor_base'||'nm3'||'&terminator'||'admin'||'&terminator'||'pck'||'&terminator'||'hig_sso_api.pkh' run_file
FROM dual 
/ 
start '&run_file'
--
...
--
SET TERM ON 
PROMPT hig_relationship_api.pkh
SET TERM OFF
SET DEFINE ON 
SELECT '&exor_base'||'nm3'||'&terminator'||'admin'||'&terminator'||'pck'||'&terminator'||'hig_relationship_api.pkh' run_file
FROM dual 
/ 
start '&run_file'
--
...
--
SET TERM ON 
PROMPT nm_inv_sdo_aggr.pkh
SET TERM OFF
SET DEFINE ON 
SELECT '&exor_base'||'nm3'||'&terminator'||'admin'||'&terminator'||'pck'||'&terminator'||'nm_inv_sdo_aggr.pkh' run_file
FROM dual 
/ 
start '&run_file'
--
...
SET TERM OFF
SET DEFINE ON
SELECT '&exor_base'||'nm3'||'&terminator'||'admin'||'&terminator'||'pck'||'&terminator'||'nm3_java_utils.pkh' run_file 
FROM dual
/
start '&run_file'
