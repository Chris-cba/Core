...
AS
   ...
   ---   PVCS Identifiers :-
   --
   --       sccsid           : $Header:   //new_vm_latest/archives/nm3/admin/pck/nm3sdo.pkb-arc   2.99   Jan 01 2019 11:06:38   Rob.Coupe  $
   --       Module Name      : $Workfile:   nm3sdo.pkb  $
   --       Date into PVCS   : $Date:   Jan 01 2019 11:06:38  $
   --       Date fetched Out : $Modtime:   Jan 01 2019 10:56:28  $
   --       PVCS Version     : $Revision:   2.99  $
   --       Based on
   ------------------------------------------------------------------
   ...
   g_body_sccsid    CONSTANT VARCHAR2 (2000) := '"$Revision:   2.99  $"';
   ...
   g_do_geom_validation      BOOLEAN
                                := NVL (
                                      hig.get_user_or_sys_opt ('SDOGEOMVAL'),
                                      'Y') = 'Y';
   ...
   --
   PROCEDURE add_segments_m (p_geom1     IN OUT NOCOPY MDSYS.sdo_geometry,
   ...
   */
   ...
   BEGIN
      ...
      THEN
         ...
         THEN
            l_lr :=
               NM3LRS.GET_DISTINCT_OFFSET (nm_lref (p_ne_id, p_measure), 'N');
   ...
   */
   ...
   BEGIN
      IF    NOT p_conn
         OR (   ABS (
                   p_geom1.sdo_ordinates (last_1 - 2) - p_geom2.sdo_ordinates (1)) >
                   p_diminfo (1).sdo_tolerance
             OR ABS (
                     p_geom1.sdo_ordinates (last_1 - 1)
                   - p_geom2.sdo_ordinates (2)) > p_diminfo (2).sdo_tolerance)
      THEN
         --   either intentionally not connected, or connected but no common ordinates
         --   so make multi-part
         ...
         LOOP
            ...
            THEN
               ...
               */
               ...
               IF     ABS (
                         retval.sdo_ordinates (ret_count) - retval.sdo_ordinates (ret_count - 3)) <=
         ...
         LOOP
            ...
            ELSE
               ...
               THEN
                  l_ord (l_ord.LAST) :=
                       p_measure
                     + (  (p_geom.sdo_ordinates (3 * (i - 1) + 3) - p_measure)
                        * l_shift);
               ELSE
      ...
      LOOP
         ...
         THEN
            ...
            THEN
               ...
               IF l_part = 'Y'
               THEN
                  IF Nm3sdo.is_clipped (l_start (i), l_end (i), l_length (i)) =
                        0
                  THEN
   ...
   IS
      ...
      */
      ...
      CURSOR get_routes (
         c_nt    IN nm_linear_types.nlt_nt_type%TYPE,
         c_gty   IN nm_linear_types.nlt_gty_type%TYPE)
      IS
         WITH route_data
              AS (  SELECT nm_ne_id_in ne_id,
                           nm_start_date start_date,
                           nm_end_date end_date
                      FROM nm_members_all, nm_elements_all
                     WHERE nm_obj_type = c_gty AND nm_ne_id_in = ne_id --and ne_id = 114223
                           AND ne_nt_type = c_nt AND ne_gty_group_type = c_gty
   ...
   */
            ...
            THEN
               ...
               l_geom :=
                  mdsys.sdo_geometry (
                     ...
                     NULL);
            ELSE
               IF is_clipped (irec.nm_begin_mp, irec.nm_end_mp, l_length) = 0
               THEN
                  l_geom :=
   ...
   BEGIN
      ...
      LOOP
         ...
         ELSE
            l_measure :=
                 l_measure
               + (p_geom.sdo_ordinates (ilm + 3) - p_geom.sdo_ordinates (ilm));
   ...
   IS
   BEGIN
      RETURN get_placement_aggr_mbr (p_layer,
   ...
   BEGIN
      ...
      ELSE
         l_x :=
            (l_mbr.sdo_ordinates (1) + l_mbr.sdo_ordinates (1 + l_dim)) / 2;
         l_y :=
            (l_mbr.sdo_ordinates (2) + l_mbr.sdo_ordinates (2 + l_dim)) / 2;

         l_m :=
            GREATEST (
               ABS (
                  l_mbr.sdo_ordinates (1 + l_dim) - l_mbr.sdo_ordinates (1)),
               ABS (
                  l_mbr.sdo_ordinates (2 + l_dim) - l_mbr.sdo_ordinates (2)));
   ...
   EXCEPTION
      WHEN NO_DATA_FOUND
      THEN
         raise_application_error (-20012, 'Theme or SDO metadata not found');
   END get_theme_srid;
   ...
   BEGIN
      ...
      THEN
         ...
         THEN
            ...
            l_end :=
               l_old_shape.sdo_ordinates (l_old_shape.sdo_ordinates.LAST);
         ...
         BEGIN
            ...
            LOOP
               ...
               DECLARE
                  l_job_id   NUMBER;
               BEGIN
                  nm3extent.create_temp_ne (pi_source_id   => i.nsm_ne_id,
                                            pi_source      => 'ROUTE',
   ...
   BEGIN
      ...
      EXECUTE IMMEDIATE
            'update mdsys.sdo_geom_metadata_table set sdo_srid = '
         || l_srid
   ...
   IS
      ...
      FUNCTION is_ad_link_type (p_inv_type IN nm_inv_types.nit_inv_type%TYPE)
         RETURN VARCHAR2
      IS
   ...
   AS
      ...
      FUNCTION format_where_clause (pi_where IN NM_THEMES_ALL.nth_where%TYPE)
   ...
   BEGIN
      ...
      THEN
         cur_string :=
            cur_string || ',' || g_nth.nth_feature_fk_column || ' number(38)';
      END IF;
      ...
      THEN
         ...
         THEN
            cur_string :=
               cur_string || ' AND ' || format_where_clause (g_nth.nth_where);
         END IF;
                           ...
                           SQRT (
                                POWER (
                                   (  l_geom.sdo_ordinates (ic)
                                    - l_geom.sdo_ordinates (ic - l_dim)),
                                   2)
                              + POWER (
                                   (  l_geom.sdo_ordinates (ic + 1)
                                    - l_geom.sdo_ordinates (ic + 1 - l_dim)),
                                   2));
                        ...
                        l_g_diff :=
                           SQRT (
                              ...
                              + POWER (
                                   ...
                                   2)
                              + POWER (
                                   (  l_geom.sdo_ordinates (ic + 2)
                                    - l_geom.sdo_ordinates (ic + 2 - l_dim)),
                                   2));
                     --          nm_debug.debug('Tol estimate = '||to_char(l_g_diff));
   ...
   BEGIN
      ...
      BEGIN
         SELECT hus_username, sdo_table_name, sdo_column_name
          ...
          WHERE sdo_owner = hus_username AND sdo_table_name = p_object_name;
      ...
      LOOP
         ...
         LOOP
            ...
            THEN
               ...
               IF compare_pt (
                     Nm3sdo.get_2d_pt (SDO_LRS.geom_segment_end_pt (retval)),
                     Nm3sdo.get_2d_pt (
         ...
         LOOP
            ...
            lpt1 :=
               mdsys.sdo_geometry (
                  2001,
                  NULL,
                  mdsys.sdo_point_type (p_geom.sdo_ordinates (p1 - ld),
      ...
      THEN
         ...
         LOOP
            ...
            THEN
               lth_id :=
                  TO_NUMBER (
                     SUBSTR (l_theme_txt, 1, INSTR (l_theme_txt, ',') - 1));
         ...
         THEN
            retval :=
               nm_theme_array (nm_theme_array_type (nm_theme_entry (lth_id)));
         ELSE
            retval := retval.add_theme (lth_id);
         END IF;
   ...
   BEGIN
      ...
      LOOP
         ...
         lpk :=
            Nm3sdo.get_nearest_theme_to_xy (lta.nta_theme_array (i).nthe_id,
                                            p_x,
                                            p_y);
   ...
   BEGIN
      RETURN get_nearest_nw_to_xy (
                p_x,
                p_y,
                nm_theme_array (nm_theme_array_type (nm_theme_entry (NULL))));
   END;
              ...
              AS (  SELECT row_num,
                      ...
                      FROM (  SELECT table_name,
                                     ...
                                     OVER (PARTITION BY table_name
                                        ...
                                        row_num,
                                     sdo_lb * (1 - NVL (c_pct, 0) / 100) sdo_lb,
                                     sdo_ub * (1 + NVL (c_pct, 0) / 100) sdo_ub,
                                     DECODE (
                                        dim_row,
                                        3, NVL (p_m_tol,
   ...
   BEGIN
      ...
      ELSE
         ...
         curstr :=
            ...
            || p_nth.nth_feature_pk_column
            || ' and   f.'
            || NVL (p_nth.nth_feature_fk_column, p_nth.nth_feature_pk_column)
            || ' =  t.'
            || p_nth.nth_pk_column
            || ' group by :new_theme, a.ntd_pk_id, a.ntd_fk_id,  SUBSTR(t.'
            || p_nth.nth_label_column
   ...
   FUNCTION validate_geometry (p_geom     IN MDSYS.sdo_geometry,
                               p_nth_id   IN nm_themes_all.nth_theme_id%TYPE,
                               p_tol      IN NUMBER)
      RETURN VARCHAR2
   IS
   ...
   BEGIN
      ...
      ELSE
         --
         retval :=
            SUBSTR (SDO_GEOM.validate_geometry_with_context (p_geom, p_tol),
                    1,
                    100);
      --
      END IF;
   ...
   IS
      ...
      CURSOR c1 (
         c_theme_id   IN NM_THEMES_ALL.nth_theme_id%TYPE)
      IS
         SELECT wktext
            ...
            THEN
               ...
               THEN
                  ...
                  BEGIN
                     ...
                     THEN
                        curstr_inv_ft :=
                           REPLACE (curstr_inv_ft, '<end_date>', 'NULL');
                     ELSE
                        curstr_inv_ft :=
                           REPLACE (curstr_inv_ft,
                                    '<end_date>',
                  ...
                  THEN
                     ...
                     curstr_partial_inv :=
                        REPLACE (curstr_partial_inv,
                                 ...
                                 p_obj_type);
                     curstr_partial_inv :=
                        REPLACE (curstr_partial_inv, '<nm_type>', p_type);
                     ...
                     THEN
                        curstr_partial_inv :=
                           REPLACE (
                              ...
                              'sdo_lrs.convert_to_std_geom(sdo_lrs.clip_geom_segment');
                        curstr_partial_inv :=
                           REPLACE (curstr_partial_inv, '<clipbr>', ')');
                     ELSE
                        curstr_partial_inv :=
                           REPLACE (curstr_partial_inv,
                                    '<clipstr>',
                                    'sdo_lrs.clip_geom_segment');
                        curstr_partial_inv :=
                           REPLACE (curstr_partial_inv, '<clipbr>', NULL);
                     END IF;
                  ...
                  ELSE
                     ...
                     curstr_ASD_whole :=
                        REPLACE (curstr_ASD_whole,
                                 ...
                                 p_obj_type);
                     curstr_ASD_whole :=
                        REPLACE (curstr_ASD_whole, '<nm_type>', p_type);
            ...
            ELSE
               ...
               curstr_partial_gty :=
                  REPLACE (curstr_partial_gty,
                           ...
                           irec.nth_feature_pk_column);
               curstr_partial_gty :=
                  REPLACE (curstr_partial_gty, '<nm_obj_type>', p_obj_type);
               curstr_partial_gty :=
               ...
               THEN
                  curstr_partial_gty :=
                     curstr_partial_gty || lf || ' and nm_end_date is null';
               END IF;
                     ...
                     ELSE
                        ...
                        BEGIN
                           ...
                           LOOP
                              ...
                              INSERT
                                INTO NM3SDM_DYN_SEG_EX (ndse_job_id,
                                                        ndse_ner_id,
                                                        ndse_ne_id_in,
                                                        ndse_ne_id_of,
                                                        ndse_shape_length,
