CREATE OR REPLACE PACKAGE BODY nm3sdo
AS
    --
    -----------------------------------------------------------------------------
    --
    ...
    g_clip_type               VARCHAR2 (30)
        := NVL (Hig.get_sysopt ('SDOCLIPTYP'), 'SDO');
    g_only_live               VARCHAR2 (1)
                                  := NVL (hig.get_sysopt ('SDOONLYLIV'), 'N');
    ...
    --
    g_error_to_ignore         ptr_vc_array
        := ptr_vc_array (ptr_vc_array_type (ptr_vc (1, 13011) -- ORA-13011: value is out of range
                                                             -- , ptr_vc( 2, 13356 ) -- ORA-13356: adjacent points are redundant
                                                             ));

    g_do_geom_validation      BOOLEAN
        := NVL (hig.get_user_or_sys_opt ('SDOGEOMVAL'), 'Y') = 'Y';
    ...
    FUNCTION Sdo_Clip (p_geom      IN MDSYS.sdo_geometry,
                       p_st        IN NUMBER,
                       p_end       IN NUMBER,
                       p_diminfo   IN MDSYS.sdo_dim_array)
        RETURN MDSYS.sdo_geometry;

    FUNCTION get_base_themes (p_nt IN ptr_vc_array)
        RETURN ptr_array;

    FUNCTION get_base_themes (p_theme_id IN NM_THEMES_ALL.nth_theme_id%TYPE)
        RETURN nm_theme_array;

    FUNCTION get_base_nt (p_ne_id IN NUMBER)
        RETURN ptr_vc_array;

    FUNCTION get_base_nt (p_nlt_id IN NUMBER)
        RETURN ptr_vc_array;

    FUNCTION Get_Ne_Shapes (p_id IN ptr_array, p_th_id IN INTEGER)
        RETURN nm_geom_array;

    FUNCTION make_tha_from_ptr (p_ptr IN ptr_array)
        RETURN nm_theme_array;

    FUNCTION get_distinct_ptr (p_ptr IN ptr_array)
        RETURN ptr_array;

    FUNCTION get_idx_from_value (p_ptr IN ptr_array, p_value IN INTEGER)
        RETURN INTEGER;

    FUNCTION get_idx_from_id (p_ptr IN ptr_array, p_id IN INTEGER)
        RETURN INTEGER;

    FUNCTION get_id (p_ptr IN ptr_array, p_value IN INTEGER)
        RETURN INTEGER;

    FUNCTION get_idx_from_id (p_ptr IN ptr_num_array, p_id IN INTEGER)
        RETURN INTEGER;

    FUNCTION get_idx (p_ga IN nm_geom_array, p_id IN INTEGER)
        RETURN INTEGER;


    PROCEDURE add_dyn_seg_exception (
        p_ner            IN INTEGER,
        p_job_id         IN INTEGER,
    ...
    PROCEDURE ins_usgm (p_table     IN VARCHAR2,
                        p_column    IN VARCHAR2,
                        p_diminfo   IN MDSYS.sdo_dim_array,
                        p_srid      IN NUMBER);

    FUNCTION join_ntl_array (p_nth   IN NM_THEMES_ALL%ROWTYPE,
                             p_ntl   IN nm_theme_list)
        RETURN nm_theme_list;

    FUNCTION Get_Parts (p_shape IN MDSYS.sdo_geometry)
        RETURN nm_geom_array;

    FUNCTION compare_pt (p_geom1      MDSYS.sdo_geometry,
                         p_geom2      MDSYS.sdo_geometry,
                         tol       IN NUMBER)
        RETURN VARCHAR2;

    FUNCTION local_join_ptr_array (p_pa      IN ptr_array,
                                   p_table   IN VARCHAR2,
                                   p_key     IN VARCHAR2)
        RETURN ptr_array;

    --
    PROCEDURE add_segments_m (
    ...
    --
    FUNCTION get_list (p_theme_txt IN VARCHAR2)
        RETURN nm_theme_array;
    ...
    --
    FUNCTION set_route_name (p_route_name IN nm_elements.ne_unique%TYPE)
    ...
    FUNCTION set_theme (p_layer IN NM_THEMES_ALL.nth_theme_id%TYPE)
        RETURN BOOLEAN
    IS
    BEGIN
        IF NVL (g_nth.nth_theme_id, -99) = p_layer
        THEN
            RETURN FALSE;
        ELSE
            RETURN TRUE;
    ...
    -----------------------------------------------------------------------------
    --
    FUNCTION get_layer_table (p_layer IN NUMBER)
        RETURN VARCHAR2
    IS
    BEGIN
    ...
    -----------------------------------------------------------------------------
    --
    FUNCTION get_layer_shape_column (p_layer IN NUMBER)
        RETURN VARCHAR2
    IS
    BEGIN
    ...
    FUNCTION get_layer_ne_column (p_layer IN NUMBER)
        RETURN VARCHAR2
    IS
    BEGIN
    ...
    IS
        ...
        CURSOR get_srids (c_themes IN nm_theme_array)
        IS
            SELECT nth_theme_id, sdo_srid
              FROM mdsys.sdo_geom_metadata_table,
                   NM_THEMES_ALL,
                   TABLE (c_themes.nta_theme_array)  t
    ...
    BEGIN
        ...
        IF l_dummy.ia (1) IS NULL
        THEN
            Hig.raise_ner (pi_appl      => Nm3type.c_hig,
                           pi_id        => 197,
                           pi_sqlcode   => -20001);
        ELSE
            FOR i IN 1 .. l_srid.na.LAST
            LOOP
                IF i = 1
                THEN
                    p_srid := l_srid.na (1);
                ELSIF NVL (l_srid.na (i), -999) != NVL (p_srid, -999)
                THEN
                    Hig.raise_ner (pi_appl      => Nm3type.c_hig,
                                   pi_id        => 280,
                                   pi_sqlcode   => -20001);
        ...
        IF p_themes.nta_theme_array (1).nthe_id IS NOT NULL
        THEN
            l_dim := Get_Theme_Diminfo (p_themes.nta_theme_array (1).nthe_id);
        ELSE
            raise_application_error (-20099,
                                     'No base theme metadata available');
        END IF;

        SELECT sdo_aggr_mbr (
                   Convert_Dim_Array_To_Mbr ( --    sdo_lrs.convert_to_std_dim_array(
        ...
        SELECT MIN (
                   LEAST (
                       Get_Dim_Element (
                           1,
                           Nm3sdo.Get_Theme_Diminfo (nth_theme_id)).sdo_tolerance,
                       Get_Dim_Element (
                           2,
                           Nm3sdo.Get_Theme_Diminfo (nth_theme_id)).sdo_tolerance))    l1
          INTO l_tol
          FROM NM_THEMES_ALL, TABLE (p_themes.nta_theme_array) l
         WHERE l.nthe_id = nth_theme_id;
        ...
        THEN
            ...
            p_diminfo :=
                mdsys.sdo_dim_array (mdsys.sdo_dim_element (
                                         'X',
                                         l_mbr.sdo_ordinates (1),
                                         l_mbr.sdo_ordinates (3),
                                         l_tol),
                                     mdsys.sdo_dim_element (
                                         'Y',
                                         l_mbr.sdo_ordinates (2),
                                         l_mbr.sdo_ordinates (4),
                                         l_tol),
                                     mdsys.sdo_dim_element (
                                         'M',
                                         0,
                                         Nm3type.c_big_number,
                                         l_m_tol));
        ELSE
            p_diminfo :=
                mdsys.sdo_dim_array (mdsys.sdo_dim_element (
    ...
    PROCEDURE Copy_Ords (p_ords1   IN OUT NOCOPY MDSYS.sdo_ordinate_array,
                         p_ords2   IN            MDSYS.sdo_ordinate_array,
                         p_from    IN            INTEGER,
                         p_to      IN            INTEGER)
    IS
        l_last   INTEGER;
    ...
    BEGIN
        ...
        FOR i IN 1 .. last1 / 3 - 1
        LOOP
            k := (i - 1) * 3;

            IF NOT st_set AND p_st <= p_geom.sdo_ordinates (k + 6)
            THEN
                --   nm_debug.debug('Start K = '||to_char(k)||' value = '||to_char(p_geom.sdo_ordinates(k + 6)));

                l_geom :=
                    mdsys.sdo_geometry (3302,
                                        p_geom.sdo_srid,
                                        NULL,
                                        mdsys.sdo_elem_info_array (1, 2, 1),
                                        mdsys.sdo_ordinate_array (
                                            p_geom.sdo_ordinates (k + 1),
                                            p_geom.sdo_ordinates (k + 2),
                                            p_geom.sdo_ordinates (k + 3),
                                            p_geom.sdo_ordinates (k + 4),
                                            p_geom.sdo_ordinates (k + 5),
                                            p_geom.sdo_ordinates (k + 6)));
                ...
                */
                ...
                IF p_end <= p_geom.sdo_ordinates (k + 6)
                THEN
                    end_id := k + 3;
                ...
                st_set := TRUE;
                st_id := k + 4;
            ELSIF p_end <= p_geom.sdo_ordinates (k + 6)
            THEN
        ...
        THEN
            ...
            Copy_Ords (l_ords,
                       p_geom.sdo_ordinates,
                       st_id,
                       end_id);
        END IF;

        Copy_Ords (l_ords,
                   mdsys.sdo_ordinate_array (x_end, y_end, p_end),
                   1,
                   3);

        RETURN mdsys.sdo_geometry (3302,
                                   p_geom.sdo_srid,
                                   NULL,
                                   mdsys.sdo_elem_info_array (1, 2, 1),
                                   l_ords);
    ...
    FUNCTION Get_Datum_Theme (p_nt IN NM_TYPES.nt_type%TYPE)
        RETURN NM_THEMES_ALL.nth_theme_id%TYPE
    IS
        retval   NM_THEMES_ALL.nth_theme_id%TYPE;

        CURSOR c_nth (c_nt IN NM_TYPES.nt_type%TYPE)
        IS
            SELECT nth_theme_id
    ...
    --
    --
    FUNCTION set_route_id (p_route_id IN nm_elements.ne_id%TYPE)
    ...
    FUNCTION get_projection (p_layer   IN NUMBER,
                             p_ne_id   IN NUMBER,
                             p_x       IN NUMBER,
                             p_y       IN NUMBER)
        RETURN MDSYS.sdo_geometry
    ...
    */
    ...
    FUNCTION get_measure (p_layer   IN NUMBER,
                          p_ne_id   IN NUMBER,
                          p_x       IN NUMBER,
                          p_y       IN NUMBER)
        RETURN nm_lref
    ...
    BEGIN
        ...
        cur_string :=
            ...
            || ', :diminfo, '
            || 'mdsys.sdo_geometry( 2001, :srid, null, mdsys.sdo_elem_info_array( 1,1,1), '
            || 'mdsys.sdo_ordinate_array( :x, :y )) ), :diminfo ) '
            || ' from '
            || g_usgm.table_name
            || ' where '
            || g_nth.nth_feature_pk_column
    ...
    */
    ...
    BEGIN
        ...
        THEN
            IF NM3NET.IS_GTY_LINEAR (l_ne.ne_gty_group_type) = 'Y'
            THEN
                l_lr :=
                    NM3LRS.GET_DISTINCT_OFFSET (nm_lref (p_ne_id, p_measure),
                                                'N');

                IF l_lr.lr_ne_id IS NOT NULL
                THEN
                    RETURN get_xy_from_measure (l_lr.lr_ne_id,
                                                l_lr.lr_offset);
                ELSE
                    RETURN NULL;
                END IF;
        ...
        ELSE
            l_layer :=
                Get_Datum_Theme (Nm3get.get_ne_all (p_ne_id).ne_nt_type);

            IF l_layer IS NULL
            THEN
                RETURN NULL;
            ELSE
                RETURN nm3sdo.get_xy_from_measure (p_layer     => l_layer,
                                                   p_ne_id     => p_ne_id,
                                                   p_measure   => p_measure);
            END IF;
        END IF;
    ...
    FUNCTION get_xy_from_measure (p_layer     IN NUMBER,
                                  p_ne_id     IN NUMBER,
                                  p_measure   IN NUMBER)
        RETURN MDSYS.sdo_geometry
        DETERMINISTIC
    ...
    IS
    BEGIN
        RETURN get_xy_from_measure (p_layer,
                                    p_lref.lr_ne_id,
                                    p_lref.lr_offset);
    END;
    ...
    FUNCTION get_x_from_pt_geometry (p_geometry IN MDSYS.sdo_geometry)
        RETURN NUMBER
    IS
        --  l_geom mdsys.sdo_geometry;
        retval   NUMBER;
    BEGIN
        ...
        IF p_geometry.sdo_elem_info IS NULL
        THEN
    ...
    FUNCTION get_y_from_pt_geometry (p_geometry IN MDSYS.sdo_geometry)
        RETURN NUMBER
    IS
        --  l_geom mdsys.sdo_geometry;
        retval   NUMBER;
    ...
    */
    ...
    FUNCTION linear_interp (p_m1      IN NUMBER,
                            p_m2      IN NUMBER,
                            p_n1      IN NUMBER,
                            p_n2      IN NUMBER,
                            p_n3      IN NUMBER,
    ...
    -----------------------------------------------------------------------------
    --
    FUNCTION get_placement_geometry (p_pl IN nm_placement)
        RETURN MDSYS.sdo_geometry
    IS
        l_dummy   INTEGER;
    BEGIN
    ...
    FUNCTION get_placement_geometry (p_pl_array IN nm_placement_array)
        RETURN MDSYS.sdo_geometry
    IS
        l_elem_info   MDSYS.sdo_elem_info_array
                          := mdsys.sdo_elem_info_array (1, 2, 1);
        l_null_pt     MDSYS.sdo_point_type
    ...
    BEGIN
        ...
        FOR ip IN 1 .. p_pl_array.npa_placement_array.LAST
        LOOP
            l_layer :=
                Get_Datum_Theme (
                    Nm3get.get_ne_all (
                        p_pl_array.npa_placement_array (ip).pl_ne_id).ne_nt_type);

            IF set_theme (l_layer)
            THEN
                set_theme_metadata (l_layer);
            END IF;

            IF     p_pl_array.npa_placement_array (ip).pl_start = 0
               AND p_pl_array.npa_placement_array (ip).pl_end =
                   Nm3net.Get_Ne_Length (
                       p_pl_array.npa_placement_array (ip).pl_ne_id)
            THEN
                cur_string :=
                       'select '
                    || g_usgm.column_name
            ...
            ELSE
                cur_string :=
                    ...
                    || g_usgm.column_name
                    || ', :diminfo, '
                    || TO_CHAR (p_pl_array.npa_placement_array (ip).pl_start)
                    || ','
                    || TO_CHAR (p_pl_array.npa_placement_array (ip).pl_end)
                    || ')'
                    || ' from '
                    || g_usgm.table_name
                    || ' where '
            ...
            ELSE
                ...
                IF SDO_LRS.connected_geom_segments (retval,
                                                    g_usgm.diminfo,
                                                    l_geom,
                                                    g_usgm.diminfo) = 'TRUE'
                THEN
                    ...
                    retval :=
                        SDO_LRS.concatenate_geom_segments (retval,
                                                           g_usgm.diminfo,
                                                           l_geom,
                                                           g_usgm.diminfo);
                ELSE
    ...
    FUNCTION get_placement_geometry (p_layer IN NUMBER, p_pl IN nm_placement)
        RETURN MDSYS.sdo_geometry
    IS
        l_elem_info   MDSYS.sdo_elem_info_array
                          := mdsys.sdo_elem_info_array (1, 2, 1);
    ...
    BEGIN
        ...
        IF     p_pl.pl_start = 0
           AND p_pl.pl_end = Nm3net.Get_Ne_Length (p_pl.pl_ne_id)
        THEN
            cur_string :=
                   'select '
                || g_usgm.column_name
    ...
    FUNCTION add_segments (p_geom1   IN MDSYS.sdo_geometry,
                           p_geom2   IN MDSYS.sdo_geometry,
                           p_conn    IN BOOLEAN DEFAULT FALSE)
        RETURN MDSYS.sdo_geometry
    IS
    ...
    BEGIN
        IF    NOT p_conn
           OR (   p_geom1.sdo_ordinates (last_1 - 2) !=
                  p_geom2.sdo_ordinates (1)
               OR p_geom1.sdo_ordinates (last_1 - 1) !=
                  p_geom2.sdo_ordinates (2))
        THEN
            --   either intentionally not connected, or connected but no common ordinates
            --   so make multi-part

            retval.sdo_elem_info.EXTEND;
            retval.sdo_elem_info (retval.sdo_elem_info.LAST) :=
                retval.sdo_ordinates.LAST + 1;
        ...
        IF     p_conn
           AND p_geom1.sdo_ordinates (last_1 - 2) = p_geom2.sdo_ordinates (1)
           AND p_geom1.sdo_ordinates (last_1 - 1) = p_geom2.sdo_ordinates (2)
        THEN
            start_ic := 4;
        ELSE
            start_ic := 1;
        ...
        FOR ic IN start_ic .. p_geom2.sdo_ordinates.LAST
        LOOP
            retval.sdo_ordinates.EXTEND;

            IF MOD (ic, 3) = 0
            THEN
                retval.sdo_ordinates (retval.sdo_ordinates.LAST) :=
                    p_geom2.sdo_ordinates (ic) + l_meas;
            ELSE
                retval.sdo_ordinates (retval.sdo_ordinates.LAST) :=
                    p_geom2.sdo_ordinates (ic);
            END IF;
        END LOOP;
    ...
    PROCEDURE add_segments (p_geom1     IN OUT NOCOPY MDSYS.sdo_geometry,
                            p_geom2     IN            MDSYS.sdo_geometry,
                            p_diminfo   IN            MDSYS.sdo_dim_array,
                            p_conn      IN            BOOLEAN DEFAULT FALSE)
    IS
    ...
    BEGIN
        IF    NOT p_conn
           OR (   ABS (
                        p_geom1.sdo_ordinates (last_1 - 2)
                      - p_geom2.sdo_ordinates (1)) >
                  p_diminfo (1).sdo_tolerance
               OR ABS (
                        p_geom1.sdo_ordinates (last_1 - 1)
                      - p_geom2.sdo_ordinates (2)) >
                  p_diminfo (2).sdo_tolerance)
        THEN
            --   either intentionally not connected, or connected but no common ordinates
            --   so make multi-part
            ...
            p_geom1.sdo_elem_info (p_geom1.sdo_elem_info.LAST) :=
                p_geom1.sdo_ordinates.LAST + 1;
        ...
        LOOP
            ...
            IF MOD (ic, 3) = 0
            THEN
                p_geom1.sdo_ordinates (p_geom1.sdo_ordinates.LAST) :=
                    p_geom2.sdo_ordinates (ic) + l_meas;
            ELSE
                p_geom1.sdo_ordinates (p_geom1.sdo_ordinates.LAST) :=
                    p_geom2.sdo_ordinates (ic);
    ...
    FUNCTION get_table_diminfo (p_table    IN VARCHAR2,
                                p_column   IN VARCHAR2 DEFAULT NULL)
        RETURN MDSYS.sdo_dim_array
    IS
        retval   MDSYS.sdo_dim_array;

        CURSOR c1 (c_table IN VARCHAR2, c_column IN VARCHAR2)
        IS
            SELECT sdo_diminfo
              FROM mdsys.sdo_geom_metadata_table
             WHERE     sdo_table_name = c_table
                   AND sdo_owner =
                       SYS_CONTEXT ('NM3CORE', 'APPLICATION_OWNER')
                   AND NVL (c_column, 'A') =
                       DECODE (c_column, NULL, 'A', sdo_column_name);
    BEGIN
    ...
    FUNCTION get_layer_dimension (p_layer IN NM_THEMES_ALL.nth_theme_id%TYPE)
        RETURN INTEGER
    IS
        retval   INTEGER;
    BEGIN
    ...
    BEGIN
        ...
        IF p_geom.sdo_elem_info.LAST > 3
        THEN
            l_elem_count := 4;
        END IF;

        IF p_geom.sdo_ordinates.LAST > 3
        THEN
            ...
            LOOP
                ...
                THEN
                    ...
                    retval.sdo_ordinates (1) :=
                        ROUND (p_geom.sdo_ordinates (1), l_round1);
                ELSIF ic = 2
                THEN
                    retval.sdo_ordinates.EXTEND;
                    retval.sdo_ordinates (2) :=
                        ROUND (p_geom.sdo_ordinates (2), l_round2);
                ELSE
                    retval.sdo_ordinates.EXTEND;
                    retval.sdo_ordinates (3) :=
                        ROUND (p_geom.sdo_ordinates (3), l_round3);
                END IF;
            END LOOP;

            -- nm_debug.debug('4 onwards');

            FOR ic IN 4 .. p_geom.sdo_ordinates.LAST
            LOOP
                retval.sdo_ordinates.EXTEND;
                ...
                IF MOD (ic, 3) = 1
                THEN
                    retval.sdo_ordinates (ret_count) :=
                        ROUND (p_geom.sdo_ordinates (ic), l_round1);
                ELSIF MOD (ic, 3) = 2
                THEN
                    retval.sdo_ordinates (ret_count) :=
                        ROUND (p_geom.sdo_ordinates (ic), l_round2);
                ELSE
                    retval.sdo_ordinates (ret_count) :=
                        ROUND (p_geom.sdo_ordinates (ic), l_round3);
                END IF;
                ...
                THEN
                    ...
                    */
                    ...
                    IF     ABS (
                                 retval.sdo_ordinates (ret_count)
                               - retval.sdo_ordinates (ret_count - 3)) <=
                           g_usgm.diminfo (3).sdo_tolerance
                       AND ABS (
                                 retval.sdo_ordinates (ret_count - 1)
                               - retval.sdo_ordinates (ret_count - 4)) <=
                           g_usgm.diminfo (2).sdo_tolerance
                       AND ABS (
                                 retval.sdo_ordinates (ret_count - 2)
                               - retval.sdo_ordinates (ret_count - 5)) <=
                           g_usgm.diminfo (1).sdo_tolerance
                    THEN
                        --         redundant
    ...
    FUNCTION get_nearest_to_xy (p_layer     IN NUMBER,
                                p_pt_geom   IN MDSYS.sdo_geometry)
        RETURN nm_elements.ne_id%TYPE
    IS
    BEGIN
        RETURN get_nearest_to_xy (p_layer,
                                  p_pt_geom.sdo_point.X,
                                  p_pt_geom.sdo_point.Y);
    END;
    ...
    BEGIN
        l_pa :=
            Get_Batch_Of_Base_Nn (p_theme => p_layer, p_x => p_x, p_y => p_y);

        curstr :=
               'SELECT /*+cardinality( p '
            || TO_CHAR (l_pa.pa.LAST)
            || ')*/ nm_ne_id_of '
    ...
    FUNCTION get_nearest_theme_to_xy (p_theme   IN NUMBER,
                                      p_x       IN NUMBER,
                                      p_y       IN NUMBER)
        RETURN NUMBER
    IS
    ...
    BEGIN
        ...
        l_pt :=
            Get_Batch_Of_Base_Nn (p_theme => p_theme, p_x => p_x, p_y => p_y);

        IF l_pt.pa.LAST IS NULL
        THEN
            RAISE NO_DATA_FOUND;
        ELSE
    ...
    --------------------------------------------------------------------------------------------------------------------
    --
    FUNCTION get_projection_to_nearest (p_layer     IN NUMBER,
                                        p_pt_geom   IN MDSYS.sdo_geometry)
        RETURN nm_lref
    IS
    BEGIN
        RETURN get_projection_to_nearest (p_layer,
                                          p_pt_geom.sdo_point.X,
                                          p_pt_geom.sdo_point.Y);
    END;
    ...
    FUNCTION get_nearest_lref (p_theme   IN NM_THEMES_ALL.nth_theme_id%TYPE,
                               p_geom    IN MDSYS.sdo_geometry)
        RETURN nm_lref
    IS
    ...
    BEGIN
          ...
          SELECT nm_theme_entry (a.nth_theme_id)
            BULK COLLECT INTO l_base_themes.nta_theme_array
        ...
        IF l_base_themes.nta_theme_array.COUNT = 0
        THEN
            SELECT nm_theme_entry (a.nth_theme_id)
              BULK COLLECT INTO l_base_themes.nta_theme_array
             ...
             WHERE     nbth_theme_id = p_theme
                   ...
                   AND NVL (a.nth_base_table_theme, a.nth_theme_id) =
                       b.nth_theme_id
                   AND EXISTS
                           (SELECT 1
                              FROM nm_theme_roles
            ...
            THEN
                SELECT nm_theme_entry (a.nnth_nth_theme_id)
                 ...
                 WHERE     nith_nth_theme_id = p_theme
                       ...
                       AND EXISTS
                               (SELECT 1
                                  FROM hig_user_roles, nm_theme_roles
                                 WHERE     hur_username =
                                           SYS_CONTEXT ('NM3_SECURITY_CTX',
                                                        'USERNAME')
        ...
        l_lref :=
            get_nearest_nw_to_xy (l_geom.sdo_point.x,
                                  l_geom.sdo_point.y,
                                  l_base_themes);

        nm_debug.debug (
            'lref = ' || l_lref.lr_ne_id || ',' || l_lref.lr_offset);
        ...
        l_lref.lr_offset :=
            LEAST (
                NVL (nerow.ne_length, l_lref.lr_offset),
                Nm3unit.get_formatted_value (
                    l_lref.lr_offset,
                    Nm3net.get_nt_units (nerow.ne_nt_type)));
    ...
    --------------------------------------------------------------------------------------------------------------------
    --
    FUNCTION get_start_point (p_layer   IN NUMBER,
    ...
    FUNCTION get_end_point (p_layer   IN NUMBER,
                            p_ne_id   IN nm_elements.ne_id%TYPE)
        RETURN MDSYS.sdo_geometry
    IS
        retval       MDSYS.sdo_geometry;
    ...
    BEGIN
        ...
        cur_string :=
               'select sdo_lrs.geom_segment_end_pt( '
            || g_usgm.column_name
            || ', :diminfo ) '
            || 'from '
            || g_usgm.table_name
    ...
    ---------------------------------------------------------------------------------------------------------------------
    --
    FUNCTION Get_Layer_Element_Geometry (p_ne_id IN nm_elements.ne_id%TYPE)
        RETURN MDSYS.sdo_geometry
    IS
        l_layer   NUMBER;
    BEGIN
    ...
    BEGIN
        l_layer := Get_Datum_Theme (Nm3get.get_ne_all (p_ne_id).ne_nt_type);
        RETURN get_layer_fragment_geometry (l_layer,
                                            p_ne_id,
                                            p_begin,
                                            p_end);
    END;
    ...
    BEGIN
        ...
        IF is_clipped (p_begin, p_end, l_length) = 0
        THEN
            retval :=
                SDO_LRS.clip_geom_segment (retval,
                                           g_usgm.diminfo,
                                           p_begin,
                                           p_end);
        END IF;
    ...
    BEGIN
        ...
        IF element_has_shape (p_layer, p_ne_id) = 'TRUE'
        THEN
            IF measures_ok (l_geom) = 'FALSE'
            THEN
                ...
                THEN
                    ...
                    THEN
                        raise_application_error (
                            -200010,
                            'Geometry is invalid for split - check the measure values');
                    END IF;
                END IF;
            END IF;
            ...
            p_geom2 :=
                SDO_LRS.SCALE_GEOM_SEGMENT (GEOM_SEGMENT    => p_geom2,
                                            DIM_ARRAY       => g_usgm.diminfo,
                                            START_MEASURE   => 0,
                                            END_MEASURE     => l_end,
                                            SHIFT_MEASURE   => 0);
        ELSE
    ...
    BEGIN
        ...
        IF     Nm3sdo.element_has_shape (p_layer, p_ne_id_1) = 'TRUE'
           AND Nm3sdo.element_has_shape (p_layer, p_ne_id_2) = 'TRUE'
        THEN
            IF p_ne_id_to_flip = p_ne_id_1
            THEN
                l_geom_1 :=
                    Nm3sdo.reverse_geometry (
                        Nm3sdo.Get_Layer_Element_Geometry (p_layer,
                                                           p_ne_id_1));
                l_geom_2 :=
                    Nm3sdo.Get_Layer_Element_Geometry (p_layer, p_ne_id_2);
            ELSIF p_ne_id_to_flip = p_ne_id_2
            THEN
                l_geom_1 :=
                    Nm3sdo.Get_Layer_Element_Geometry (p_layer, p_ne_id_1);
                l_geom_2 :=
                    Nm3sdo.reverse_geometry (
                        Nm3sdo.Get_Layer_Element_Geometry (p_layer,
                                                           p_ne_id_2));
            ...
            IF SDO_LRS.connected_geom_segments (l_geom_1,
                                                g_usgm.diminfo,
                                                l_geom_2,
                                                g_usgm.diminfo) = 'TRUE'
            THEN
                IF l_connect = -2 OR l_connect = 1 OR l_connect = 0
                THEN
                    p_geom :=
                        SDO_LRS.concatenate_geom_segments (l_geom_1,
                                                           g_usgm.diminfo,
                                                           l_geom_2,
                                                           g_usgm.diminfo);
                ELSIF l_connect = 2 OR l_connect = -1
                THEN
                    p_geom :=
                        SDO_LRS.concatenate_geom_segments (l_geom_2,
                                                           g_usgm.diminfo,
                                                           l_geom_1,
                                                           g_usgm.diminfo);
                END IF;
            ...
            END IF;
        ELSIF    Nm3sdo.element_has_shape (p_layer, p_ne_id_1) = 'TRUE'
              OR Nm3sdo.element_has_shape (p_layer, p_ne_id_2) = 'TRUE'
        THEN
            Hig.raise_ner (pi_appl      => Nm3type.c_hig,
                           pi_id        => 201,
                           pi_sqlcode   => -20001);
    ...
    IS
        ...
        l_length      NUMBER;
    BEGIN
        IF NOT test_theme_for_update (p_layer)
        THEN
        ...
        THEN
            retval := p_geom;
            SDO_LRS.redefine_geom_segment (retval,
                                           g_usgm.diminfo,
                                           0,
                                           p_length_to_end);
        ELSE
            ...
            LOOP
                ...
                THEN
                    ...
                    l_ord :=
                        mdsys.sdo_ordinate_array (p_geom.sdo_ordinates (1));
                    l_ord.EXTEND;
                ...
                ELSE
                    ...
                    IF     p_geom.sdo_ordinates (3 * (i - 1) + 3) > p_measure
                       AND NOT l_insert_pt
                    THEN
                        ...
                        IF    l_pt.sdo_ordinates (1) <>
                              p_geom.sdo_ordinates (3 * (i - 2) + 1)
                           OR l_pt.sdo_ordinates (2) <>
                              p_geom.sdo_ordinates (3 * (i - 2) + 2)
                           OR l_pt.sdo_ordinates (3) <>
                              p_geom.sdo_ordinates (3 * (i - 2) + 3)
                        THEN
                    ...
                    l_ord.EXTEND;
                    l_ord (l_ord.LAST) :=
                        p_geom.sdo_ordinates (3 * (i - 1) + 1);
                    l_ord.EXTEND;
                    l_ord (l_ord.LAST) :=
                        p_geom.sdo_ordinates (3 * (i - 1) + 2);
                    l_ord.EXTEND;
            ...
            retval :=
                mdsys.sdo_geometry (p_geom.sdo_gtype,
                                    p_geom.sdo_srid,
                                    p_geom.sdo_point,
                                    p_geom.sdo_elem_info,
                                    l_ord);

            IF p_ne_id IS NOT NULL
            THEN
                l_length := Nm3net.Get_Ne_Length (p_ne_id);

                IF SDO_LRS.geom_segment_end_measure (retval, g_usgm.diminfo) !=
                   l_length
                THEN
                    SDO_LRS.redefine_geom_segment (retval,
                                                   g_usgm.diminfo,
    ...
    PROCEDURE redefine_geom_segment1 (
        p_geom      IN OUT NOCOPY MDSYS.sdo_geometry,
    ...
    BEGIN
        ...
        THEN
            raise_application_error (
                -20001,
                'Invalid dimension for handling measures');
        END IF;

        --task 0110661 - remove the -1 that was used as compensation for error in get_rounding

        l_dp :=
            GREATEST (Nm3unit.get_rounding (p_diminfo (3).sdo_tolerance), 0);

        FOR i IN 1 .. p_geom.sdo_ordinates.LAST
        LOOP
            IF MOD (i, 3) = 0
            THEN
                BEGIN
                    p_geom.sdo_ordinates (i) :=
                          p_start
                        + ROUND (
                              (  (p_geom.sdo_ordinates (i) - l_old_start)
                               * l_new_length
                               / l_old_length),
                              l_dp);
                EXCEPTION
    ...
    FUNCTION get_route_shape (p_ne_id     IN nm_elements.ne_id%TYPE,
                              ...
                              p_diminfo   IN MDSYS.sdo_dim_array,
                              p_srid      IN NUMBER,
                              p_part      IN VARCHAR2 DEFAULT NULL)
        RETURN MDSYS.sdo_geometry
    IS
        /*
    ...
    BEGIN
        ...
        THEN
            l_part :=
                Nm3net.is_gty_partial (
                    Nm3get.get_ne_all (p_ne_id).ne_gty_group_type);
        ELSE
            l_part := p_part;
        END IF;
        ...
        THEN
            ...
            l_action := 'MULTIPLE';
        ELSIF p_th.pa (1).ptr_id IS NULL
        THEN
        ...
        */
        ...
        cur_string :=
               'select nm_ne_id_of ne_id, a.ne_length, nm_begin_mp, nm_end_mp, nm_cardinality, nm_slk, nm_true, nm_end_slk, nm_end_true, a.ne_type, a.ne_nt_type '
        ...
        LOOP
            ...
            l_geom_tab (i) :=
                Nm3sdo.Get_Layer_Element_Geometry (l_th_id, l_ne_id (i));
        ...
        LOOP
            IF l_ne_type (i) != 'D'
            THEN
                IF l_geom_tab (i).sdo_elem_info IS NOT NULL
                THEN
                    ...
                    IF l_part = 'Y'
                    THEN
                        IF Nm3sdo.is_clipped (l_start (i),
                                              l_end (i),
                                              l_length (i)) =
                           0
                        THEN
                            ...
                            l_geom_tab (i) :=
                                SDO_LRS.clip_geom_segment (l_geom_tab (i),
                                                           p_diminfo,
                                                           l_start (i),
                                                           l_end (i));
                    ...
                    redefine_geom_segment1 (l_geom_tab (i),
                                            p_diminfo,
                                            NVL (l_slk (i), 0),
                                            NVL (l_end_slk (i), 0),
                                            0);

                    IF l_cardinality (i) < 0
                    THEN
                        ...
                        l_geom_tab (i) :=
                            Nm3sdo.reverse_geometry (l_geom_tab (i));
                    ...
                    IF retval.sdo_elem_info IS NULL
                    THEN
                    ...
                    ELSE
                        ...
                        ELSE
                            ...
                            IF SDO_LRS.connected_geom_segments (
                                   retval,
                                   p_diminfo,
                                   l_geom_tab (i),
                                   p_diminfo) = 'TRUE'
        ...
        IF nm3sdo.get_no_parts (retval) > 3 AND retval.sdo_gtype = 3302
        THEN
    ...
    BEGIN
        ...
        l_diminfo (3).sdo_tolerance :=
            NM3UNIT.GET_TOL_FROM_UNIT_MASK (l_unit);
    ...
    IS
        ...
        cur_string      VARCHAR2 (2000)
            :=    'select nm_ne_id_of ne_id, a.ne_length, nm_begin_mp, nm_end_mp, nm_cardinality, nm_slk, nm_true, a.ne_type, a.ne_nt_type '
               || '  from nm_elements a, nm_members '
               || '  where nm_ne_id_in = :c_ne_id '
               || '  and   nm_ne_id_of = a.ne_id '
               || '  order by nm_seq_no ';
    ...
    FUNCTION get_shape_from_nm (p_ne_id      nm_elements.ne_id%TYPE,
                                ...
                                p_end_mp     nm_members.nm_begin_mp%TYPE)
        RETURN MDSYS.sdo_geometry
    IS
        l_layer   NUMBER;
    ...
    BEGIN
        ...
        IF l_layer IS NOT NULL
        THEN
            RETURN get_shape_from_nm (l_layer,
                                      p_ne_id,
                                      p_ne_id_of,
                                      p_begin_mp,
                                      p_end_mp);
    ...
    IS
        ...
        l_geom          MDSYS.sdo_geometry;
        l_length        nm_elements.ne_length%TYPE
                            := Nm3net.get_datum_element_length (p_ne_id_of);
    ...
    BEGIN
        ...
        IF ABS (l_geom_length - l_length) > g_usgm.diminfo (3).sdo_tolerance
        THEN
            Hig.raise_ner (pi_appl      => Nm3type.c_hig,
                           pi_id        => 204,
                           pi_sqlcode   => -20001);
        ...
        IF p_end_mp > l_length + g_usgm.diminfo (3).sdo_tolerance
        THEN
        ...
        THEN
            ...
            IF is_clipped (p_begin_mp, p_end_mp, l_length) = 0
            THEN
                ...
                l_geom :=
                    SDO_LRS.clip_geom_segment (
                        l_geom,
                        g_usgm.diminfo,
                        LEAST (p_begin_mp, l_geom_length),
                        LEAST (p_end_mp, l_geom_length));
            ELSE
    ...
    IS
    BEGIN
        internal_member_dynseg (p_table_name   => p_table_name,
                                p_type         => 'I',
                                p_obj_type     => p_inv_type,
                                p_seq_name     => p_seq_name,
                                p_only_live    => g_only_live,
                                p_job_id       => p_job_id,
                                p_limit        => 5000);
    END;
    ...
    IS
        ...
        */
        ...
        CURSOR get_routes (c_nt    IN nm_linear_types.nlt_nt_type%TYPE,
                           c_gty   IN nm_linear_types.nlt_gty_type%TYPE)
        IS
            WITH
                route_data
                AS
                    (  SELECT nm_ne_id_in       ne_id,
                              nm_start_date     start_date,
                              nm_end_date       end_date
                         FROM nm_members_all, nm_elements_all
                        WHERE     nm_obj_type = c_gty
                              AND nm_ne_id_in = ne_id     --and ne_id = 114223
                              AND ne_nt_type = c_nt
                              AND ne_gty_group_type = c_gty
                     GROUP BY nm_ne_id_in, nm_start_date, nm_end_date)
            SELECT ne_id, start_date member_date FROM route_data
            UNION
            SELECT ne_id, end_date member_date FROM route_data
            ORDER BY 1, 2;
    ...
    BEGIN
        ...
        l_seq_str :=
            'select ' || p_nth.nth_sequence_name || '.nextval from dual';

        cur_str2 :=
               'insert into '
            || p_nth.nth_feature_table
        ...
        FOR irec IN get_routes (l_nlt.nlt_nt_type, l_nlt.nlt_gty_type)
        LOOP
            EXECUTE IMMEDIATE l_seq_str
                INTO l_id;

            IF irec.ne_id = l_ne_saved
            THEN
                BEGIN
                    nm3user.set_effective_date (
                        NVL (l_date_saved, TRUNC (SYSDATE)));


                    l_geom :=
                        get_route_shape (p_ne_id     => irec.ne_id, -- l_ga.nga(i).ng_ne_id,
                                         p_nt        => l_nt,
                                         p_th        => l_th,
                                         p_nth_tab   => l_nth_row_tab,
                                         p_diminfo   => l_diminfo,
    ...
    BEGIN
        ...
        SELECT LISTAGG (nm3sdo.get_gofg_substr (nth_feature_pk_column,
                                                nth_feature_table,
          ...
          FROM (  SELECT top_group_type,
                         ...
                         dim,
                         nth_use_history
                    FROM (SELECT MIN (levl) OVER (PARTITION BY top_group_type)
                                     min_levl,
                                 t3.*
                            FROM (SELECT ROW_NUMBER ()
                                             OVER (
                                                 PARTITION BY parent_group_type
                                                 ORDER BY
                                                     parent_group_type,
                                                     child_group_type)    rn,
                                         t2.*,
                                         nm3sdo.get_dimension (
                                             nm3sdo.GET_THEME_DIMINFO (
                                                 nth_theme_id))           dim
                                    FROM (  SELECT *
                                              FROM v_nm_sub_group_structure g,
                                                   v_nm_network_themes   t
                                             WHERE     g.child_nt_type =
                                                       t.nt_type
                                                   AND NVL (g.child_group_type,
                                                            'Â£$%^') =
                                                       NVL (t.gty_type,
                                                            'Â£$%^')
                                                   AND nth_base_table_theme
                                                           IS NULL
                                                   AND nth_feature_table NOT LIKE
                                                           'SECT_SS%'
                                          ORDER BY ord) t2) t3) t4
                   WHERE levl = min_levl
        ...
        --
        FOR ne_rec IN (SELECT ne_id
                         FROM nm_elements
                        WHERE ne_gty_group_type = p_gty_type)
        LOOP
            nm_debug.debug (
                   'generating data for gty = '
                || p_gty_type
                || ' ne = '
                || ne_rec.ne_id);
            --
    ...
    FUNCTION get_gofg_substr (p_feature_pk_column      IN VARCHAR2,
                              p_feature_table          IN VARCHAR2,
                              p_feature_shape_column   IN VARCHAR2,
                              p_dim                    IN INTEGER,
                              p_use_history            IN VARCHAR2)
        RETURN VARCHAR2
    IS
    BEGIN
        RETURN    ' select nm_ne_id_in ne_id, nm_ne_id_of, '
               ...
               || '       START WITH nm_ne_id_in = ne_data.root_ne ), '
               || p_feature_table
               || ' s '
               || ' WHERE     nm_ne_id_of = s.'
               || p_feature_pk_column
               || CASE p_use_history
                      WHEN 'Y' THEN ' and end_date is null '
                      WHEN 'N' THEN NULL
                  END;
    END;
    ...
    BEGIN
        ...
        ELSE
            ...
            FOR ip IN 4 .. p_geom.sdo_ordinates.LAST
            LOOP
                IF     NOT l_set
                   AND (   MOD ((ip - 1) / 3, p_interval) = 0
                        OR ip = p_geom.sdo_ordinates.LAST - 2)
                THEN
                    l_set := TRUE;
                END IF;
    ...
    IS
        ...
        */
        ...
        mem_string       VARCHAR2 (2000)
            :=    'select nm_ne_id_of, nm_begin_mp, nm_end_mp, nm_cardinality, nm_slk, nm_true '
               || '  from nm_members '
               || '  where nm_ne_id_in = :c_ne_id '
               || '  order by nm_seq_no ';

        mem_string_sc    VARCHAR2 (2000)
            :=    'select nm_ne_id_of, nm_begin_mp, nm_end_mp, nm_cardinality, nm_slk, nm_true '
    ...
    BEGIN
        ...
        IF Nm3net.is_sub_class_used (Nm3net.get_gty_type (p_ne_id))
        THEN
            ...
            THEN
                ...
                THEN
                    l_geom :=
                        SDO_LRS.clip_geom_segment (l_geom,
                                                   ...
                                                   l_end);
                    l_geom :=
                        SDO_LRS.SCALE_GEOM_SEGMENT (l_geom,
                                                    g_usgm.diminfo,
                                                    0,
                                                    (l_end - l_start),
                                                    0);
                ...
                THEN
                    ...
                    LOOP
                        ...
                        IF     NOT l_set
                           AND (   MOD ((ip - 1) / 3, p_interval) = 0
                                OR ip = l_geom.sdo_ordinates.LAST - 2)
                        THEN
                            l_set := TRUE;
                ...
                ELSE
                    ...
                    LOOP
                        ...
                        THEN
                            l_ord (l_oip) :=
                                l_geom.sdo_ordinates (ip) + l_total_length;
                            l_length := l_ord (l_oip);
                        ELSE
                            l_ord (l_oip) := l_geom.sdo_ordinates (ip);
                        END IF;
        ...
        retval :=
            mdsys.sdo_geometry (l_geom.sdo_gtype,
                                l_geom.sdo_srid,
                                l_geom.sdo_point,
                                l_elem,
                                l_ord);
    ...
    IS
    BEGIN
        RETURN sample_route (p_layer,
                             Nm3net.get_ne_id (p_route_name),
                             p_interval);
    END;
    ...
    FUNCTION element_has_shape (p_ne_id IN nm_elements.ne_id%TYPE)
        RETURN VARCHAR2
    IS
        retval   VARCHAR2 (5) := 'FALSE';
        nerow    nm_elements%ROWTYPE := Nm3get.get_ne_all (p_ne_id);

        CURSOR c1 (c_nt_type IN VARCHAR2, c_gty_type IN VARCHAR2)
        IS
            SELECT nnth.nnth_nth_theme_id
              FROM NM_LINEAR_TYPES nlt, NM_NW_THEMES nnth
             WHERE     nlt.NLT_ID = nnth.nnth_nlt_id
                   AND NVL (c_gty_type, Nm3type.c_nvl) =
                       NVL (nlt.NLT_GTY_TYPE, Nm3type.c_nvl)
                   AND nlt.NLT_NT_TYPE = c_nt_type;
    BEGIN
        FOR irec IN c1 (nerow.ne_nt_type, nerow.ne_gty_group_type)
        LOOP
            IF element_has_shape (irec.nnth_nth_theme_id, p_ne_id) = 'TRUE'
            THEN
                retval := 'TRUE';
                EXIT;
            END IF;
    ...
    BEGIN
        ...
        IF set_theme (l_nth.nth_theme_id)
        THEN
    ...
    FUNCTION Set_Srid (p_layer IN NUMBER, p_geom MDSYS.sdo_geometry)
        RETURN MDSYS.sdo_geometry
    IS
        l_srid   NUMBER;
    BEGIN
        ...
        RETURN mdsys.sdo_geometry (p_geom.sdo_gtype,
                                   g_usgm.srid,
                                   p_geom.sdo_point,
                                   p_geom.sdo_elem_info,
                                   p_geom.sdo_ordinates);
    ...
    FUNCTION Set_Srid (p_geom IN MDSYS.sdo_geometry, p_srid IN NUMBER)
        RETURN MDSYS.sdo_geometry
    IS
    BEGIN
        RETURN mdsys.sdo_geometry (p_geom.sdo_gtype,
    ...
    FUNCTION project (p_geom IN MDSYS.sdo_geometry, p_srid IN NUMBER)
        RETURN MDSYS.sdo_geometry
    IS
    BEGIN
    ...
    FUNCTION get_midpoint (p_geom IN MDSYS.sdo_geometry)
        RETURN MDSYS.sdo_geometry
    IS
        ldim     INTEGER;
        ldis     NUMBER;
        ...
        retval   MDSYS.sdo_geometry;
    BEGIN
        IF Nm3sdo.get_type_from_gtype (p_geom.sdo_gtype) != 2
        THEN
            Hig.raise_ner (pi_appl      => Nm3type.c_hig,
                           pi_id        => 284,
                           pi_sqlcode   => -20001);
        ...
        IF retval.sdo_point IS NULL
        THEN
            RETURN mdsys.sdo_geometry (
                       2001,
                       retval.sdo_srid,
                       mdsys.sdo_point_type (retval.sdo_ordinates (1),
                                             retval.sdo_ordinates (2),
                                             NULL),
                       NULL,
                       NULL);
        ELSE
            RETURN mdsys.sdo_geometry (
                       2001,
                       retval.sdo_srid,
                       mdsys.sdo_point_type (retval.sdo_point.x,
                                             retval.sdo_point.y,
                                             NULL),
                       NULL,
                       NULL);
    ...
    -----------------------------------------------------------------------------
    --
    FUNCTION get_2d_pt (p_x IN NUMBER, p_y IN NUMBER)
        RETURN MDSYS.sdo_geometry
    IS
        retval   MDSYS.sdo_geometry;
        l_srid   NUMBER;
    ...
    FUNCTION get_2d_pt (p_geom MDSYS.sdo_geometry)
        RETURN MDSYS.sdo_geometry
    IS
        --l_geom mdsys.sdo_geometry;
        -- Task 0108546
    ...
    FUNCTION get_distance_points (p_layer      IN NUMBER,
    ...
    BEGIN
        ...
        THEN
            l_end :=
                SDO_LRS.geom_segment_end_measure (p_geometry, g_usgm.diminfo);
            ...
            LOOP
                ...
                --
                IF p_interval * irec <= l_end
                THEN
                    l_pt_geom :=
                        SDO_LRS.locate_pt (l_geometry,
                                           g_usgm.diminfo,
                                           p_interval * irec);
                    ...
                    l_ordinates (l_ordinates.LAST) :=
                        l_pt_geom.sdo_ordinates (1);

                    l_ordinates.EXTEND;
                    l_ordinates (l_ordinates.LAST) :=
                        l_pt_geom.sdo_ordinates (2);
                END IF;
            END LOOP;

            retval :=
                mdsys.sdo_geometry (2005,
                                    g_usgm.srid,
                                    NULL,
    ...
    BEGIN
        l_route_units :=
            Nm3get.get_nt (pi_nt_type => Nm3get.get_ne (p_ne_id).ne_nt_type).nt_length_unit;

        RETURN get_distance_points (
                   p_layer,
                   Nm3unit.convert_unit (p_unit_id,
                                         l_route_units,
                                         p_interval),
                   Get_Layer_Element_Geometry (p_layer, p_ne_id));
    END;
    ...
    FUNCTION is_clipped (p_start    IN NUMBER,
                         p_end      IN NUMBER,
                         p_length   IN NUMBER)
        RETURN NUMBER
    IS
        retval   NUMBER;
    BEGIN
        IF p_end = p_length AND p_start = 0
        THEN
    ...
    ----------------------------------------------------------------------------------------------------------
    --
    FUNCTION get_placement_mp_geometry (p_layer      IN NUMBER,
    ...
    BEGIN
        ...
        cur_string :=
               'select /*+cardinality( e '
            || TO_CHAR (p_pl_array.npa_placement_array.LAST)
            || ') */ '
            || ' b.ne_length, e.pl_ne_id, e.pl_start, e.pl_end, e.pl_measure, '
            || ' decode( nm3sdo.is_clipped( e.pl_start, e.pl_end, b.ne_length), '
            || ' 1, a.'
            || g_usgm.column_name
            || ', sdo_lrs.clip_geom_segment( a.'
            || g_usgm.column_name
            || ', :l_dim_info, e.pl_start, e.pl_end ))'
            || ' from nm_elements b, '
            || g_usgm.table_name
            || ' a, table ( '
    ...
    BEGIN
        ...
        cur_string :=
               'select b.ne_length, e.nte_ne_id_of, e.nte_begin_mp, e.nte_end_mp, '
            || ' decode( nm3sdo.is_clipped( e.nte_begin_mp, e.nte_end_mp, b.ne_length), '
            || ' 1, a.'
            || g_usgm.column_name
            || ', sdo_lrs.clip_geom_segment( a.'
            || g_usgm.column_name
            || ', :l_dim_info, e.nte_begin_mp, e.nte_end_mp ))'
            || ' from nm_elements b, '
            || g_usgm.table_name
            || ' a, nm_nw_temp_extents e  '
            || ' where a.'
            || g_nth.nth_feature_pk_column
            || ' = e.nte_ne_id_of '
    ...
    */
    ...
    BEGIN
        ...
        THEN
            ...
            BEGIN
                l_diminfo :=
                    calculate_table_diminfo (pi_table_name, pi_column_name);
        ...
        ELSE
            ...
            THEN
                ...
                l_diminfo :=
                    SDO_LRS.convert_to_std_dim_array (coalesce_nw_diminfo);
            ELSE
        ...
        ins_usgm (l_rec_usgm.table_name,
                  l_rec_usgm.column_name,
                  l_rec_usgm.diminfo,
                  l_rec_usgm.srid);
        /*
    ...
    -----------------------------------------------------------------------------
    --
    FUNCTION clone_layer (p_layer         IN NUMBER,
                          p_table_name    IN VARCHAR2,
                          p_column_name   IN VARCHAR2)
        RETURN NUMBER
    IS
    ...
    FUNCTION clone_2d_from_3d_layer (p_layer         IN NUMBER,
                                     p_table_name    IN VARCHAR2,
                                     p_column_name   IN VARCHAR2)
        RETURN NUMBER
    IS
    BEGIN
        ...
        ins_usgm (p_table_name,
                  p_column_name,
                  SDO_LRS.convert_to_std_dim_array (g_usgm.diminfo),
                  g_usgm.srid);
    ...
    FUNCTION clone_pt_from_line (p_layer         IN NUMBER,
                                 p_table_name    IN VARCHAR2,
                                 p_column_name   IN VARCHAR2)
        RETURN NUMBER
    IS
    ...
    FUNCTION get_sdo_pt (p_layer IN NUMBER, p_x IN NUMBER, p_y IN NUMBER)
        RETURN MDSYS.SDO_GEOMETRY
        DETERMINISTIC
    IS
    BEGIN
    ...
    BEGIN
        ...
        IF l_start.lr_ne_id = l_end.lr_ne_id
        THEN
            IF l_start.lr_offset < l_end.lr_offset
            THEN
                RETURN nm_placement (l_start.lr_ne_id,
                                     l_start.lr_offset,
                                     l_end.lr_offset,
                                     0);
            ELSIF l_start.lr_offset > l_end.lr_offset
            THEN
                RETURN nm_placement (l_start.lr_ne_id,
                                     l_end.lr_offset,
                                     l_start.lr_offset,
    ...
    IS
        ...
        BEGIN
            SELECT COUNT (*)
              INTO l_count
              FROM TABLE (g_error_to_ignore.pa) val
    ...
    BEGIN
        ...
        IF g_usgm.srid != NVL (p_geom.sdo_srid, -99999)
        THEN
        ...
        --  END IF;
        --
        IF use_surrogate_key = 'N' OR l_is_datum
        THEN
            cur_string :=
                   'insert into '
                || g_usgm.table_name
        ...
        ELSE
            cur_string :=
                ...
                || g_usgm.column_name
                || ', objectid)'
                || 'select :ne_val, :geom_val, '
                || get_spatial_seq (p_layer)
                || '.nextval from dual';
        END IF;
    ...
    FUNCTION get_table_srid (p_table_name    IN VARCHAR2,
                             p_column_name   IN VARCHAR2)
        RETURN NUMBER
    IS
        CURSOR c1 (c_tab VARCHAR2, c_col VARCHAR2)
        IS
            SELECT sdo_srid
              FROM mdsys.sdo_geom_metadata_table
    ...
    PROCEDURE add_new_inv_shapes (
        ...
        P_GEOM    IN OUT NOCOPY MDSYS.sdo_geometry)
    IS
        CURSOR c_inv (c_base_layer IN NUMBER, c_ne_id IN NUMBER)
        IS
            SELECT nth_feature_table            ftab,
                   nth_feature_pk_column        pkcol,
                   nth_feature_pk_column        fkcol,
                   nth_feature_shape_column     geocol,
                   nm_ne_id_in,
                   nm_obj_type,
                   nm_begin_mp,
                   nm_end_mp,
    ...
    BEGIN
        ...
        FOR irec IN c_inv (p_layer, p_ne_id)
        LOOP
            ...
            ELSE
                ...
                THEN
                    ...
                    l_geom :=
                        mdsys.sdo_geometry (
                            ...
                            NULL);
                ELSE
                    IF is_clipped (irec.nm_begin_mp,
                                   irec.nm_end_mp,
                                   l_length) =
                       0
                    THEN
                        l_geom :=
                            SDO_LRS.clip_geom_segment (p_geom,
                                                       g_usgm.diminfo,
                                                       irec.nm_begin_mp,
                                                       irec.nm_end_mp);
                    ELSE
                ...
                EXECUTE IMMEDIATE   'insert into '
                                 || irec.ftab
                                 || ' ( '
                                 || 'ne_id, ne_id_of, nm_begin_mp, nm_end_mp,'
                                 || irec.geocol
                                 || ', start_date )'
                                 || ' values ( :ne_id, :ne_id_of, :nm_begin_mp, :nm_end_mp, :geom, :start_date )'
                    USING irec.nm_ne_id_in,
                          p_ne_id,
                          irec.nm_begin_mp,
                          irec.nm_end_mp,
                          l_geom,
    ...
    BEGIN
        ...
        LOOP
            ...
            ELSE
                ...
                eleminfo (eleminfo.LAST) :=
                    iend - p_geom.sdo_elem_info (lelem) + 2;
                eleminfo.EXTEND;
        ...
        LOOP
            ...
            THEN
                retval :=
                    mdsys.sdo_ordinate_array (p_geom.sdo_ordinates (ilx));
                retval.EXTEND;
            ...
            ELSE
                l_measure :=
                      l_measure
                    + (  p_geom.sdo_ordinates (ilm + 3)
                       - p_geom.sdo_ordinates (ilm));
    ...
    FUNCTION get_placement_mbr (p_layer      IN NUMBER,
                                p_pl_array   IN nm_placement_array)
        RETURN MDSYS.sdo_geometry
    IS
        -- replace with 9i aggr function

        CURSOR c_placement (c_pl_array nm_placement_array)
        IS
        ...
        l_geom       MDSYS.sdo_geometry;
        l_eleminfo   MDSYS.sdo_elem_info_array
    ...
    BEGIN
        ...
        FOR irec IN c_placement (p_pl_array)
        LOOP
            ...
            IF is_clipped (irec.pl_start, irec.pl_end, irec.ne_length) = 0
            THEN
                l_geom :=
                    SDO_LRS.clip_geom_segment (l_geom,
                                               g_usgm.diminfo,
        ...
        retval :=
            mdsys.sdo_geometry (2003,
                                ...
                                NULL,
                                l_eleminfo,
                                mdsys.sdo_ordinate_array (l_minx,
                                                          l_miny,
                                                          l_maxx,
                                                          l_maxy));
    ...
    PROCEDURE get_3d_layer_extent (p_geom   IN     MDSYS.sdo_geometry,
                                   minx     IN OUT NUMBER,
                                   maxx     IN OUT NUMBER,
                                   miny     IN OUT NUMBER,
                                   maxy     IN OUT NUMBER)
    ...
    BEGIN
        ...
        FOR ic IN 1 .. num_points
        LOOP
            ic1 := 1 + 3 * (ic - 1);
            ic2 := ic1 + 1;

            IF p_geom.sdo_ordinates (ic1) < minx
            THEN
                minx := p_geom.sdo_ordinates (ic1);
            END IF;

            IF p_geom.sdo_ordinates (ic1) > maxx
            THEN
                maxx := p_geom.sdo_ordinates (ic1);
            END IF;

            IF p_geom.sdo_ordinates (ic2) < miny
            THEN
                miny := p_geom.sdo_ordinates (ic2);
            END IF;

            IF p_geom.sdo_ordinates (ic2) > maxy
            THEN
    ...
    FUNCTION npt_line (p_geom   IN MDSYS.sdo_geometry,
                       npt      IN INTEGER,
                       ipt      IN INTEGER)
        RETURN MDSYS.sdo_geometry
    IS
        ...
        ordval   MDSYS.sdo_ordinate_array
            := mdsys.sdo_ordinate_array (p_geom.sdo_ordinates (lstart));
    ...
    FUNCTION get_mbr (p_geom IN MDSYS.sdo_geometry)
        RETURN MDSYS.sdo_geometry
    IS
        retval   MDSYS.sdo_geometry;
    BEGIN
    ...
    FUNCTION get_mbr (p_layer IN NUMBER, p_ne_id IN nm_elements.ne_id%TYPE)
        RETURN MDSYS.sdo_geometry
    IS
        l_geom   MDSYS.sdo_geometry;
    ...
    FUNCTION get_aggr_mbr (p_layer   IN NUMBER,
                           p_ne_id   IN nm_elements.ne_id%TYPE)
        RETURN MDSYS.sdo_geometry
    IS
    BEGIN
        RETURN get_placement_aggr_mbr (
                   p_layer,
                   Nm3pla.get_placement_from_ne (p_ne_id));
    END;
    ...
    FUNCTION get_placement_aggr_mbr (p_layer      IN NUMBER,
                                     p_pl_array   IN nm_placement_array)
        RETURN MDSYS.sdo_geometry
    IS
        cur_string   VARCHAR2 (2000);
    ...
    BEGIN
        cur_string :=
            'select sdo_aggr_mbr( nm3sdo.get_placement_mp_geometry( :b_layer, :b_pl_array )) from dual';
    ...
    FUNCTION get_centre_and_size (p_geom IN MDSYS.sdo_geometry)
        RETURN MDSYS.sdo_geometry
    IS
        --l_geom mdsys.sdo_geometry;
        l_mbr     MDSYS.sdo_geometry;
    ...
    BEGIN
        ...
        THEN
            ...
            ELSE
                ...
                RETURN mdsys.sdo_geometry (3001,
                                           p_geom.sdo_srid,
                                           l_point,
                                           NULL,
                                           NULL);
        ...
        ELSE
            l_x :=
                  (l_mbr.sdo_ordinates (1) + l_mbr.sdo_ordinates (1 + l_dim))
                / 2;
            l_y :=
                  (l_mbr.sdo_ordinates (2) + l_mbr.sdo_ordinates (2 + l_dim))
                / 2;

            l_m :=
                GREATEST (
                    ABS (
                          l_mbr.sdo_ordinates (1 + l_dim)
                        - l_mbr.sdo_ordinates (1)),
                    ABS (
                          l_mbr.sdo_ordinates (2 + l_dim)
                        - l_mbr.sdo_ordinates (2)));
            ...
            RETURN mdsys.sdo_geometry (3001,
                                       l_mbr.sdo_srid,
                                       mdsys.sdo_point_type (l_x, l_y, l_m),
                                       NULL,
                                       NULL);
        END IF;
    END;
    ...
    FUNCTION get_centre_and_size (p_theme IN NUMBER, p_pk_id IN NUMBER)
        RETURN MDSYS.sdo_geometry
    IS
        l_geom   MDSYS.sdo_geometry;
        l_mbr    MDSYS.sdo_geometry;
    ...
    FUNCTION get_gdo_centre_and_size (p_session_id IN NUMBER)
        RETURN MDSYS.sdo_geometry
    IS
    ...
    BEGIN
        SELECT *
         ...
         WHERE gdo_session_id = p_session_id AND ROWNUM = 1;

        IF l_gdo.gdo_dynamic_theme = 'Y'
        THEN
            l_dynamic := TRUE;
        ELSE
            l_dynamic := FALSE;
        ...
        IF l_mbr.sdo_gtype IS NOT NULL
        THEN
    ...
    PROCEDURE get_point_coordinates (p_geom   IN     MDSYS.SDO_GEOMETRY,
                                     p_x         OUT NUMBER,
                                     p_y         OUT NUMBER,
                                     p_m         OUT NUMBER)
    IS
        l_geom   MDSYS.sdo_geometry;
    BEGIN
        IF get_type_from_gtype (p_geom.sdo_gtype) != 1
        THEN
            Hig.raise_ner (pi_appl      => Nm3type.c_hig,
                           pi_id        => 288,
                           pi_sqlcode   => -20001);
        --    RAISE_APPLICATION_ERROR(-20001, 'Not a point geometry');
        ELSIF p_geom.sdo_point.x IS NULL
        THEN
            Hig.raise_ner (pi_appl      => Nm3type.c_hig,
                           pi_id        => 289,
                           pi_sqlcode   => -20001);
    ...
    PROCEDURE get_geometry_array (p_geom   IN     MDSYS.sdo_geometry,
    ...
    BEGIN
        get_point_coordinates (
            get_2d_pt (get_theme_shape (p_theme_id, p_pk_id)),
            p_x,
            p_y,
            p_z);
    END;
    ...
    FUNCTION get_theme_shape (p_theme_id IN NUMBER, p_pk_id IN NUMBER)
        RETURN MDSYS.sdo_geometry
    IS
    ...
    BEGIN
        cur_string :=
               'select '
            || nth_rec.nth_feature_shape_column
            || ' from '
            || nth_rec.nth_feature_table
            || ' where '
            || nth_rec.nth_feature_pk_column
            || ' = :PK_VAL';
    ...
    FUNCTION get_theme_shape (p_theme_name IN VARCHAR2, p_pk_id IN NUMBER)
        RETURN MDSYS.sdo_geometry
    IS
        -- this assumes that only one shape exists in the theme table for the PK value. If dates are used and the theme
        -- should be a date-tracked view else there will be difficulties.
    ...
    FUNCTION Get_Theme_Diminfo (p_nth_id IN NUMBER)
        RETURN MDSYS.sdo_dim_array
    IS
        CURSOR c1 (c_nth_id IN NUMBER)
        IS
            SELECT sdo_diminfo
    ...
    FUNCTION Get_Theme_SRID (p_nth_id IN NUMBER)
        RETURN INTEGER
    IS
        retval   INTEGER;
    BEGIN
        SELECT sdo_srid
         ...
         WHERE     nth_feature_table = sdo_table_name
               ...
               AND nth_theme_id = p_nth_id;
    EXCEPTION
        WHEN NO_DATA_FOUND
        THEN
            raise_application_error (-20012,
                                     'Theme or SDO metadata not found');
    END get_theme_srid;
    ...
    FUNCTION get_dimension (p_diminfo IN MDSYS.sdo_dim_array)
        RETURN NUMBER
    IS
    BEGIN
    ...
    --------------------------------------------------------------------------------------
    --
    FUNCTION get_theme_metadata (p_nth_id IN NUMBER)
        RETURN user_sdo_geom_metadata%ROWTYPE
    IS
        CURSOR c1 (c_nth_id IN NUMBER)
        IS
        ...
        -- NOTE: this will not compile in TOAD this this a TOAD bug not an issue with the package
        -- Execute it in SQL*PLUS to get it to compile.
    ...
    --------------------------------------------------------------------------------------
    --
    FUNCTION get_theme_gtype (p_nth_id IN NUMBER)
        RETURN NUMBER
    IS
        --
        /*
        ...
        */
        ...
        BEGIN
            SELECT ntg_gtype
             ...
             WHERE ntg_theme_id = p_nth_id AND ROWNUM = 1;
    ...
    BEGIN
        ...
        THEN
            ...
            THEN
                retval :=
                    get_table_gtype (l_nth.nth_feature_table,
    ...
    --------------------------------------------------------------------------------------
    --
    FUNCTION get_theme_gtype_text (p_nth_id IN NUMBER)
        RETURN VARCHAR2
    IS
        /*
        cursor c1( c_nth_id in number ) is
    ...
    FUNCTION get_table_gtype (p_table       IN VARCHAR2,
                              p_column      IN VARCHAR2,
    ...
    BEGIN
        ...
        THEN
            lstr :=
                'select max(' || p_column || '.sdo_gtype) from ' || p_table;
        ELSE
            lstr :=
                   'select a.'
                || p_column
    ...
    IS
        ...
        CURSOR c_pk_type (c_tab IN VARCHAR2, c_col IN VARCHAR2)
        IS
            SELECT data_length, data_precision, data_scale
              FROM USER_TAB_COLUMNS
             WHERE table_name = c_tab AND column_name = c_col;
    ...
    BEGIN
        ...
        THEN
            ...
            THEN
                ...
                FETCH c_pk_type
                    INTO l_data_length, l_data_precision, l_data_scale;

                IF    c_pk_type%NOTFOUND
                   OR l_data_length != 22
                   OR l_data_precision != 38
                   OR l_data_scale != 0
                THEN
                    CLOSE c_pk_type;
    ...
    --------------------------------------------------------------------------------------------------------------
    --
    FUNCTION is_table_regd (p_feature_table IN VARCHAR2, p_col IN VARCHAR2)
        RETURN BOOLEAN
    IS
        l_dummy   NUMBER;
    BEGIN
    ...
    --------------------------------------------------------------------------------------------------------------
    --
    FUNCTION validate_theme (p_theme    IN NUMBER,
    ...
    IS
        ...
        CURSOR diminfo_cur (p_table_name VARCHAR2, p_column_name VARCHAR2)
        IS
            SELECT dim.sdo_tolerance
              FROM all_sdo_geom_metadata usgm, TABLE (usgm.diminfo) dim
    ...
    BEGIN
        ...
        OPEN diminfo_cur (nth.nth_feature_table,
                          nth.nth_feature_shape_column);
        ...
        IF l_tolerance IS NULL
        THEN
            l_tolerance := c_default_tolerance;
        END IF;

        EXECUTE IMMEDIATE   'SELECT ROWID, '
                         || nth.nth_feature_shape_column
                         || ' FROM '
                         || nth.nth_feature_table
        ...
        LOOP
            ...
            BEGIN
                l_result :=
                    SDO_GEOM.VALIDATE_GEOMETRY_WITH_CONTEXT (l_shape (i),
                                                             l_tolerance,
                                                             NULL,
                                                             'TRUE');
            --
    ...
    --------------------------------------------------------------------------------------------------------------
    --
    FUNCTION get_shape_from_gdo (p_session_id IN NUMBER)
        RETURN MDSYS.sdo_geometry
    IS
        nth           NM_THEMES_ALL%ROWTYPE;

        CURSOR c1 (c_session_id IN NUMBER)
        IS
    ...
    BEGIN
        ...
        l_old_shape :=
            Nm3sdo.get_theme_shape (nth.nth_theme_id, l_tab_pk (1));


        IF l_old_shape.sdo_gtype = 1002
        THEN
            IF l_old_shape.sdo_elem_info IS NULL
            THEN
                l_new_shape :=
                    mdsys.sdo_geometry (
                        l_old_shape.sdo_gtype,
            ...
            ELSE
                l_new_shape :=
                    mdsys.sdo_geometry (
                        ...
                        l_old_shape.sdo_srid,
                        NULL,
                        mdsys.sdo_elem_info_array (1, 1002, 1),
                        mdsys.sdo_ordinate_array (l_tab_x (1), l_tab_y (1)));
            END IF;
        ELSIF l_old_shape.sdo_gtype = 1003
        THEN
            IF l_old_shape.sdo_elem_info IS NULL
            THEN
                l_new_shape :=
                    mdsys.sdo_geometry (
                        l_old_shape.sdo_gtype,
                        l_old_shape.sdo_srid,
                        mdsys.sdo_point_type (l_tab_x (1),
                                              l_tab_y (1),
                                              l_old_shape.sdo_point.z),
                        NULL,
                        NULL);
            ELSE
                l_new_shape :=
                    mdsys.sdo_geometry (
                        ...
                        mdsys.sdo_ordinate_array (
                            l_tab_x (1),
                            l_tab_y (1),
                            l_old_shape.sdo_ordinates (
                                l_old_shape.sdo_ordinates.LAST)));
            END IF;
        ELSIF l_old_shape.sdo_gtype IN (2002,
                                        2003,
                                        3002,
                                        3003)
        THEN
            FOR irec IN 1 .. l_tab_x.COUNT
            LOOP
                IF irec > 1
                THEN
                    IF    l_tab_theme (irec) != l_nth_name
                       OR l_tab_theme (irec) IS NULL
                    THEN
                        Hig.raise_ner (pi_appl      => Nm3type.c_hig,
                                       pi_id        => 235,
                                       pi_sqlcode   => -20001);
            ...
            IF l_old_shape.sdo_gtype IN (2002, 2003)
            THEN
                l_new_shape :=
                    mdsys.sdo_geometry (l_old_shape.sdo_gtype,
                                        l_old_shape.sdo_srid,
                                        NULL,
                                        mdsys.sdo_elem_info_array (1, 2, 1),
                                        l_ord);
            ELSIF l_old_shape.sdo_gtype = 3002
            THEN
                l_new_shape :=
                ...
                l_new_shape :=
                    SDO_LRS.convert_to_lrs_geom (l_new_shape, l_start, l_end);
            ELSIF l_old_shape.sdo_gtype = 3003
            THEN
                l_new_shape :=
                    mdsys.sdo_geometry (2003,
                                        l_old_shape.sdo_srid,
    ...
    PROCEDURE update_shape_from_gdo (p_session_id IN NUMBER)
    IS
        nth           NM_THEMES_ALL%ROWTYPE;
    ...
    BEGIN
        ...
        IF NOT test_theme_for_update (nth.nth_theme_id)
        THEN
            HIG.RAISE_NER ('NET', 339);
        END IF;
    ...
    FUNCTION get_table_mbr (p_table IN VARCHAR2, p_column IN VARCHAR2)
        RETURN MDSYS.sdo_geometry
    AS
        l_diminfo   MDSYS.sdo_dim_array;

        CURSOR c1 (c_tab IN VARCHAR2, c_col IN VARCHAR2)
        IS
            SELECT diminfo
    ...
    BEGIN
        ...
        */
        ...
        IF l_diminfo (3).sdo_lb IS NOT NULL
        THEN
            retval :=
                mdsys.sdo_geometry (3303,
                                    NULL,
                                    NULL,
    ...
    FUNCTION calculate_table_diminfo (
        ...
        p_estimate_new_tol      VARCHAR2 DEFAULT 'N')
        RETURN MDSYS.sdo_dim_array
    IS
    ...
    BEGIN
        ...
        BEGIN
            curstring :=
                'select sdo_aggr_mbr(' || p_column || ') from ' || p_table;

            EXECUTE IMMEDIATE curstring
                INTO l_mbr;
        EXCEPTION
            WHEN SUBSCRIPT_BEYOND_COUNT
            THEN
                raise_application_error (
                    -20006,
                    'Error in MBR calculation - check the gtypes are consistent');
            WHEN NO_DATA_FOUND
            THEN
                raise_application_error (
                    -20001,
                    'No data in table - cannot calculate the diminfo');
        END;
        ...
        THEN
            raise_application_error (
                -20001,
                'No MBR calculated - cannot calculate the diminfo');
        END IF;
        ...
        IF p_estimate_new_tol = 'Y'
        THEN
            IF get_type_from_gtype (l_gtype) > 1
            THEN
                l_tol_array := get_sample_tolerance (p_table, p_column);
            ELSE
                l_tol_array :=
                    num_array_type (l_tol,
                                    l_tol,
                                    l_tol,
                                    l_tol);
            END IF;
            ...
            retval :=
                mdsys.sdo_dim_array (mdsys.sdo_dim_element (
                                     ...
                                     mdsys.sdo_dim_element (
                                         'Y',
                                         l_mbr.sdo_ordinates (2),
                                         l_mbr.sdo_ordinates (5),
                                         l_tol_array (2)),
                                     mdsys.sdo_dim_element (
                                         l_z_or_m,
                                         l_mbr.sdo_ordinates (3),
                                         l_mbr.sdo_ordinates (6),
                                         l_tol_array (3)));
        ELSIF l_dim = 3 AND l_lrs_dim = 3
        THEN
            ...
            retval :=
                mdsys.sdo_dim_array (mdsys.sdo_dim_element (
                                     ...
                                     mdsys.sdo_dim_element (
                                         ...
                                         l_mbr.sdo_ordinates (4),
                                         l_tol_array (2)),
                                     mdsys.sdo_dim_element (l_z_or_m,
                                                            0,
                                                            999999999,
                                                            l_tol_array (3)));
        ELSIF l_mbr.get_dims = 4
        THEN
            retval :=
                mdsys.sdo_dim_array (mdsys.sdo_dim_element (
                                         'X',
    ...
    BEGIN
        ...
        EXCEPTION
            ...
            THEN
                RAISE_APPLICATION_ERROR (
                    -20101,
                    'Cannot find a network theme for ' || p_theme_id);
        END;
        ...
        nserow.nse_name :=
               'Spatial Extent '
            || TO_CHAR (nserow.nse_id)
            || ' from '
            || TO_CHAR (p_nsperow.nspe_id);
        ...
        l_mask :=
            CASE WHEN p_whole_flag = 'Y' THEN 'INSIDE' ELSE 'ANYINTERACT' END;
        --
        cur_string :=
            ...
            || ' , nsm_ne_id '
            || lf
            || ' , to_number(nsm_begin_mp) '
            || lf
            || ' , to_number(nsm_end_mp) '
            || lf
            || ' , nsm_seq_no '
            || lf
            || ' , nsm_datum '
            ...
            || ' ( '
            || lf
            || 'SELECT :nse_id nsm_nse_id,'
            || lf
            || ' nm3ddl.sequence_nextval(''nsm_id_seq'') nsm_id, '
            || lf
            || ' s.ne_id nsm_ne_id, '
            || ' nm3unit.get_formatted_value( sdo_lrs.geom_segment_start_measure(intersection), 1) nsm_begin_mp,'
            || lf
            || ' nm3unit.get_formatted_value( sdo_lrs.geom_segment_end_measure(intersection), 1) nsm_end_mp,'
            || lf
            || ' rownum nsm_seq_no, '
            || lf
            || ''''
            || CASE WHEN l_group_or_datum = 'D' THEN 'Y' ELSE 'N' END
            || ''''
            || ' nsm_datum,'
            || lf
            || ' '
            ...
            || lf
            || ' SELECT s.'
            || nthrow.nth_feature_pk_column
            || ' ne_id '
            || lf
            || ', s.'
            || nthrow.nth_feature_shape_column
            || ' shape '
            || lf
            || ', nspe_boundary '
            || lf
            || ' FROM '
            || nthrow.nth_feature_table
            || ' s, nm_spatial_extents '
            || lf
            || ' WHERE sdo_relate ( s.'
            || nthrow.nth_feature_shape_column
            || lf
            ...
            || ' ) '
            || lf
            || ' SELECT ne_id, sdo_lrs.lrs_intersection( all_results.shape, nspe_boundary, 0.05) intersection '
            || lf
            || ' FROM all_results '
            || lf
            || ') s '
        ...
        THEN
            ...
            --
            INSERT INTO nm_saved_extent_member_datums (nsd_nse_id,
                                                       nsd_nsm_id,
                                                       nsd_ne_id,
                                                       nsd_begin_mp,
                                                       nsd_end_mp,
    ...
    --------------------------------------------------------------------------------------------------------------
    --
    PROCEDURE Set_Srid (p_table    IN VARCHAR2,
                        p_column   IN VARCHAR2,
                        p_srid     IN NUMBER)
    IS
        l_srid   VARCHAR2 (10);
    ...
    BEGIN
        ...
        EXECUTE IMMEDIATE   'update mdsys.sdo_geom_metadata_table set sdo_srid = '
                         || l_srid
                         || ' where sdo_table_name = '
                         || ''''
                         || p_table
                         || ''''
                         || ' and sdo_column_name = '
    ...
    --------------------------------------------------------------------------------------------------------------
    --
    FUNCTION get_spatial_index (p_table IN VARCHAR2, p_column IN VARCHAR2)
        RETURN VARCHAR2
    IS
        CURSOR c_ind (c_table IN VARCHAR2, c_column IN VARCHAR2)
        IS
            SELECT i.index_name
    ...
    --------------------------------------------------------------------------------------------------------------
    --
    PROCEDURE re_project (p_table      IN VARCHAR2,
                          p_column     IN VARCHAR2,
    ...
    BEGIN
        ...
        IF l_old_srid IS NULL
        THEN
            Set_Srid (p_table, p_column, p_old_srid);
        ELSIF l_old_srid != p_old_srid
        THEN
            Hig.raise_ner (pi_appl      => Nm3type.c_hig,
                           pi_id        => 240,
                           pi_sqlcode   => -20001);
        ...
        EXECUTE IMMEDIATE   'update mdsys.sdo_geom_metadata_table set sdo_srid = '
                         || TO_CHAR (p_new_srid)
                         || ','
                         || ' sdo_diminfo =  nm3sdo.calculate_table_diminfo('
                         || ''''
                         || p_table
                         || ''''
                         || ','
    ...
    FUNCTION get_polygon_from_gdo (p_session_id IN NUMBER)
        RETURN MDSYS.sdo_geometry
    IS
        CURSOR c1 (c_session_id IN NUMBER)
        IS
    ...
    BEGIN
        ...
        LOOP
            IF irec > 1
            THEN
                IF    l_tab_theme (irec) != l_theme.nth_theme_name
                   OR l_tab_theme (irec) IS NULL
                THEN
                    Hig.raise_ner (pi_appl      => Nm3type.c_hig,
                                   pi_id        => 235,
        ...
        IF    l_ord (l_ord.LAST) != l_ord (2)
           OR l_ord (l_ord.LAST - 1) != l_ord (1)
        THEN
            l_ord.EXTEND;
    ...
    ----------------------------------------------------------------------------------------------------------------------------------------
    --
    PROCEDURE create_spatial_idx (p_table    IN VARCHAR2,
    ...
    ----------------------------------------------------------------------------------------------------------------------------------------
    --
    PROCEDURE create_qtree_idx (p_table    IN VARCHAR2,
                                p_column   IN VARCHAR2 DEFAULT 'GEOLOC')
    IS
        cur_string   VARCHAR2 (2000);
    ...
    PROCEDURE Change_Affected_Shapes (p_layer   IN NUMBER,
                                      p_ne_id   IN nm_elements.ne_id%TYPE)
    IS
        CURSOR c_themes (c_layer IN NUMBER)
        IS
            SELECT nth.nth_theme_id,
                   nth.nth_feature_table,
                   nat_gty_group_type     objtype,
                   2                      dim_setting,
                   'G'                    g_or_i,
                   nth_xsp_column,
                   nth_sequence_name
            ...
            SELECT nth.nth_theme_id,
                   nth.nth_feature_table,
                   nith_nit_id                                   objtype,
                   DECODE (nit_pnt_or_cont,  'P', 2,  'C', 3)    dim_setting,
                   'I'                                           g_or_i,
                   nth_xsp_column,
                   nth_sequence_name
              FROM NM_THEMES_ALL  nth,
            ...
            SELECT nth.nth_theme_id,
                   ...
                   nith_nit_id
                       objtype,
                   DECODE (nit_pnt_or_cont,  'P', 2,  'C', 3)
                       dim_setting,
                   'F'
                       g_or_i,
                   NULL
        ...
        FUNCTION get_dims (p_theme IN NUMBER)
            RETURN INTEGER
        IS
    ...
    BEGIN
        ...
        LOOP
            ...
            THEN
                ...
                THEN
                    IF     irec.nth_xsp_column IS NULL
                       AND NVL (hig.get_sysopt ('XSPOFFSET'), 'N') = 'N' -- check prod opt
                    THEN
                        get_shape_str :=
                               'sdo_lrs.convert_to_std_geom(nm3sdo.get_shape_from_nm( '
                            || TO_CHAR (p_layer)
                            || ', nm_ne_id_in, nm_ne_id_of, nm_begin_mp, nm_end_mp ))';
                    ELSE
                        get_shape_str :=
                ...
                ELSE
                    IF    (    irec.g_or_i = 'I'
                           AND is_ad_link_type (irec.objtype) = 'FALSE')
                       OR irec.g_or_i = 'G'
                    THEN
                        lstr :=
                               'insert into '
                            ...
                            || 'select '
                            || get_spatial_seq (irec.nth_theme_id)
                            || '.nextval, nm_ne_id_in, nm_ne_id_of, nm_begin_mp, nm_end_mp, '
                            || get_shape_str
                            || ','
                            || ' nm_start_date, nm_end_date '
                            || ' from nm_members_all '
                    ...
                    ELSE
                        lstr :=
                            ...
                            || 'select '
                            || get_spatial_seq (irec.nth_theme_id)
                            || '.nextval, nad_iit_ne_id, nm_ne_id_of, nm_begin_mp, nm_end_mp, '
                            || get_shape_str
                            || ','
                            || ' nm_start_date, nm_end_date '
            ...
            ELSE                                              -- foreign table
                ...
                DECLARE
                    l_nth   nm_themes_all%ROWTYPE
                                := Nm3get.get_nth (irec.nth_theme_id);
                BEGIN
                    IF l_nth.nth_feature_table != l_nth.nth_table_name
                    THEN
                        ...
                        IF    l_nth.nth_rse_fk_column IS NULL
                           OR l_nth.nth_st_chain_column IS NULL
                           OR l_nth.nth_end_chain_column IS NULL
                        THEN
                            RAISE_APPLICATION_ERROR (
                                -20001,
                                'You need to link the feature and asset table and the network measures');
                        ELSIF l_nth.nth_feature_fk_column IS NULL
                        THEN
    ...
    FUNCTION create_spatial_seq (p_theme_id IN NUMBER)
        RETURN VARCHAR2
    IS
        l_seq_name             VARCHAR2 (30)
                                   := 'NTH_' || TO_CHAR (p_theme_id) || '_SEQ';
        ex_object_exists       EXCEPTION;
        PRAGMA EXCEPTION_INIT (ex_object_exists, -955);
        ex_object_exists_ddl   EXCEPTION;
        PRAGMA EXCEPTION_INIT (ex_object_exists_ddl, -20306);
    BEGIN
        BEGIN
            Nm3ddl.create_object_and_syns (
                l_seq_name,
                'create sequence ' || l_seq_name || ' start with 1');
        EXCEPTION
            WHEN ex_object_exists_ddl
            THEN
                NULL;
        END;
    ...
    FUNCTION get_spatial_seq (p_theme_id IN NUMBER)
        RETURN VARCHAR2
    IS
        l_seq_name   VARCHAR2 (30)
                         := 'NTH_' || TO_CHAR (p_theme_id) || '_SEQ';
    ...
    FUNCTION get_spatial_index_type (p_table_name IN VARCHAR2)
        RETURN VARCHAR2
    IS
        CURSOR c1 (c_table_name IN VARCHAR2)
        IS
    ...
    FUNCTION get_spatial_index_type (p_theme_id IN NUMBER)
        RETURN VARCHAR2
    IS
    ...
    FUNCTION Convert_Dim_Array_To_Mbr (p_layer NUMBER)
        RETURN MDSYS.sdo_geometry
    IS
    ...
    FUNCTION Convert_Dim_Array_To_Mbr (p_diminfo   IN MDSYS.sdo_dim_array,
            ...
            retval :=
                mdsys.sdo_geometry (3303,
                                    ...
                                    mdsys.sdo_ordinate_array (
                                        ...
                                        p_diminfo (2).sdo_ub,
                                        p_diminfo (3).sdo_ub));
        ELSIF p_diminfo.LAST = 4
        THEN
            retval :=
                mdsys.sdo_geometry (4003,
                                    p_srid,
                                    NULL,
                                    mdsys.sdo_elem_info_array (1, 1003, 3),
                                    mdsys.sdo_ordinate_array (
    ...
    IS
        ...
        l_srid        NUMBER;
    BEGIN
        IF is_table_regd (p_table, p_shape_col)
        THEN
            IF p_override_sdo_meta = 'Y'
            THEN
        ...
        THEN
            ...
            BEGIN
                l_diminfo :=
                    calculate_table_diminfo (p_table,
                                             p_shape_col,
                                             p_tol,
                                             p_estimate_new_tol);
            EXCEPTION
                ...
                THEN
                    ...
                    IF l_diminfo.LAST IS NULL
                    THEN
                        raise_application_error (
                            -20001,
                            'No data on which to base initial extent of the table');
                    ELSE
                        l_diminfo :=
    ...
    --
    FUNCTION get_tol_from_unit_mask (p_unit IN NUMBER)
        RETURN NUMBER
    IS
    BEGIN
    ...
    FUNCTION get_theme_mbr (p_nth_id IN NUMBER)
        RETURN MDSYS.sdo_geometry
    IS
        l_nth   NM_THEMES_ALL%ROWTYPE;
    BEGIN
        ...
        RETURN get_table_mbr (l_nth.nth_feature_table,
                              l_nth.nth_feature_shape_column);
    END;
    ...
    -----------------------------------------------------------------------------
    --
    FUNCTION get_theme_centre_and_size (p_nth_id IN NUMBER)
        RETURN MDSYS.sdo_geometry
    IS
    BEGIN
    ...
    FUNCTION calculate_theme_mbr (p_nth_id IN NUMBER)
        RETURN MDSYS.sdo_geometry
    IS
        l_nth   NM_THEMES_ALL%ROWTYPE := Nm3get.get_nth (p_nth_id);
    BEGIN
        RETURN calculate_table_mbr (l_nth.nth_feature_table,
                                    l_nth.nth_feature_shape_column);
    END;
    ...
    FUNCTION calculate_table_mbr (p_table_name   IN VARCHAR2,
                                  p_column       IN VARCHAR2)
        RETURN MDSYS.sdo_geometry
    IS
    BEGIN
        RETURN Convert_Dim_Array_To_Mbr (
                   calculate_table_diminfo (p_table_name, p_column));
    END;

    --
    -----------------------------------------------------------------------------
    --
    FUNCTION calculate_theme_cent_and_size (p_nth_id IN NUMBER)
        RETURN MDSYS.sdo_geometry
    IS
    BEGIN
    ...
    BEGIN
        ...
        IF set_theme (p_nth_id)
        THEN
            set_theme_metadata (p_nth_id);
        END IF;

        IF g_nth.nth_feature_table IS NULL
        THEN
            Hig.raise_ner (pi_appl      => Nm3type.c_hig,
                           pi_id        => 243,
                           pi_sqlcode   => -20001);
        --    raise_application_error(-20001, 'Needs a Feature table');
        ELSIF g_nth.nth_feature_shape_column IS NULL
        THEN
            Hig.raise_ner (pi_appl      => Nm3type.c_hig,
                           pi_id        => 244,
                           pi_sqlcode   => -20001);
        ...
        IF g_usgm.srid IS NULL AND p_base_theme IS NULL
        THEN
            --  only suitable for X,Y data

            l_srid := NULL;
        ELSIF p_base_theme IS NOT NULL
        THEN
        ...
        cur_string :=
               'create table '
            || g_nth.nth_feature_table
            || '( '
            || g_nth.nth_feature_pk_column
            || '   number(38) not null,'
            || g_nth.nth_feature_shape_column
            || '  mdsys.sdo_geometry not null';

        IF     g_nth.nth_feature_fk_column IS NOT NULL
           AND g_nth.nth_feature_fk_column != g_nth.nth_feature_pk_column
        THEN
            cur_string :=
                   cur_string
                || ','
                || g_nth.nth_feature_fk_column
                || ' number(38)';
        END IF;
        ...
        IF g_nth.nth_x_column IS NOT NULL AND g_nth.nth_y_column IS NOT NULL
        THEN
            cur_string :=
                   'insert into '
                || g_nth.nth_feature_table
            ...
            THEN
                cur_string :=
                    cur_string || ', ' || g_nth.nth_feature_fk_column;
            END IF;
            ...
            THEN
                cur_string :=
                       cur_string
                    || ' from '
                    || g_nth.nth_table_name
                    || ' WHERE '
                    || format_where_clause (g_nth.nth_where);
            ELSE
            ...
            IF p_base_theme IS NOT NULL
            THEN
                l_dummy :=
                    Nm3sdo.clone_pt_from_line (
                        p_base_theme,
                        g_nth.nth_feature_table,
                        g_nth.nth_feature_shape_column);
            ELSE
                register_table (g_nth.nth_feature_table,
                                g_nth.nth_feature_shape_column,
                                'N');
            END IF;
        --next block deals with point item dyn-seg

        ELSIF     g_nth.nth_st_chain_column IS NOT NULL
              AND g_nth.nth_end_chain_column IS NULL
        THEN
            IF p_base_theme IS NULL
            THEN
                Hig.raise_ner (pi_appl      => Nm3type.c_hig,
                               pi_id        => 245,
                               pi_sqlcode   => -20001);
            ...
            cur_string :=
                ...
                || ' select a.'
                || g_nth.nth_pk_column
                || ', nm3sdo.get_2d_pt(sdo_lrs.locate_pt( b.'
                || l_base.nth_feature_shape_column
                || ', a.'
                || g_nth.nth_st_chain_column;

            IF g_nth.nth_offset_field IS NOT NULL
            THEN
            ...
            cur_string :=
                ...
                || g_nth.nth_table_name
                || ' a,'
                || l_base.nth_feature_table
                || ' b'
                || '  where a.'
                || g_nth.nth_rse_fk_column
                || ' = b.'
                || l_base.nth_feature_pk_column;
            ...
            l_dummy :=
                Nm3sdo.clone_pt_from_line (p_base_theme,
                                           g_nth.nth_feature_table,
                                           g_nth.nth_feature_shape_column);
        --third block dealing with linear dyn-seg

        ELSIF     g_nth.nth_st_chain_column IS NOT NULL
              AND g_nth.nth_end_chain_column IS NOT NULL
        THEN
            IF p_base_theme IS NULL
            THEN
                Hig.raise_ner (pi_appl      => Nm3type.c_hig,
            ...
            ELSE
                ...
                */
                ...
                cur_string :=
                    ...
                    || g_nth.nth_end_chain_column
                    || ', '
                    || TO_CHAR (g_usgm.diminfo (1).sdo_tolerance)
                    || ')';
            END IF;
            ...
            l_dummy :=
                Nm3sdo.clone_layer (p_base_theme,
                                    g_nth.nth_feature_table,
                                    g_nth.nth_feature_shape_column);
        ELSE
            --  what else is there?
        ...
        Nm3sdo.create_spatial_idx (g_nth.nth_feature_table,
                                   g_nth.nth_feature_shape_column);
        ...
        BEGIN
            ...
            cur_string :=
                ...
                || g_nth.nth_feature_table
                || ' ADD CONSTRAINT '
                || SUBSTR (g_nth.nth_feature_table, 1, 27)
                || '_PK PRIMARY KEY '
                || ' ( '
                || g_nth.nth_feature_pk_column
                || ')';
        ...
        THEN
            cur_string :=
                   'create index '
                || SUBSTR (g_nth.nth_feature_table, 1, 23)
                || '_FK_IDX'
                || ' on '
                || g_nth.nth_feature_table
                || ' ( '
        ...
        UPDATE NM_THEMES_ALL
           SET nth_theme_type = 'SDO', nth_storage = 'S'
    ...
    FUNCTION get_dim_from_gtype (p_gtype IN NUMBER)
        RETURN INTEGER
    IS
    BEGIN
    ...
    -----------------------------------------------------------------------------
    --
    FUNCTION get_lrs_from_gtype (p_gtype IN NUMBER)
        RETURN INTEGER
    IS
    BEGIN
    ...
    -----------------------------------------------------------------------------
    --
    FUNCTION get_type_from_gtype (p_gtype IN NUMBER)
        RETURN INTEGER
    IS
    BEGIN
    ...
    BEGIN
        ...
        LOOP
            ...
            FOR elerec IN 1 .. l_parts
            LOOP
                --    nm_debug.debug('Elem loop '||to_char( elerec ));

                EXIT WHEN elerec > p_elem;

                IF l_geom.sdo_elem_info ((elerec - 1) * 3 + 2) > 0
                THEN
                    FOR vertrec IN 2 .. l_geom.sdo_ordinates.LAST / l_dim
                    LOOP
                        ...
                        THEN
                            ...
                            THEN
                                ...
                                l_g_diff :=
                                    SQRT (
                                          POWER (
                                              (  l_geom.sdo_ordinates (ic)
                                               - l_geom.sdo_ordinates (
                                                     ic - l_dim)),
                                              2)
                                        + POWER (
                                              (  l_geom.sdo_ordinates (
                            ...
                            l_m_diff :=
                                ABS (
                                      l_geom.sdo_ordinates (ic + l_dim - 1)
                                    - l_geom.sdo_ordinates (ic - 1));
                        ...
                        FOR dimrec IN 1 .. l_dim
                        LOOP
                            IF dimrec = 1 OR dimrec = 2 OR l_lrs = 0
                            THEN
                                IF retval (dimrec) > l_g_diff
                                THEN
                                    retval (dimrec) :=
                                          0.5
                                        * POWER (
    ...
    PROCEDURE drop_metadata (p_object_name IN VARCHAR2)
    IS
    BEGIN
        DECLARE
    ...
    IS
        -- Constants
        using_sdm_subuser_reg    CONSTANT BOOLEAN
            := Hig.get_sysopt ('SDMREGULYR') = 'Y' ;
        registering_sde_layers   CONSTANT BOOLEAN
            := Hig.get_sysopt ('REGSDELAY') = 'Y' ;
        using_public_synonyms    CONSTANT BOOLEAN
            := Hig.get_sysopt ('HIGPUBSYN') = 'Y' ;
        ...
        --
        l_nth                             NM_THEMES_ALL%ROWTYPE;
        l_user                            HIG_USERS.hus_username%TYPE
                                              := UPPER (p_username);
        l_owner                           VARCHAR2 (30)
            := SYS_CONTEXT ('NM3CORE', 'APPLICATION_OWNER');
            ...
            THEN
                ...
                LOOP
                    BEGIN
                        ...
                        INSERT INTO all_sdo_geom_metadata (owner,
                                                           table_name,
                                                           column_name,
                                                           diminfo,
                                                           srid)
                ...
                --
                IF registering_sde_layers AND NOT using_public_synonyms
                THEN
                    BEGIN
                        EXECUTE IMMEDIATE(   ' begin nm3sde.create_sub_sde_layer ( p_theme_id => '
                                          || TO_CHAR (p_theme_id)
                                          || ',p_username => '''
                                          || TO_CHAR (p_username)
                                          || ''');'
                    ...
                    EXCEPTION
                        ...
                        THEN
                            ...
                            Nm_Debug.debug_on;
                            Nm_Debug.DEBUG (
                                'Error copying SDE layer for ' || p_theme_id);
                            Nm_Debug.debug_off;
                    END;
                END IF;

                IF registering_sde_layers AND using_public_synonyms
                THEN
    ...
    IS
        CURSOR get_sdo_owner (cp_table IN VARCHAR2, cp_column IN VARCHAR2)
        IS
            SELECT owner
        ...
        using_sde_layers   CONSTANT BOOLEAN
                                        := Hig.get_sysopt ('REGSDELAY') = 'Y' ;
        c_str                       Nm3type.max_varchar2;

        --
        PROCEDURE delete_sdo_metadata (pi_table_name   IN VARCHAR2,
    ...
    FUNCTION get_distance (p_theme   IN NUMBER,
                           p_pk      IN NUMBER,
                           p_x       IN NUMBER,
                           p_y       IN NUMBER)
        RETURN NUMBER
    IS
        ...
        l_nth_meta     user_sdo_geom_metadata%ROWTYPE
                           := get_theme_metadata (p_theme);
    ...
    BEGIN
        ...
        IF get_dimension (l_pt_diminfo) = 3
        THEN
    ...
    FUNCTION within_tolerance (p_theme   IN NUMBER,
                               p_pk      IN NUMBER,
                               p_x       IN NUMBER,
                               p_y       IN NUMBER)
        RETURN VARCHAR2
    ...
    FUNCTION Make_Single_Part (p_geom      IN MDSYS.sdo_geometry,
                               p_diminfo   IN MDSYS.sdo_dim_array)
        RETURN MDSYS.sdo_geometry
    IS
        lp       NUMBER;
        ...
        FUNCTION used (l_ia IN OUT NOCOPY int_array, l_i IN INTEGER)
            RETURN BOOLEAN
        IS
            l_dummy   INTEGER;
        BEGIN
    ...
    BEGIN
        ...
        LOOP
            ...
            FOR i IN 2 .. lp
            LOOP
                IF NOT used (l_used, i)
                THEN
                    --        Nm_Debug.DEBUG('Part '||TO_CHAR(i)||' is unused');

                    IF compare_pt (
                           Nm3sdo.get_2d_pt (
                               SDO_LRS.geom_segment_end_pt (retval)),
                           Nm3sdo.get_2d_pt (
    ...
    FUNCTION get_distance_between_parts (p_geom IN MDSYS.sdo_geometry)
        RETURN NUMBER
    IS
        lp         NUMBER;
        ld         NUMBER;
            ...
            LOOP
                ...
                lpt1 :=
                    mdsys.sdo_geometry (
                        ...
                        NULL,
                        mdsys.sdo_point_type (
                            p_geom.sdo_ordinates (p1 - ld),
                            p_geom.sdo_ordinates (p1 - ld + 1),
                            NULL),
                        NULL,
                        NULL);
                ...
                lpt2 :=
                    mdsys.sdo_geometry (
                        2001,
                        NULL,
                        mdsys.sdo_point_type (p_geom.sdo_ordinates (p1),
                                              p_geom.sdo_ordinates (p1 + 1),
                                              NULL),
                        NULL,
                        NULL);
    ...
    FUNCTION get_objects_in_buffer (
        p_nth_id           IN NUMBER,
        p_geometry         IN MDSYS.sdo_geometry,
        p_buffer           IN NUMBER,
    ...
    IS
        ...
        retval             nm_theme_list
            := nm_theme_list (nm_theme_list_type (nm_theme_detail (NULL,
                                                                   NULL,
                                                                   NULL,
                                                                   NULL,
                                                                   NULL,
        ...
        l_dim              MDSYS.sdo_dim_array
                               := nm3sdo.get_theme_diminfo (p_nth_id);
        l_tol              NUMBER;
        ...
        FUNCTION is_nw_theme (p_theme IN NUMBER)
            RETURN BOOLEAN
        IS
            CURSOR c1 (c_theme IN NUMBER)
            IS
                SELECT 1
                  FROM NM_NW_THEMES
    ...
    BEGIN
        l_tol :=
            Nm3sdo.get_table_diminfo (p_nth.nth_feature_table,
                                      p_nth.nth_feature_shape_column) (1).sdo_tolerance;
        ...
        THEN
            l_geometry :=
                SDO_UTIL.rectify_geometry (
                    SDO_UTIL.remove_duplicate_vertices (l_geometry, l_tol),
                    l_tol);
            l_valid := validate_geometry (l_geometry, NULL, l_tol);
        END IF;
        ...
        THEN
            ...
            ELSE
                RAISE_APPLICATION_ERROR (
                    -20001,
                    'Invalid Geometry - error code ' || TO_CHAR (l_valid));
            END IF;
        END IF;
        ...
        THEN
            cur_string :=
                ...
                || l_nth.nth_pk_column
                || ',SUBSTR(t.'
                || SUBSTR (l_nth.nth_label_column, 1, 100)
                || ',1,100), null'
                || ', t.'
                || l_nth.nth_pk_column;
            ...
            THEN
                cur_string :=
                       cur_string
                    || ', sdo_geom.sdo_distance(t.'
                    || l_nth.nth_feature_shape_column
                    || ', :p_geometry, :l_tol )';

                IF is_nw_theme (p_nth_id)
                THEN
                    cur_string :=
                           cur_string
                        || ', sdo_lrs.get_measure( sdo_lrs.project_pt(t.'
                        || l_nth.nth_feature_shape_column
                        || ', :p_geometry ))';
                ELSE
            ...
            THEN
                ...
                cur_string :=
                    ...
                    || ' from '
                    || l_nth.nth_table_name
                    || ' t, gis_data_objects g '
                    || ' where gdo_session_id = '
                    || TO_CHAR (p_gdo_session_id)
                    || ' and gdo_pk_id = '
                    || l_nth.nth_pk_column
                    || ' and '
                    || ' sdo_within_distance ( t.'
                    || l_nth.nth_feature_shape_column
                    || ', :shape, '
                    || ''''
                    || 'distance = '
            ...
            ELSE
                cur_string :=
                    ...
                    || '''';
            END IF;
        ELSIF p_nth.nth_feature_fk_column IS NOT NULL
        THEN
            cur_string :=
                   'select distinct t.'
                || l_nth.nth_pk_column
            ...
            THEN
                ...
                THEN
                    cur_string :=
                           cur_string
                        || ', sdo_lrs.get_measure( sdo_lrs.project_pt(f.'
                        || l_nth.nth_feature_shape_column
                        || ', :p_geometry ))';
                ELSE
                    cur_string := cur_string || ', null ';
            ...
            THEN
                ...
                cur_string :=
                    ...
                    || ' t, '
                    || l_nth.nth_feature_table
                    || ' f, gis_data_objects g '
                    || ' where g.gdo_session_id = '
                    || TO_CHAR (p_gdo_session_id)
                    || ' and g.gdo_pk_id = t.'
                    || l_nth.nth_pk_column
                    || ' and sdo_within_distance ( f.'
                    || l_nth.nth_feature_shape_column
                    || ', :shape, '
                    || ''''
                    || 'distance = '
            ...
            ELSE
                EXECUTE IMMEDIATE cur_string
                    BULK COLLECT INTO l_pk_array,
                         ...
                         l_dist_array,
                         l_meas_array
                    USING l_geometry, l_tol, l_geometry;
            END IF;
        ELSE
            EXECUTE IMMEDIATE cur_string
                BULK COLLECT INTO l_pk_array,
        ...
        LOOP
            ...
            THEN
                ...
                THEN
                    ...
                    THEN
                        retval :=
                            nm_theme_list (
                                nm_theme_list_type (
                                    nm_theme_detail (l_nth.nth_theme_id,
                                                     l_pk_array (i),
                                                     l_fk_array (i),
                                                     l_label_array (i),
                                                     l_dist_array (i),
                ...
                ELSE
                    ...
                    THEN
                        retval :=
                            retval.add_detail (l_nth.nth_theme_id,
                                               l_pk_array (i),
                                               l_fk_array (i),
                                               l_label_array (i),
                                               l_dist_array (i),
    ...
    IS
        ...
        CURSOR c_obj (c_theme_array    IN nm_theme_array_type,
                      ...
                      c_buffer         IN NUMBER,
                      c_buffer_units   IN NUMBER)
        IS
            SELECT nm3sdo.get_objects_in_buffer (nthe_id,
                                                 c_geometry,
                                                 c_buffer,
                                                 c_buffer_units,
                                                 'TRUE')
        ...
        l_theme_array   nm_theme_array
                            := get_list (p_theme_txt => p_theme_txt);

        l_tmp           nm_theme_list := NM3ARRAY.INIT_NM_THEME_LIST;
        retval          nm_theme_list := NM3ARRAY.INIT_NM_THEME_LIST;
    BEGIN
        OPEN c_obj (l_theme_array.nta_theme_array,
                    p_geometry,
                    p_buffer,
                    p_buffer_units);
    ...
    IS
        ...
        CURSOR c3 (c_theme IN NUMBER)
        IS
            SELECT a.nnth_nth_theme_id
              FROM nm_inv_themes,
                   nm_inv_nw,
                   nm_linear_types,
                   NM_NW_THEMES  a
    ...
    BEGIN
        ...
        THEN
            ...
            IF l_theme_list.COUNT = 0
            THEN
                --    No base themes to snap to - find the network theme that the inventory theme can relate to and find one that has roles against it.
        ...
        THEN
            ...
            LOOP
                ...
                l_ntl :=
                    Get_Objects_In_Buffer (l_nth.nth_theme_id,
                                           p_geom,
                                           l_nth.nth_tolerance,
                                           l_nth.nth_tol_units,
                                           l_get_projection);
                ...
                IF l_ntl.ntl_theme_list.COUNT > 0
                THEN
                    --      Nm_Debug.DEBUG('Snapped and retrieved '||l_ntl.ntl_theme_list.COUNT||' rows');

                    IF l_ntl.ntl_theme_list (1).ntd_pk_id IS NOT NULL
                    THEN
                        IF retval.ntl_theme_list (1).ntd_theme_id IS NULL
                        THEN
                            retval := l_ntl;
                        ELSE
    ...
    IS
        ...
        lth_id        NUMBER;
    BEGIN
        IF p_theme_txt IS NOT NULL
        THEN
            l_theme_txt := p_theme_txt;

            WHILE INSTR (l_theme_txt, ',') > 0
            LOOP
                --        Nm_Debug.DEBUG('In loop '||l_theme_txt);

                IF INSTR (l_theme_txt, ',') > 0
                THEN
                    lth_id :=
                        TO_NUMBER (
                            SUBSTR (l_theme_txt,
                ...
                THEN
                    retval :=
                        nm_theme_array (
                            nm_theme_array_type (nm_theme_entry (lth_id)));
                    linit := 1;
                ELSE
                ...
                l_theme_txt :=
                    SUBSTR (l_theme_txt, INSTR (l_theme_txt, ',') + 1);
            END LOOP;
        ...
        ELSE
            RETURN nm_theme_array (
                       nm_theme_array_type (nm_theme_entry (NULL)));
        END IF;
    END;
    ...
    BEGIN
        ...
        THEN
              ...
              SELECT nm_theme_entry (a.nthe_id)
                ...
                FROM TABLE (p_theme_array.nta_theme_array) a,
        ...
        IF l_theme_list2.COUNT > 0
        THEN
            --    Nm_Debug.DEBUG('Looping through, count = '||TO_CHAR(l_theme_list2.COUNT));

            IF p_theme_array.nta_theme_array (1).nthe_id IS NOT NULL
            THEN
                FOR i IN 1 .. l_theme_list2.COUNT
                LOOP
                    ...
                    l_ntl :=
                        nm3sdo.Get_Objects_In_Buffer (l_nth.nth_theme_id,
                                                      p_geom,
                                                      l_nth.nth_tolerance,
                                                      l_nth.nth_tol_units,
                                                      l_get_projection);
    ...
    BEGIN
        ...
        THEN
            ...
            ELSE
                l_rnd :=
                    Nm3unit.get_rounding (l_sdo.diminfo (1).sdo_tolerance);
            END IF;
    ...
    IS
        ...
        CURSOR c1 (c_nt IN ptr_vc_array)
        IS
    ...
    IS
        ...
        CURSOR c_nbth (c_theme_id IN NM_THEMES_ALL.nth_theme_id%TYPE)
        IS
    ...
    IS
        ...
        CURSOR c1 (c_ne_id IN NUMBER)
        IS
            SELECT ptr_vc (ROWNUM, nng_nt_type)
              FROM nm_nt_groupings, NM_ELEMENTS_ALL
             WHERE ne_id = c_ne_id AND ne_gty_group_type = nng_group_type;
    BEGIN
        OPEN c1 (p_ne_id);
    ...
    IS
        ...
        CURSOR c1 (c_nlt_id IN NUMBER)
        IS
            SELECT ptr_vc (ROWNUM, nng_nt_type)
              FROM nm_nt_groupings, NM_LINEAR_TYPES
             WHERE nlt_id = c_nlt_id AND nlt_gty_type = nng_group_type;
    BEGIN
        OPEN c1 (p_nlt_id);
    ...
    --------------------------------------------------------------------------------------------------------------------
    --
    FUNCTION join_ptr_array (p_nth   IN NM_THEMES_ALL%ROWTYPE,
                             p_pa    IN ptr_array)
        RETURN ptr_array
    IS
        curstr   VARCHAR2 (2000);
    ...
    BEGIN
        curstr :=
               'select /*+cardinality ( t '
            || TO_CHAR (p_pa.pa.LAST)
            || ') */ ptr( t.ptr_id, t.ptr_value ) from table ( :pa.pa ) t, '
            || p_nth.nth_feature_table
            || ' where t.ptr_value = '
            || p_nth.nth_feature_pk_column
            || ' order by t.ptr_id';
    ...
    FUNCTION Get_Batch_Of_Base_Nn (p_theme   IN NUMBER,
                                   p_geom    IN MDSYS.sdo_geometry)
        RETURN ptr_array
    IS
        nthrow       NM_THEMES_ALL%ROWTYPE;
    ...
    BEGIN
        ...
        IF nthbas.nth_base_table_theme IS NOT NULL
        THEN
      ...
      */
        ...
        EXECUTE IMMEDIATE cur_string
            BULK COLLECT INTO retval.pa
            USING p_geom, g_batch_size;

        IF nthrow.nth_feature_table != nthrow.nth_table_name
        THEN
            retval :=
                local_join_ptr_array (retval,
                                      nthrow.nth_feature_table,
                                      nthrow.nth_feature_pk_column);
        END IF;
    ...
    FUNCTION Get_Batch_Of_Base_Nn (p_theme      IN NUMBER,
                                   p_geom       IN MDSYS.sdo_geometry,
                                   p_ne_array   IN nm_cnct_ne_array_type)
        RETURN ptr_array
    IS
        nthrow       NM_THEMES_ALL%ROWTYPE;
        nthbas       NM_THEMES_ALL%ROWTYPE;
    ...
    BEGIN
        ...
        cur_string :=
               'select /*+cardinality( ne_list '
            || TO_CHAR (p_ne_array.LAST)
            || ')*/ '
            || 'ptr(rownum, ft.'
            || nthrow.nth_feature_pk_column
            || ') from '
    ...
    FUNCTION get_buffer_of_base_ids (p_theme    IN NUMBER,
                                     p_geom     IN MDSYS.sdo_geometry,
                                     p_buffer   IN NUMBER DEFAULT 1)
        RETURN ptr_array
    IS
        nthrow       NM_THEMES_ALL%ROWTYPE;
        nthbas       NM_THEMES_ALL%ROWTYPE;
    ...
    --------------------------------------------------------------------------------------------------------------------------
    --
    FUNCTION get_nearest_nw_to_xy (p_x       IN NUMBER,
                                   p_y       IN NUMBER,
                                   p_theme   IN nm_theme_array)
        RETURN nm_lref
    IS
    ...
    BEGIN
        ...
        IF    lta.nta_theme_array.LAST IS NULL
           OR lta.nta_theme_array.LAST <= 0
           OR lta.nta_theme_array (lta.nta_theme_array.LAST).nthe_id IS NULL
        THEN
            lta := Nm3sdo.get_nw_themes;
        END IF;

        FOR i IN 1 .. lta.nta_theme_array.LAST
        LOOP
            --    Nm_Debug.DEBUG( NVL( TO_CHAR( lta.nta_theme_array(i).nthe_id ), 'NULL'));

            lpk :=
                Nm3sdo.get_nearest_theme_to_xy (
                    lta.nta_theme_array (i).nthe_id,
                    p_x,
                    p_y);
            ...
            ld :=
                Nm3sdo.get_distance (lta.nta_theme_array (i).nthe_id,
                                     lpk,
                                     p_x,
                                     p_y);
    ...
    BEGIN
        cur_string :=
               'select /*+cardinality (a '
            || TO_CHAR (p_id.pa.LAST)
            || ')*/ '
            || ' nm_geom( a.ptr_id, '
            || TO_CHAR (nth.nth_feature_shape_column)
            || ') from '
            || nth.nth_feature_table
            || ', table ( :pa ) a '
            || 'where a.ptr_value = '
    ...
    FUNCTION get_spatial_column_name (p_table_name IN VARCHAR2)
        RETURN VARCHAR2
    IS
        retval   VARCHAR2 (30);
    BEGIN
        SELECT column_name
          INTO retval
          FROM user_tab_columns
         WHERE table_name = p_table_name AND data_type = 'SDO_GEOMETRY';
    ...
    IS
        retval   INTEGER := -1;
    BEGIN
        FOR i IN 1 .. p_ptr.pa.LAST
        LOOP
            IF p_ptr.pa (i).ptr_value = p_value
            THEN
    ...
    BEGIN
        ...
        IF retval.pa (1) IS NOT NULL
        THEN
            FOR i IN 1 .. retval.pa.LAST
            LOOP
    ...
    BEGIN
        ...
        FOR i IN 1 .. p_ga.nga.LAST
        LOOP
            isave := i;

            IF p_ga.nga (i).ng_ne_id = p_id
            THEN
                retval := i;
                EXIT;
            END IF;
    ...
    FUNCTION coalesce_nw_diminfo (p_pct_increase   IN NUMBER DEFAULT 0,
                                  ...
                                  p_m_tol          IN NUMBER DEFAULT NULL)
        RETURN MDSYS.sdo_dim_array
    AS
        retval          MDSYS.sdo_dim_array;
        l_mbr           MDSYS.sdo_geometry;
        ...
        CURSOR c_dim (c_pct IN NUMBER, c_tol IN NUMBER, c_m_tol IN NUMBER)
        IS
            WITH
                sdo_data
                AS
                    (  SELECT row_num,
                              MIN (sdo_lb)            lb,
                              MAX (sdo_ub)            ub,
                              MIN (sdo_tolerance)     tol,
                              srid
                         FROM (  SELECT table_name,
                                        dim_row,
                                        ROW_NUMBER ()
                                            OVER (PARTITION BY table_name
                                                  ORDER BY table_name, dim_row)
                                            row_num,
                                        sdo_lb * (1 - NVL (c_pct, 0) / 100)
                                            sdo_lb,
                                        sdo_ub * (1 + NVL (c_pct, 0) / 100)
                                            sdo_ub,
                                        DECODE (
                                            dim_row,
                                            3, NVL (p_m_tol,
                                                    NVL (p_tol, sdo_tolerance)),
                                            NVL (p_tol, sdo_tolerance))
                                            sdo_tolerance,
                                        srid
                                   FROM (SELECT table_name,
                                                ...
                                                sdo_tolerance,
                                                srid
                                           FROM (  SELECT nth_feature_table,
                                                          nth_feature_shape_column
                                                     FROM NM_NW_THEMES,
                                                          NM_THEMES_ALL,
                                                          NM_LINEAR_TYPES
                                                    WHERE     nnth_nth_theme_id =
                                                              nth_theme_id
                                                          AND nnth_nlt_id = nlt_id
                                                          AND nlt_g_i_d = 'D'
                                                          AND nth_base_table_theme
                     ...
                     ORDER BY 1)
            SELECT CAST (MULTISET (  SELECT mdsys.sdo_dim_element (
                                                TO_CHAR (row_num),
                                                lb,
                                                ub,
                                                tol)
                                       FROM sdo_data
                                   ORDER BY row_num) AS MDSYS.sdo_dim_array)
              FROM DUAL;
    ...
    EXCEPTION
        ...
        THEN
            ...
            raise_application_error (-20001, 'No Network metadata available');
        WHEN ex_3d_diminfo
        THEN
    ...
    BEGIN
        ...
        IF p_nth.nth_table_name = p_nth.nth_feature_table
        THEN
            curstr :=
                   'select /*+cardinality( a '
                || TO_CHAR (p_ntl.ntl_theme_list.LAST)
                || ') */ '
                || ' nm_theme_detail( :new_theme, a.ntd_pk_id, a.ntd_fk_id,  SUBSTR(t.'
                || p_nth.nth_label_column
                || ',1,100), a.ntd_distance, a.ntd_measure, :new_descr )   '
                || ' FROM TABLE ( :p_ntl.ntl_theme_list ) a, '
                || p_nth.nth_feature_table
                || ' t '
                || ' where a.ntd_pk_id  =  t.'
                || p_nth.nth_feature_pk_column
                || ' group by :new_theme, a.ntd_pk_id, a.ntd_fk_id,  SUBSTR(t.'
                || p_nth.nth_label_column
                || ',1,100), a.ntd_distance, a.ntd_measure, :new_descr '
        ...
        ELSE
            ...
            curstr :=
                ...
                || p_nth.nth_feature_table
                || ' f,'
                || p_nth.nth_table_name
                || ' t'
                || ' where a.ntd_pk_id  = f.'
                || p_nth.nth_feature_pk_column
                || ' and   f.'
                || NVL (p_nth.nth_feature_fk_column,
                        p_nth.nth_feature_pk_column)
                || ' =  t.'
                || p_nth.nth_pk_column
                || ' group by :new_theme, a.ntd_pk_id, a.ntd_fk_id,  SUBSTR(t.'
        ...
        EXECUTE IMMEDIATE curstr
            BULK COLLECT INTO retval.ntl_theme_list
            ...
            USING p_nth.nth_theme_id,
                  p_nth.nth_theme_name,
                  p_ntl,
                  p_nth.nth_theme_id,
                  p_nth.nth_theme_name;
    ...
    BEGIN
        ...
        THEN
            ...
            THEN
                --
                IF NVL (l_usgm.srid, Nm3type.c_big_number) !=
                   NVL (p_geom.sdo_srid, Nm3type.c_big_number)
                THEN
                    retval :=
                        '13365: Layer SRID does not match geometry SRID';
                END IF;
            ...
            ELSE
                ...
                THEN
                    ...
                    retval :=
                        SUBSTR (
                            SDO_GEOM.validate_geometry_with_context (p_geom,
                                                                     p_tol),
                            1,
                            100);
                END IF;
        ...
        ELSE
            ...
            retval :=
                SUBSTR (
                    SDO_GEOM.validate_geometry_with_context (p_geom, p_tol),
                    1,
                    100);
        --
        END IF;
    ...
    FUNCTION get_shape_length (p_geom     IN MDSYS.sdo_geometry,
                               p_nth_id   IN nm_themes_all.nth_theme_id%TYPE,
                               p_tol      IN NUMBER)
        RETURN NUMBER
    IS
    ...
    BEGIN
        ...
        THEN
            ...
            l_usgm :=
                get_usgm (l_nth.nth_feature_table,
                          l_nth.nth_feature_shape_column);

            retval := SDO_GEOM.sdo_length (p_geom, l_usgm.diminfo);
        ELSIF p_tol IS NOT NULL
        THEN
    ...
    FUNCTION get_srs_text (p_theme_id IN NM_THEMES_ALL.nth_theme_id%TYPE)
        RETURN cs_srs.wktext%TYPE
    IS
    ...
    BEGIN
        ...
        ELSE
            ...
            LOOP
                ...
                retval.nga (i) :=
                    nm_geom (i, SDO_UTIL.EXTRACT (p_shape, i, 1));
            END LOOP;
        END IF;
    ...
    BEGIN
        Nm_Debug.DEBUG (
               'compare '
            || TO_CHAR (p_geom1.sdo_point.x)
            || ' with '
            || TO_CHAR (p_geom2.sdo_point.x)
            || ' and  '
            || TO_CHAR (p_geom1.sdo_point.y)
            || ' with '
            || TO_CHAR (p_geom2.sdo_point.y));

        IF     ABS (p_geom1.sdo_point.x - p_geom2.sdo_point.x) <= l_tol
           AND ABS (p_geom1.sdo_point.y - p_geom2.sdo_point.y) <= l_tol
        THEN
            retval := 'TRUE';
        END IF;
