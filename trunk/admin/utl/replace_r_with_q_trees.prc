CREATE OR REPLACE procedure replace_R_with_Q_trees( p_level in number default 6 ) 
--
--------------------------------------------------------------------------------
--   PVCS Identifiers :-
--
...
begin
  ...
  if hig.GET_APPLICATION_OWNER_ID = nm3user.GET_USER_ID then
    ...
    begin
      ...
      (hov_id, hov_value )
      values 
      ('SDOQTTILES', to_char(p_level) );
    exception
      when dup_val_on_index then
        update hig_option_values
        set hov_value = to_char(p_level)
    ...
    open get_rts;
    fetch get_rts bulk collect into l_nth_theme_id, l_nth_feature_table, l_nth_feature_shape_column, l_index_name, l_sdo_index_type;
    close get_rts;
    ...
    if l_nth_theme_id.count > 0 then
      ...
      for i in 1..l_nth_theme_id.count loop
        ...
        begin
          set_progress('Create Qtree on  '||l_nth_feature_table(i));
          nm3sdo.CREATE_QTREE_IDX(l_nth_feature_table(i), l_nth_feature_shape_column(i));
        exception
          when others then
