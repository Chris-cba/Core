--   PVCS Identifiers :-
--
--       sccsid           : $Header:   //new_vm_latest/archives/lb/install/exnm04070012en_updt55.sql-arc   1.0   May 17 2018 10:28:18   Rob.Coupe  $
--       Module Name      : $Workfile:   exnm04070012en_updt55.sql  $ 
--       Date into PVCS   : $Date:   May 17 2018 10:28:18  $
--       Date fetched Out : $Modtime:   May 17 2018 10:26:56  $
...
Begin
  ...
  and hpr_version in ('4.2', '4,3', '4.4', '4.5', '4.7.0.0', '4.7.0.1', '4.7.0.2', '4.7.0.3', '4.7.0.4', '4.7.0.5');  
...
--
  ...
  BEGIN
	...
	BEGIN
	   select status into lb_get_status
	   from user_objects where object_name = 'LB_GET' and object_type = 'PACKAGE_BODY';
--
       if lb_get_status = 'INVALID' then
...
--	     
    EXCEPTION
	   when others
	   then
	   raise_application_error( -20003, 'Problems in the status of LB' );
    END;
--	
...
--  
  Exception
    ...
    when others then
      raise_application_error( -20004, 'There seems a problem with the LB code, please re-compile prior to running this upgrade '||sqlerrm);
  end;
...
--      
      execute immediate 'CREATE OR REPLACE '||chr(13)||chr(10)
                      ||'TYPE LB_XRPT as object ( '||chr(13)||chr(10)
                      ||'   refnt integer, '||chr(13)||chr(10)
                      ||'   refnt_type integer, '||chr(13)||chr(10)
                      ||'   obj_type varchar2(4), '||chr(13)||chr(10)
                      ||'   obj_id integer, '||chr(13)||chr(10)
                      ||'   seg_id integer, '||chr(13)||chr(10)
                      ||'   seq_id integer, '||chr(13)||chr(10)
                      ||'   dir_flag integer, '||chr(13)||chr(10)
                      ||'   start_m number, '||chr(13)||chr(10)
                      ||'   end_m number, '||chr(13)||chr(10)
                      ||'   m_unit integer, '||chr(13)||chr(10)
                      ||'   xsp varchar2(4), '||chr(13)||chr(10)
                      ||'   offset number, '||chr(13)||chr(10)
                      ||'   start_date date, '||chr(13)||chr(10)
                      ||'   end_date date  '||chr(13)||chr(10)
                      ||')';
end;            
/
...
insert into lb_objects(object_name, object_type)
select 'V_NLT_XSPS', 'VIEW' from dual
where not exists ( select 1 from lb_objects where object_name = 'V_NLT_XSPS' and object_type = 'VIEW') ;

insert into lb_objects(object_name, object_type)
select 'V_NLT_ELEMENT_XSPS', 'VIEW' from dual
where not exists ( select 1 from lb_objects where object_name = 'V_NLT_ELEMENT_XSPS' and object_type = 'VIEW') ;

insert into lb_objects(object_name, object_type)
select 'V_NLT_XSP_RVRS', 'VIEW' from dual
where not exists ( select 1 from lb_objects where object_name = 'V_NLT_XSP_RVRS' and object_type = 'VIEW') ;

insert into lb_objects(object_name, object_type)
select 'V_NM_ELEMENT_XSP', 'VIEW' from dual
where not exists ( select 1 from lb_objects where object_name = 'V_NM_ELEMENT_XSP' and object_type = 'VIEW') ;

insert into lb_objects(object_name, object_type)
select 'V_NM_ELEMENT_XSP_RVRS', 'VIEW' from dual
where not exists ( select 1 from lb_objects where object_name = 'V_NM_ELEMENT_XSP_RVRS' and object_type = 'VIEW') ;
...
insert into lb_objects(object_name, object_type)
select 'LB_XRPT', 'TYPE' from dual
where not exists ( select 1 from lb_objects where object_name = 'LB_XRPT' and object_type = 'TYPE') ;

insert into lb_objects(object_name, object_type)
select 'LB_XRPT_TAB', 'TYPE' from dual
where not exists ( select 1 from lb_objects where object_name = 'LB_XRPT_TAB' and object_type = 'TYPE') ;
...
where o.object_name in ( 'LB_NW_EDIT', 'LB_ELEMENT_HISTORY','LB_EDIT_TRANSACTION','LB_EDIT_TRANSACTION_TAB', 'MAKE_NW_FROM_LREFS', 'V_NM_ELEMENT_XSP', 'V_NM_ELEMENT_XSP_RVRS',
                         'LB_XRPT', 'LB_XRPT_TAB', 'V_NLT_XSPS', 'V_NLT_ELEMENT_XSPS', 'V_NLT_XSP_RVRS' )
...
--
  hig2.upgrade(p_product        => 'LB'
              ,p_upgrade_script => 'exnm04070012en_updt55.sql'
              ,p_remarks        => 'NET 4700 FIX 55 Build 12 - LB version 4.7.0.6'
              ,p_to_version     => '4.7.0.6');
--
  commit;
--
EXCEPTION
